#!/usr/bin/env python3
"""Train C39-CMEQ as direct, independent formal validation shards."""
...
from dmea_ht.c39_cmeq import (  # noqa: E402
    C39CMEQModel,
    HEAD_PREFIXES,
    MODALITIES,
    PAIR_NAMES,
...
def gradient_norm(model: C39CMEQModel) -> float:
    total = 0.0
...
def parameter_drift_rows(
    model: C39CMEQModel, initial_state: Mapping[str, torch.Tensor], seed: int
) -> List[Dict[str, Any]]:
    ...
    for batch in loader:
        ...
        with torch.set_grad_enabled(is_train):
            ...
            if not bool(torch.isfinite(loss)):
                raise RuntimeError("C39 non-finite BCE loss")
            if is_train:
        ...
        modality_states.append(arrays["modality_states"])
        pair_states.append(arrays["pair_states"])
        for index, patient_id in enumerate(batch["patient_id"]):
            ...
            history_entropy = float(
                -(history_weights.clip(1e-8, 1.0) * np.log(history_weights.clip(1e-8, 1.0))).sum()
            )
            row: Dict[str, Any] = {
                "patient_id": str(patient_id),
                "label": int(labels[index]),
                "visit_count_audit_only": count,
                "valid_modality_count": int(arrays["modality_valid"][index].sum()),
                "pair_valid_count": int(arrays["pair_valid"][index].sum()),
                ...
                "history_kernel_entropy": history_entropy,
            }
            for modality_index, modality in enumerate(MODALITIES):
                row[f"modality_state_norm_{modality}"] = float(
                    np.linalg.norm(arrays["modality_states"][index, modality_index])
                )
                row[f"latest_state_norm_{modality}"] = float(
                    np.linalg.norm(arrays["latest_states"][index, modality_index])
                )
                row[f"history_state_norm_{modality}"] = float(
                    np.linalg.norm(arrays["history_states"][index, modality_index])
                )
                row[f"modality_valid_{modality}"] = bool(
                    arrays["modality_valid"][index, modality_index]
                )
            for pair_index, pair in enumerate(PAIR_NAMES):
                row[f"pair_state_norm_{pair}"] = float(
                    np.linalg.norm(arrays["pair_states"][index, pair_index])
                )
                row[f"pair_valid_{pair}"] = bool(arrays["pair_valid"][index, pair_index])
...
def train_seed(
    ...
    if not names or any(not name.startswith(HEAD_PREFIXES) for name in names):
        raise RuntimeError(f"C39 trainable scope violation: {names}")
    if any(
        parameter.requires_grad
        for name, parameter in model.named_parameters()
        if name.startswith("sources.")
    ):
        raise RuntimeError("C39 modality sources must remain frozen")
    count = trainable_parameter_count(model)
    if count > int(config["c39"]["trainable_parameter_limit"]):
        raise RuntimeError(f"C39 capacity contract failed: {count}")
    initial_state = {
    ...
    if best_state is None:
        raise RuntimeError(f"C39 seed {seed} produced no checkpoint")
    model.load_state_dict(best_state, strict=True)
    for row in epoch_rows:
        row["selected_by_val_auc"] = int(row["epoch"]) == best_epoch
    val_result = run_epoch(model, loaders["val"], None, device)
    if val_result["metrics"]["prediction_std"] <= 0.0:
        raise RuntimeError(f"C39 seed {seed} produced constant validation predictions")
...
def save_split(result: Dict[str, Any], out_dir: Path, split: str) -> Dict[str, Any]:
    ...
    np.savez_compressed(
        ...
        pair_states=split_result["pair_states"][order].astype(np.float32),
    )
...
def validation_seed_stage(
    ...
    if seed_dir.exists():
        raise RuntimeError(f"C39 seed output already exists: {seed_dir}")
    ...
    status_path.write_text(json.dumps(status, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"status": "C39_VALIDATION_SEED_COMPLETE", "seed": seed}))
...
def validation_finalize_stage(config: Dict[str, Any], out_dir: Path, device: torch.device) -> None:
    ...
    for seed in SEEDS:
        ...
        if status.get("status") != "COMPLETE":
            raise RuntimeError(f"C39 seed {seed} shard incomplete")
    ...
    pd.concat(drift_parts, ignore_index=True).sort_values(["seed", "parameter_name"]).to_csv(
    ...
    print(json.dumps({"status": "C39_VALIDATION_COMPLETE", "seeds": list(SEEDS)}))


def reporting_test_stage(
    config: Dict[str, Any], rows: Sequence[Dict[str, Any]], out_dir: Path, device: torch.device
) -> None:
    decision_path = resolve_path(config["project"]["report_dir"]) / "c39_validation_decision.json"
    if not decision_path.exists():
        raise RuntimeError("C39 validation decision must be frozen before reporting-only test")
    ...
    if (
        ...
        raise RuntimeError("C39 validation/test isolation contract failed")
    metrics_path = out_dir / "reports" / "metrics_by_seed.csv"
    metrics = pd.read_csv(metrics_path)
    if set(metrics["split"]) != {"val"}:
        raise RuntimeError("C39 reporting-only test requires validation-only metrics")
    loader = build_loaders(config, rows, ("test",))["test"]
    for seed in SEEDS:
        model = C39CMEQModel(config, seed).to(device)
        payload = checkpoint_payload(out_dir / "checkpoints" / f"seed_{seed}_best.pt")
        if int(payload.get("seed", -1)) != seed:
            raise RuntimeError(f"C39 checkpoint seed mismatch for {seed}")
        ...
        metrics = pd.concat(
            [
                metrics,
                pd.DataFrame(
                    [save_split({"seed": seed, "best_epoch": payload["best_epoch"], "test": result}, out_dir, "test")]
                ),
            ],
            ignore_index=True,
        )
    ...
    status_path.write_text(json.dumps(status, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"status": "C39_REPORTING_TEST_COMPLETE", "seeds": list(SEEDS)}))
...
def direct_multiseed_stage(
    ...
    gate_path = resolve_path(config["project"]["report_dir"]) / "c39_gate.json"
    if not gate_path.exists():
        raise RuntimeError("C39 direct execution requires the completed gate")
    gate = json.loads(gate_path.read_text(encoding="utf-8"))
    if gate.get("status") != "C39_CMEQ_DIRECT_MULTI_SEED_AUTHORIZED" or int(gate.get("passed", 0)) != int(gate.get("total", 0)):
        raise RuntimeError("C39 direct execution requires an authorized gate")
    if (out_dir / "seed_runs").exists():
        raise RuntimeError("C39 formal seed outputs already exist")
    for child in ("reports", "predictions", "checkpoints", "representations"):
    ...
    if any(code != 0 for code in codes):
        raise RuntimeError(f"C39 validation shard failed: {codes}")
    subprocess.run(
        [sys.executable, str(script), "--config", str(config_path), "--stage", "validation-finalize"],
        check=True,
    )
    collector = REPO_ROOT / "scripts" / "collect_phase_c39_report.py"
    subprocess.run(
    ...
    print(json.dumps({"status": "C39_DIRECT_MULTI_SEED_COMPLETE", "seeds": list(SEEDS)}))
...
def main() -> None:
    ...
    if str(config.get("phase", "")).lower() != "c39":
        raise RuntimeError("C39 phase contract is missing")
    if [int(seed) for seed in config["training"]["seeds"]] != list(SEEDS):
        raise RuntimeError("C39 formal seeds must remain [0, 42, 3407]")
    ...
    if args.stage == "validation-seed":
        if args.seed not in SEEDS:
            raise RuntimeError(f"Unsupported C39 seed: {args.seed}")

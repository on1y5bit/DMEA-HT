#!/usr/bin/env python3
"""Authorize C43-MCFE direct formal execution with a compact contract gate."""
...
from dmea_ht.c43_mcfe import C43MCFEModel, HEAD_PREFIXES, trainable_parameter_count  # noqa: E402
...
def main() -> None:
    ...
    source_text = inspect.getsource(C43MCFEModel)
    for seed in SEEDS:
        set_seed(seed)
    ...
    scope_pass &= bool(trainable_counts) and max(trainable_counts.values()) <= int(config["c43"]["trainable_parameter_limit"])
    train_source = (REPO_ROOT / "scripts" / "train_phase_c43.py").read_text(encoding="utf-8")
    base_train_source = (REPO_ROOT / "scripts" / "train_phase_c40.py").read_text(encoding="utf-8")
    collector_source = (REPO_ROOT / "scripts" / "collect_phase_c43_report.py").read_text(encoding="utf-8")
    ...
    ) and bool(config["c43"]["shortcut_fields_used_as_inputs"] is False)
    route_pass = (
        all(term in source_text for term in ("bio_conditioner", "image_film", "text_film", "cross_modal_fusion", "history_mask", "variance", "log2"))
        and "requires_grad_(True)" not in source_text
        and "torch.no_grad" in source_text
        and all(term not in source_text for term in ("C27", "C37", "C38", "C39", "C40", "C42", "evidence_graph", "patient_query", "visit_score", "temporal_linear"))
        and config["c43"]["learned_visit_score"] is False
    )
    test_blocked = (
        "C43 Validation decision must be frozen before reporting-only Test" in train_source
        and "set(metrics[\"split\"]) != {\"val\"}" in train_source
        and "c43_validation_decision.json" in train_source
    )
    direct_contract = (
    ...
    checks = [
        ...
        ("06_new_fusion_gradients", gradient_pass),
        ("07_shortcut_fields_excluded", shortcut_pass),
        ("08_within_visit_mechanism_flow_contract", route_pass),
    ...
    pd.DataFrame(gradient_rows).to_csv(report_dir / "c43_gradient_audit.csv", index=False)
    passed = sum(bool(value) for _, value in checks)
    status = "C43_MCFE_DIRECT_MULTI_SEED_AUTHORIZED" if passed == len(checks) else "C43_PATH_GATE_FAIL"
    payload = {
        "phase": "C43-MCFE",
        "status": status,
        "passed": passed,
    ...
    (report_dir / "c43_gate.json").write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"status": status, "passed": passed, "total": len(checks)}))
    if status != "C43_MCFE_DIRECT_MULTI_SEED_AUTHORIZED":
        raise SystemExit(2)

#!/usr/bin/env python3
"""Freeze and report C42-E2E-PET Validation and reporting-only Test evidence."""
...
def freeze_validation_decision(config: Mapping[str, Any], run_dir: Path, report_dir: Path) -> Dict[str, Any]:
    ...
    if set(metrics["split"]) != {"val"}:
        raise RuntimeError("C42 Validation decision requires Validation-only metrics")
    comparisons, positive, inversions = base.validation_comparisons(config, run_dir, metrics)
    comparisons = rename_c41_columns(comparisons, "C41", "C42")
    positive = rename_c41_columns(positive, "c41", "c42")
    inversions = rename_c41_columns(inversions, "C41", "C42")
    ...
    shortcuts["combination"] = "C42-E2E-PET"
    ...
    for path in sorted((run_dir / "seed_runs").glob("seed_*/reports/run_config.json")):
        ...
        capacity_pass &= int(runtime["trainable_parameter_count"]) <= int(config["c42"]["trainable_parameter_limit"])
    if not capacity_pass or not health_pass:
        label = "C42_TRAINING_INVALID"
    elif not shortcut_pass:
        label = "C42_SHORTCUT_CONCERN"
    elif not positive_pass:
        label = "C42_POSITIVE_DAMAGE"
    elif not ranking_pass:
        label = "C42_RANKING_DAMAGE"
    elif not auc_pass:
        label = "C42_NO_AUC_GAIN"
    else:
    ...
    decision = {
        ...
        "strict_best": "C42_E2E_PET" if promoted else "KEEP_DEMA_C17_STRICT_BEST",
    ...
    comparisons.to_csv(report_dir / "c42_metrics_by_seed.csv", index=False)
    pd.DataFrame([{"split": "val", "AUC_mean": mean_auc, "AUC_std": std_auc, "C17_AUC_mean": float(comparisons["C17_AUC"].mean()), "C27_AUC_mean": float(comparisons["C27_AUC"].mean()), "C42_minus_C27_AUC_mean": float(comparisons["C42_minus_C27_AUC"].mean())}]).to_csv(report_dir / "c42_metrics_summary.csv", index=False)
    epoch.to_csv(report_dir / "c42_metrics_by_epoch.csv", index=False)
    pd.read_csv(run_dir / "reports" / "parameter_drift.csv").to_csv(report_dir / "c42_parameter_drift.csv", index=False)
    pd.read_csv(run_dir / "reports" / "patient_diagnostics_val.csv").to_csv(report_dir / "c42_patient_diagnostics_val.csv", index=False)
    health.to_csv(report_dir / "c42_training_health.csv", index=False)
    positive.to_csv(report_dir / "c42_positive_preservation.csv", index=False)
    inversions.to_csv(report_dir / "c42_pairwise_inversion_summary.csv", index=False)
    shortcuts.to_csv(report_dir / "c42_shortcut_audit.csv", index=False)
    (report_dir / "c42_validation_decision.json").write_text(json.dumps(decision, indent=2) + "\n", encoding="utf-8")
    (report_dir / "c42_route_decision.md").write_text(
        "\n".join(
            [
                "# C42-E2E-PET Validation Decision",
                "",
                f"- Decision: `{label}`.",
...
def write_final_report(config: Mapping[str, Any], run_dir: Path, report_dir: Path) -> Dict[str, Any]:
    decision = json.loads((report_dir / "c42_validation_decision.json").read_text(encoding="utf-8"))
    metrics = pd.read_csv(run_dir / "reports" / "metrics_by_seed.csv")
    if set(metrics["split"]) != {"val", "test"}:
        raise RuntimeError("C42 final report requires Validation and reporting-only rows")
    test = metrics[metrics["split"] == "test"]
    summary = pd.read_csv(report_dir / "c42_metrics_summary.csv")
    summary = pd.concat(
    ...
    summary.to_csv(report_dir / "c42_metrics_summary.csv", index=False)
    positive = pd.read_csv(report_dir / "c42_positive_preservation.csv")
    inversions = pd.read_csv(report_dir / "c42_pairwise_inversion_summary.csv")
    health = pd.read_csv(report_dir / "c42_training_health.csv")
    shortcut = pd.read_csv(report_dir / "c42_shortcut_audit.csv")
    lines = [
        "# DMEA-HT Phase C42-E2E-PET Final Report",
        "",
        ...
        f"- Aggregate C17 TP-to-C42 FN / FN-to-C42 TP: `{int(positive['c17_tp_to_c42_fn'].sum())}`/`{int(positive['c17_fn_to_c42_tp'].sum())}`.",
        f"- C27-to-C42 repaired/introduced pairs: `{int(inversions['C27_to_C42_repaired'].sum())}`/`{int(inversions['C27_to_C42_introduced'].sum())}`.",
    ...
    (report_dir / "phase_c42_dema_final_report.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
    return decision
...
def main() -> None:
    ...
    if str(config.get("phase", "")).lower() != "c42":
        raise RuntimeError("C42 report requires the formal C42 config")
    ...
    if args.stage == "validation":
        ...
        print(json.dumps({"status": "C42_VALIDATION_DECISION_FROZEN", "decision": decision["decision_label"]}))
    else:
        decision = write_final_report(config, run_dir, report_dir)
        print(json.dumps({"status": "C42_FINAL_REPORT_COMPLETE", "decision": decision["decision_label"]}))

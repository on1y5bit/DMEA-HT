#!/usr/bin/env python3
"""Authorize C54-LRRA direct formal execution with a strict contract gate."""
...
def main() -> None:
    ...
    scope_pass &= bool(trainable_counts) and max(trainable_counts.values()) <= int(config["c54"]["trainable_parameter_limit"])
    train_source = (REPO_ROOT / "scripts" / "train_phase_c54.py").read_text(encoding="utf-8")
    ...
    route_pass = (
        ...
        and config["c54"]["learned_visit_score"] is False
        and config["c54"]["temporal_attention"] is False
        and config["c54"]["router"] is False
    )
    test_blocked = (
        "C54 Validation decision must be frozen before reporting-only Test" in train_source
        and 'set(metrics["split"]) != {"val"}' in train_source
        and "c54_validation_decision.json" in train_source
    )
    direct_contract = (
    ...
    checks = [
        ...
        ("06_low_rank_adapter_gradients_and_frozen_source", gradient_pass),
        ("07_shortcut_fields_excluded", shortcut_pass),
        ("08_low_rank_representation_adaptation_contract", route_pass),
    ...
    pd.DataFrame(gradient_rows).to_csv(report_dir / "c54_gradient_audit.csv", index=False)
    passed = sum(bool(value) for _, value in checks)
    status = "C54_LRRA_DIRECT_MULTI_SEED_AUTHORIZED" if passed == len(checks) else "C54_PATH_GATE_FAIL"
    payload = {
        "phase": "C54-LRRA",
        "status": status,
        "passed": passed,
    ...
    (report_dir / "c54_gate.json").write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"status": status, "passed": passed, "total": len(checks)}))
    if status != "C54_LRRA_DIRECT_MULTI_SEED_AUTHORIZED":
        raise SystemExit(2)

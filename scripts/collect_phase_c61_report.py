#!/usr/bin/env python3
"""Freeze and report C61-CBPI Validation and reporting-only Test evidence."""
...
def transform(value: Any) -> Any:
    ...
    if isinstance(value, str):
        return value.replace("C54", "C61").replace("c54", "c61").replace("LRRA", "CBPI")
    return value


def materialize_c61(report_dir: Path) -> None:
    for source_name in REPORT_FILES:
        source = report_dir / source_name
        if not source.exists():
            continue
        target_name = source_name.replace("c54", "c61").replace("C54", "C61")
...
def main() -> None:
    ...
    if str(config.get("phase", "")).lower() != "c61":
        raise RuntimeError("C61 report requires the formal C61 config")
    ...
    if args.stage == "validation":
        ...
        materialize_c61(report_dir)
        print(json.dumps({"status": "C61_VALIDATION_DECISION_FROZEN", "decision": transform(decision["decision_label"])}))
    else:
        decision = shared.write_final_report(translated, run_dir, report_dir)
        materialize_c61(report_dir)
        print(json.dumps({"status": "C61_FINAL_REPORT_COMPLETE", "decision": transform(decision["decision_label"])}))

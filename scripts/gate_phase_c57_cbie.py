#!/usr/bin/env python3
"""Authorize C57-CBIE direct formal execution with a strict contract gate."""
...
from dmea_ht.c57_cbie import C57CBIEModel, HEAD_PREFIXES, trainable_parameter_count  # noqa: E402
...
def main() -> None:
    ...
    source_text = inspect.getsource(C57CBIEModel)
    ...
    scope_pass &= bool(trainable_counts) and max(trainable_counts.values()) <= int(config["c57"]["trainable_parameter_limit"])
    train_source = (REPO_ROOT / "scripts" / "train_phase_c57.py").read_text(encoding="utf-8")
    ...
    ) and bool(config["c57"]["shortcut_fields_used_as_inputs"] is False)
    route_pass = (
        all(term in source_text for term in ("C57CBIEModel", "_image_text_kernel", "bio_values", "bio_nonlinear", "image_text_projection", "bio_projection", "joint_state", "tanh"))
        and all(term in inherited_text for term in ("latest_mask", "history_mask", "variance", "log2", "raw_image", "aligned_image"))
        and list(config["c57"]["stream_order"]) == ["raw_image", "raw_text", "raw_bio", "aligned_image", "aligned_text", "aligned_bio"]
        and "visit_score" not in route_text
        and "temporal" not in route_text.lower()
        and "router" not in route_text.lower()
        and all(term not in route_text for term in ("C27", "C37", "C38", "C39", "C40", "C42", "C43", "C44", "C50", "FiLM", "MultiheadAttention"))
        and config["c57"]["learned_visit_score"] is False
        and config["c57"]["temporal_attention"] is False
        and config["c57"]["router"] is False
    )
    test_blocked = (
        "C57 Validation decision must be frozen before reporting-only Test" in train_source
        and 'set(metrics["split"]) != {"val"}' in train_source
        and "c57_validation_decision.json" in train_source
    )
    direct_contract = (
    ...
    checks = [
        ...
        ("06_continuous_bio_gradients_and_frozen_source", gradient_pass),
        ("07_shortcut_fields_excluded", shortcut_pass),
        ("08_continuous_bio_interaction_contract", route_pass),
    ...
    pd.DataFrame(gradient_rows).to_csv(report_dir / "c57_gradient_audit.csv", index=False)
    passed = sum(bool(value) for _, value in checks)
    status = "C57_CBIE_DIRECT_MULTI_SEED_AUTHORIZED" if passed == len(checks) else "C57_PATH_GATE_FAIL"
    payload = {
        "phase": "C57-CBIE",
        "status": status,
        "passed": passed,
    ...
    (report_dir / "c57_gate.json").write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"status": status, "passed": passed, "total": len(checks)}))
    if status != "C57_CBIE_DIRECT_MULTI_SEED_AUTHORIZED":
        raise SystemExit(2)

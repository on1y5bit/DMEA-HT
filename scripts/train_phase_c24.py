#!/usr/bin/env python3
"""Train the authorized C24 decision-side-preserving residual on the server."""
...
def run_epoch(
    model: C24DecisionSideResidualModel,
    loader: DataLoader,
    ...
    for batch in loader:
        ...
        with torch.set_grad_enabled(is_train):
            outputs = model(batch)
            terms = c24_loss_terms(outputs, batch, loss_cfg)
            if is_train:
        ...
        for index, patient_id in enumerate(batch["patient_id"]):
            ...
            confidence_bound = float(arrays["confidence_bound"][index])
            boundary_bound = float(arrays["boundary_bound"][index])
            safe_bound = float(arrays["safe_local_bound"][index])
            active_bound = (
                "zero_bound" if safe_bound <= 1e-12
                else ("boundary_limited" if boundary_bound <= confidence_bound else "confidence_limited")
            )
            base_logit = float(arrays["frozen_c17_logit"][index])
            final_logit = float(arrays["logit"][index])
            base_class = int(base_logit >= 0.0)
            final_class = int(final_logit >= 0.0)
            row: Dict[str, Any] = {
                "patient_id": str(patient_id),
                "label": int(labels[index]),
                "frozen_c17_logit": float(arrays["frozen_c17_logit"][index]),
                "frozen_c17_prob": float(arrays["frozen_c17_prob"][index]),
                "frozen_c17_predicted_class": base_class,
                "abs_frozen_c17_logit": abs_logit,
                "confidence_gate": float(arrays["confidence_gate"][index]),
                "confidence_bound": confidence_bound,
                "boundary_bound": boundary_bound,
                "safe_local_bound": safe_bound,
                "active_bound_type": active_bound,
                "confidence_group": group,
                "mechanism_representation_norm": float(arrays["mechanism_representation_norm"][index]),
                "raw_delta_c24": float(arrays["raw_delta_c24"][index]),
                "delta_c24": float(arrays["delta_c24"][index]),
                "logit": final_logit,
                "prob": float(arrays["prob"][index]),
                "final_predicted_class": final_class,
                "decision_side_preserved": bool(base_class == final_class),
            }
    ...
    delta = frame["delta_c24"].to_numpy(dtype=float)
    gate = frame["confidence_gate"].to_numpy(dtype=float)
    confidence_bound = frame["confidence_bound"].to_numpy(dtype=float)
    boundary_bound = frame["boundary_bound"].to_numpy(dtype=float)
    safe_bound = frame["safe_local_bound"].to_numpy(dtype=float)
    ...
    metrics.update(
        {
            ...
            "confidence_bound_mean": _mean(confidence_bound),
            "boundary_bound_mean": _mean(boundary_bound),
            "safe_local_bound_mean": _mean(safe_bound),
            ...
            "fraction_confidence_limited": _mean(frame["active_bound_type"].eq("confidence_limited").to_numpy(dtype=float)),
            "fraction_boundary_limited": _mean(frame["active_bound_type"].eq("boundary_limited").to_numpy(dtype=float)),
            "fraction_zero_bound": _mean(frame["active_bound_type"].eq("zero_bound").to_numpy(dtype=float)),
            ...
            "decision_side_violation_count": int((~frame["decision_side_preserved"].astype(bool)).sum()),
            "pairwise_inversion_count": pairwise_inversions(y, p),
            "n_rows": int(len(frame)),
        }
    )
    if metrics["decision_side_violation_count"]:
        raise RuntimeError(f"C24 decision-side violation count: {metrics['decision_side_violation_count']}")
...
def train_seed(
    ...
    if not trainable or any(not name.startswith("residual_head.") for name, _ in trainable):
        raise RuntimeError(f"C24 trainable scope violation: {[name for name, _ in trainable]}")
    ...
    if best_state is None:
        raise RuntimeError(f"C24 seed {seed} produced no validation-selected checkpoint")
...
def main() -> None:
    ...
    if str(config.get("phase", "")).lower() != "c24" or not config.get("c24"):
        raise RuntimeError("C24 config/phase contract is missing")
    ...
    status: Dict[str, Any] = {
        "phase": "C24", "status": "RUNNING", "started_at": started, "completed_seeds": [],

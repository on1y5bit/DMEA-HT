#!/usr/bin/env python3
"""Freeze and report C58-MLTF Validation and reporting-only Test evidence."""
...
def transform(value: Any) -> Any:
    ...
    if isinstance(value, str):
        return value.replace("C54", "C58").replace("c54", "c58").replace("LRRA", "MLTF")
    return value


def materialize_c58(report_dir: Path) -> None:
    for source_name in REPORT_FILES:
        source = report_dir / source_name
        if not source.exists():
            continue
        target_name = source_name.replace("c54", "c58").replace("C54", "C58")
...
def main() -> None:
    ...
    if str(config.get("phase", "")).lower() != "c58":
        raise RuntimeError("C58 report requires the formal C58 config")
    ...
    if args.stage == "validation":
        ...
        materialize_c58(report_dir)
        print(json.dumps({"status": "C58_VALIDATION_DECISION_FROZEN", "decision": transform(decision["decision_label"])}))
    else:
        decision = shared.write_final_report(translated, run_dir, report_dir)
        materialize_c58(report_dir)
        print(json.dumps({"status": "C58_FINAL_REPORT_COMPLETE", "decision": transform(decision["decision_label"])}))

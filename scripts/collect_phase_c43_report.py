#!/usr/bin/env python3
"""Freeze and report C43-MCFE Validation and reporting-only Test evidence."""
...
def freeze_validation_decision(config: Mapping[str, Any], run_dir: Path, report_dir: Path) -> Dict[str, Any]:
    ...
    if set(metrics["split"]) != {"val"}:
        raise RuntimeError("C43 Validation decision requires Validation-only metrics")
    comparisons, positive, inversions = base.validation_comparisons(config, run_dir, metrics)
    comparisons = rename_c41_columns(comparisons, "C41", "C43")
    positive = rename_c41_columns(positive, "c41", "c43")
    inversions = rename_c41_columns(inversions, "C41", "C43")
    ...
    shortcuts["combination"] = "C43-MCFE"
    ...
    positive_pass = bool(
        float(positive["c43_minus_c17_sensitivity"].min()) >= -0.10
        and int(positive["c17_tp_to_c43_fn"].sum()) <= int(positive["c17_fn_to_c43_tp"].sum()) + 3
    )
    mean_c27_inversions = float(inversions["C27_inversions"].mean())
    mean_c43_inversions = float(inversions["C43_inversions"].mean())
    ranking_pass = bool(
        (mean_c43_inversions - mean_c27_inversions) / max(mean_c27_inversions, 1.0) <= 0.10
        and int(inversions["C43_minus_C27_inversions"].max()) <= 20
    )
    ...
    for path in sorted((run_dir / "seed_runs").glob("seed_*/reports/run_config.json")):
        ...
        capacity_pass &= int(runtime["trainable_parameter_count"]) <= int(config["c43"]["trainable_parameter_limit"])
    if not capacity_pass or not health_pass:
        label = "C43_TRAINING_INVALID"
    elif not shortcut_pass:
        label = "C43_SHORTCUT_CONCERN"
    elif not positive_pass:
        label = "C43_POSITIVE_DAMAGE"
    elif not ranking_pass:
        label = "C43_RANKING_DAMAGE"
    elif not auc_pass:
        label = "C43_NO_AUC_GAIN"
    else:
    ...
    decision = {
        ...
        "strict_best": "C43_MCFE" if promoted else "KEEP_DEMA_C17_STRICT_BEST",
    ...
    comparisons.to_csv(report_dir / "c43_metrics_by_seed.csv", index=False)
    pd.DataFrame(
        [
            {
                "split": "val",
                ...
                "C43_minus_C27_AUC_mean": float(comparisons["C43_minus_C27_AUC"].mean()),
            }
        ]
    ).to_csv(report_dir / "c43_metrics_summary.csv", index=False)
    epoch.to_csv(report_dir / "c43_metrics_by_epoch.csv", index=False)
    pd.read_csv(run_dir / "reports" / "parameter_drift.csv").to_csv(report_dir / "c43_parameter_drift.csv", index=False)
    pd.read_csv(run_dir / "reports" / "patient_diagnostics_val.csv").to_csv(report_dir / "c43_patient_diagnostics_val.csv", index=False)
    health.to_csv(report_dir / "c43_training_health.csv", index=False)
    positive.to_csv(report_dir / "c43_positive_preservation.csv", index=False)
    inversions.to_csv(report_dir / "c43_pairwise_inversion_summary.csv", index=False)
    shortcuts.to_csv(report_dir / "c43_shortcut_audit.csv", index=False)
    (report_dir / "c43_validation_decision.json").write_text(json.dumps(decision, indent=2) + "\n", encoding="utf-8")
    (report_dir / "c43_route_decision.md").write_text(
        "\n".join(
            [
                "# C43-MCFE Validation Decision",
                "",
                f"- Decision: `{label}`.",
...
def write_final_report(config: Mapping[str, Any], run_dir: Path, report_dir: Path) -> Dict[str, Any]:
    decision = json.loads((report_dir / "c43_validation_decision.json").read_text(encoding="utf-8"))
    metrics = pd.read_csv(run_dir / "reports" / "metrics_by_seed.csv")
    if set(metrics["split"]) != {"val", "test"}:
        raise RuntimeError("C43 final report requires Validation and reporting-only rows")
    test = metrics[metrics["split"] == "test"]
    summary = pd.read_csv(report_dir / "c43_metrics_summary.csv")
    summary = pd.concat(
        [
            summary,
            pd.DataFrame(
    ...
    summary.to_csv(report_dir / "c43_metrics_summary.csv", index=False)
    positive = pd.read_csv(report_dir / "c43_positive_preservation.csv")
    inversions = pd.read_csv(report_dir / "c43_pairwise_inversion_summary.csv")
    health = pd.read_csv(report_dir / "c43_training_health.csv")
    shortcut = pd.read_csv(report_dir / "c43_shortcut_audit.csv")
    lines = [
        "# DMEA-HT Phase C43-MCFE Final Report",
        "",
        ...
        f"- Aggregate C17 TP-to-C43 FN / FN-to-C43 TP: `{int(positive['c17_tp_to_c43_fn'].sum())}`/`{int(positive['c17_fn_to_c43_tp'].sum())}`.",
        f"- C27-to-C43 repaired/introduced pairs: `{int(inversions['C27_to_C43_repaired'].sum())}`/`{int(inversions['C27_to_C43_introduced'].sum())}`.",
    ...
    (report_dir / "phase_c43_dema_final_report.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
    return decision
...
def main() -> None:
    ...
    if str(config.get("phase", "")).lower() != "c43":
        raise RuntimeError("C43 report requires the formal C43 config")
    ...
    if args.stage == "validation":
        ...
        print(json.dumps({"status": "C43_VALIDATION_DECISION_FROZEN", "decision": decision["decision_label"]}))
    else:
        decision = write_final_report(config, run_dir, report_dir)
        print(json.dumps({"status": "C43_FINAL_REPORT_COMPLETE", "decision": decision["decision_label"]}))

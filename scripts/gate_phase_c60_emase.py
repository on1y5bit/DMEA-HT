#!/usr/bin/env python3
"""Authorize C60-EMASE direct formal execution with a strict contract gate."""
...
from dmea_ht.c60_emase import C60EMASEModel, HEAD_PREFIXES, trainable_parameter_count  # noqa: E402
...
def main() -> None:
    ...
    source_text = inspect.getsource(C60EMASEModel)
    ...
    scope_pass &= bool(trainable_counts) and max(trainable_counts.values()) <= int(config["c60"]["trainable_parameter_limit"])
    train_source = (REPO_ROOT / "scripts" / "train_phase_c60.py").read_text(encoding="utf-8")
    ...
    ) and bool(config["c60"]["shortcut_fields_used_as_inputs"] is False)
    route_pass = (
        all(term in c59_text for term in ("C59PMESEModel", "instance_encoder", "_fixed_patient_set_statistics", "set_max", "patient_mean"))
        and all(term in inherited_text for term in ("latest_mask", "history_mask", "variance", "log2", "raw_image", "aligned_image"))
        and list(config["c60"]["stream_order"]) == ["raw_image", "raw_text", "raw_bio", "aligned_image", "aligned_text", "aligned_bio"]
        and int(config["c60"]["instance_dim"]) == 128
        and config["c60"]["ema_enabled"] is True
        and abs(float(config["c60"]["ema_decay"]) - 0.99) <= 1e-12
        ...
        and config["c60"]["learned_visit_score"] is False
        and config["c60"]["temporal_attention"] is False
        and config["c60"]["router"] is False
        and config["c60"]["visit_selector"] is False
    )
    ema_contract = (
        all(term in train_source for term in ("EMAOptimizer", "ema_state", "update_ema", "ema_validation", "ema_checkpoint"))
        and "C60 seed" in train_source
    )
    test_blocked = (
        "C60 Validation decision must be frozen before reporting-only Test" in train_source
        and 'set(metrics["split"]) != {"val"}' in train_source
        and "c60_validation_decision.json" in train_source
    )
    direct_contract = (
        "subprocess.Popen" in train_source
        and "validation-seed" in train_source
        and 'f"seed_{seed}_best.pt"' in train_source
        and "ema_checkpoint" in train_source
        and len(SEEDS) == 3
    ...
    checks = [
        ...
        ("06_ema_model_gradients_and_frozen_source", gradient_pass),
        ("07_shortcut_fields_excluded", shortcut_pass),
        ("08_patient_level_evidence_route_and_ema_contract", route_pass and ema_contract),
    ...
    pd.DataFrame(gradient_rows).to_csv(report_dir / "c60_gradient_audit.csv", index=False)
    passed = sum(bool(value) for _, value in checks)
    status = "C60_EMASE_DIRECT_MULTI_SEED_AUTHORIZED" if passed == len(checks) else "C60_PATH_GATE_FAIL"
    payload = {
        "phase": "C60-EMASE",
        "status": status,
        "passed": passed,
    ...
    (report_dir / "c60_gate.json").write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"status": status, "passed": passed, "total": len(checks)}))
    if status != "C60_EMASE_DIRECT_MULTI_SEED_AUTHORIZED":
        raise SystemExit(2)

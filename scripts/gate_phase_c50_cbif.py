#!/usr/bin/env python3
"""Authorize C50-CBIF direct formal execution with a strict contract gate."""
...
from dmea_ht.c50_cbif import C50CBIFModel, HEAD_PREFIXES, trainable_parameter_count  # noqa: E402
...
def main() -> None:
    ...
    source_text = inspect.getsource(C50CBIFModel)
    ...
    scope_pass &= bool(trainable_counts) and max(trainable_counts.values()) <= int(config["c50"]["trainable_parameter_limit"])
    train_source = (REPO_ROOT / "scripts" / "train_phase_c50.py").read_text(encoding="utf-8")
    base_train_source = (REPO_ROOT / "scripts" / "train_phase_c40.py").read_text(encoding="utf-8")
    collector_source = (REPO_ROOT / "scripts" / "collect_phase_c50_report.py").read_text(encoding="utf-8")
    ...
    ) and bool(config["c50"]["shortcut_fields_used_as_inputs"] is False)
    route_pass = (
        all(
            term in source_text
            for term in (
        ...
        and config["c50"]["learned_visit_score"] is False
        and config["c50"]["temporal_attention"] is False
        and config["c50"]["router"] is False
        and config["c50"]["continuous_bio_conditioning"] is True
    )
    test_blocked = (
        "C50 Validation decision must be frozen before reporting-only Test" in train_source
        and 'set(metrics["split"]) != {"val"}' in train_source
        and "c50_validation_decision.json" in train_source
    )
    direct_contract = (
    ...
    checks = [
        ...
        ("06_conditioning_gradients_and_frozen_source", gradient_pass),
        ("07_shortcut_fields_excluded", shortcut_pass),
        ("08_patient_conditioned_cross_modal_contract", route_pass),
    ...
    pd.DataFrame(gradient_rows).to_csv(report_dir / "c50_gradient_audit.csv", index=False)
    passed = sum(bool(value) for _, value in checks)
    status = "C50_CBIF_DIRECT_MULTI_SEED_AUTHORIZED" if passed == len(checks) else "C50_PATH_GATE_FAIL"
    payload = {
        "phase": "C50-CBIF",
        "status": status,
        "passed": passed,
    ...
    (report_dir / "c50_gate.json").write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"status": status, "passed": passed, "total": len(checks)}))
    if status != "C50_CBIF_DIRECT_MULTI_SEED_AUTHORIZED":
        raise SystemExit(2)

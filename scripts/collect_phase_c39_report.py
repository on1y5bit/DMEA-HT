#!/usr/bin/env python3
"""Freeze and report C39-CMEQ Validation and reporting-only Test evidence."""
...
from scripts.train_phase_c39 import SEEDS  # noqa: E402
...
def ensure_audit_shortcut_columns(
    ...
    if any(field not in result.columns or result[field].isna().all() for field in missing):
        raise RuntimeError(f"C39 audit-only shortcut columns unavailable: {missing}")
    return result
...
def aligned_validation(
    ...
    c39 = read_prediction(run_dir / "predictions" / f"val_predictions_seed_{seed}.csv")
    baselines = {
        "C17": read_prediction(
    ...
    ids = c39["patient_id"].to_numpy(dtype=str)
    labels = c39["label"].to_numpy(dtype=int)
    if len(c39) != 94 or int((labels == 1).sum()) != 47:
        raise RuntimeError(f"C39 validation balance failed for seed {seed}")
    for name, frame in baselines.items():
        if not np.array_equal(ids, frame["patient_id"].to_numpy(dtype=str)):
            raise RuntimeError(f"C39 {name} patient alignment failed for seed {seed}")
        if not np.array_equal(labels, frame["label"].to_numpy(dtype=int)):
            raise RuntimeError(f"C39 {name} label alignment failed for seed {seed}")
    return c39, baselines, labels
...
def validation_comparisons(
    ...
    for seed in SEEDS:
        c39, baselines, labels = aligned_validation(config, run_dir, seed)
        probabilities = {
            "C39": c39[probability_column(c39)].to_numpy(dtype=float),
            **{
        ...
        if len(metric) != 1:
            raise RuntimeError(f"C39 validation metric row missing for seed {seed}")
        ...
        metric_rows.append(
            {
                ...
                **{
                    f"C39_minus_{name}_AUC": float(row["AUC"]) - auc(labels, value)
        ...
        c39_class = probabilities["C39"] >= 0.5
        c17_counts = binary_counts(labels, probabilities["C17"])
        c39_counts = binary_counts(labels, probabilities["C39"])
        positive = labels == 1
        positive_rows.append(
            {
                "seed": seed,
                "c17_tp_to_c39_fn": int((positive & c17_class & ~c39_class).sum()),
                "c17_fn_to_c39_tp": int((positive & ~c17_class & c39_class).sum()),
                "c17_sensitivity": c17_counts["Sensitivity"],
                "c39_sensitivity": c39_counts["Sensitivity"],
                "c39_minus_c17_sensitivity": c39_counts["Sensitivity"] - c17_counts["Sensitivity"],
            }
        )
        c27_inversions = inversion_vector(labels, probabilities["C27"])
        c39_inversions = inversion_vector(labels, probabilities["C39"])
        inversion_rows.append(
            {
                ...
                "C39_inversions": int(c39_inversions.sum()),
                "C39_minus_C27_inversions": int(c39_inversions.sum() - c27_inversions.sum()),
                "C39_inversion_ratio_vs_C27": float(
                    (c39_inversions.sum() - c27_inversions.sum()) / max(c27_inversions.sum(), 1)
                ),
                "C27_to_C39_repaired": int((c27_inversions & ~c39_inversions).sum()),
                "C27_to_C39_introduced": int((~c27_inversions & c39_inversions).sum()),
            }
        )
...
def training_health(run_dir: Path, epoch: pd.DataFrame) -> Tuple[pd.DataFrame, bool]:
    ...
    for seed in SEEDS:
        ...
        head_ok = bool(
            np.isfinite(head_gradient)
            and head_gradient > 0.0
            and len(drift_values) > 0
...
def freeze_validation_decision(
    ...
    if set(metrics["split"]) != {"val"}:
        raise RuntimeError("C39 validation decision requires validation-only metrics")
    ...
    positive_pass = bool(
        float(positive["c39_minus_c17_sensitivity"].min()) >= -0.10
        and int(positive["c17_tp_to_c39_fn"].sum())
        <= int(positive["c17_fn_to_c39_tp"].sum()) + 3
    )
    mean_c27_inversions = float(inversions["C27_inversions"].mean())
    mean_c39_inversions = float(inversions["C39_inversions"].mean())
    ranking_pass = bool(
        (mean_c39_inversions - mean_c27_inversions) / max(mean_c27_inversions, 1.0) <= 0.10
        and int(inversions["C39_minus_C27_inversions"].max()) <= 20
    )
    ...
    for path in sorted((run_dir / "seed_runs").glob("seed_*/reports/run_config.json")):
        ...
        capacity_pass &= int(runtime["trainable_parameter_count"]) <= int(config["c39"]["trainable_parameter_limit"])

    if not capacity_pass or not health_pass:
        label = "C39_TRAINING_INVALID"
    elif not shortcut_pass:
        label = "C39_SHORTCUT_CONCERN"
    elif not positive_pass:
        label = "C39_POSITIVE_DAMAGE"
    elif not ranking_pass:
        label = "C39_RANKING_DAMAGE"
    elif not auc_pass:
        label = "C39_NO_AUC_GAIN"
    else:
    ...
    decision = {
        ...
        "strict_best": "C39_CMEQ" if promoted else "KEEP_DEMA_C17_STRICT_BEST",
    ...
    comparisons.to_csv(report_dir / "c39_metrics_by_seed.csv", index=False)
    pd.DataFrame(
        [
            {
                "split": "val",
                ...
                "C39_minus_C27_AUC_mean": float(comparisons["C39_minus_C27_AUC"].mean()),
            }
        ]
    ).to_csv(report_dir / "c39_metrics_summary.csv", index=False)
    epoch.to_csv(report_dir / "c39_metrics_by_epoch.csv", index=False)
    pd.read_csv(run_dir / "reports" / "parameter_drift.csv").to_csv(report_dir / "c39_parameter_drift.csv", index=False)
    pd.read_csv(run_dir / "reports" / "patient_diagnostics_val.csv").to_csv(report_dir / "c39_patient_diagnostics_val.csv", index=False)
    health.to_csv(report_dir / "c39_training_health.csv", index=False)
    positive.to_csv(report_dir / "c39_positive_preservation.csv", index=False)
    inversions.to_csv(report_dir / "c39_pairwise_inversion_summary.csv", index=False)
    shortcuts.to_csv(report_dir / "c39_shortcut_audit.csv", index=False)
    (report_dir / "c39_validation_decision.json").write_text(json.dumps(decision, indent=2) + "\n", encoding="utf-8")
    (report_dir / "c39_route_decision.md").write_text(
        "\n".join(
            [
                "# C39-CMEQ Validation Decision",
                "",
                f"- Decision: `{label}`.",
...
def write_final_report(config: Mapping[str, Any], run_dir: Path, report_dir: Path) -> Dict[str, Any]:
    decision = json.loads((report_dir / "c39_validation_decision.json").read_text(encoding="utf-8"))
    metrics = pd.read_csv(run_dir / "reports" / "metrics_by_seed.csv")
    if set(metrics["split"]) != {"val", "test"}:
        raise RuntimeError("C39 final report requires validation and reporting-only rows")
    test = metrics[metrics["split"] == "test"]
    summary = pd.read_csv(report_dir / "c39_metrics_summary.csv")
    summary = pd.concat(
        [
            summary,
            pd.DataFrame(
    ...
    summary.to_csv(report_dir / "c39_metrics_summary.csv", index=False)
    positive = pd.read_csv(report_dir / "c39_positive_preservation.csv")
    inversions = pd.read_csv(report_dir / "c39_pairwise_inversion_summary.csv")
    health = pd.read_csv(report_dir / "c39_training_health.csv")
    shortcut = pd.read_csv(report_dir / "c39_shortcut_audit.csv")
    lines = [
        "# DMEA-HT Phase C39-CMEQ Final Report",
        "",
        ...
        f"- Aggregate C17 TP-to-C39 FN / FN-to-C39 TP: `{int(positive['c17_tp_to_c39_fn'].sum())}`/`{int(positive['c17_fn_to_c39_tp'].sum())}`.",
        f"- C27-to-C39 repaired/introduced pairs: `{int(inversions['C27_to_C39_repaired'].sum())}`/`{int(inversions['C27_to_C39_introduced'].sum())}`.",
    ...
    (report_dir / "phase_c39_dema_final_report.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
    return decision
...
def main() -> None:
    ...
    if str(config.get("phase", "")).lower() != "c39":
        raise RuntimeError("C39 report requires the formal C39 config")
    ...
    if args.stage == "validation":
        ...
        print(json.dumps({"status": "C39_VALIDATION_DECISION_FROZEN", "decision": decision["decision_label"]}))
    else:
        decision = write_final_report(config, run_dir, report_dir)
        print(json.dumps({"status": "C39_FINAL_REPORT_COMPLETE", "decision": decision["decision_label"]}))

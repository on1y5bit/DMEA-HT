#!/usr/bin/env python3
"""Authorize C52-ECR direct formal execution with a strict contract gate."""
...
from dmea_ht.c52_ecr import C52ECRModel, HEAD_PREFIXES, trainable_parameter_count  # noqa: E402
...
def main() -> None:
    ...
    source_text = inspect.getsource(C52ECRModel)
    ...
    scope_pass &= bool(trainable_counts) and max(trainable_counts.values()) <= int(config["c52"]["trainable_parameter_limit"])
    train_source = (REPO_ROOT / "scripts" / "train_phase_c52.py").read_text(encoding="utf-8")
    base_train_source = (REPO_ROOT / "scripts" / "train_phase_c40.py").read_text(encoding="utf-8")
    collector_source = (REPO_ROOT / "scripts" / "collect_phase_c52_report.py").read_text(encoding="utf-8")
    ...
    ) and bool(config["c52"]["shortcut_fields_used_as_inputs"] is False)
    route_pass = (
        all(
            term in source_text
            for term in (
        ...
        and list(config["c52"]["stream_order"]) == ["raw_image", "raw_text", "raw_bio", "aligned_image", "aligned_text", "aligned_bio"]
        and "visit_score" not in route_text
        and "temporal" not in route_text.lower()
        and "router" not in route_text.lower()
        and all(term not in route_text for term in ("C27", "C37", "C38", "C39", "C40", "C42", "C43", "C44"))
        and config["c52"]["learned_visit_score"] is False
        and config["c52"]["temporal_attention"] is False
        and config["c52"]["router"] is False
    )
    test_blocked = (
        "C52 Validation decision must be frozen before reporting-only Test" in train_source
        and 'set(metrics["split"]) != {"val"}' in train_source
        and "c52_validation_decision.json" in train_source
    )
    direct_contract = (
    ...
    checks = [
        ...
        ("06_conservation_gradients_and_frozen_source", gradient_pass),
        ("07_shortcut_fields_excluded", shortcut_pass),
        ("08_patient_evidence_conservation_contract", route_pass),
    ...
    pd.DataFrame(gradient_rows).to_csv(report_dir / "c52_gradient_audit.csv", index=False)
    passed = sum(bool(value) for _, value in checks)
    status = "C52_ECR_DIRECT_MULTI_SEED_AUTHORIZED" if passed == len(checks) else "C52_PATH_GATE_FAIL"
    payload = {
        "phase": "C52-ECR",
        "status": status,
        "passed": passed,
    ...
    (report_dir / "c52_gate.json").write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"status": status, "passed": passed, "total": len(checks)}))
    if status != "C52_ECR_DIRECT_MULTI_SEED_AUTHORIZED":
        raise SystemExit(2)

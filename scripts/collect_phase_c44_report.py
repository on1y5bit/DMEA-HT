#!/usr/bin/env python3
"""Freeze and report C44-MISE Validation and reporting-only Test evidence."""
...
def freeze_validation_decision(config: Mapping[str, Any], run_dir: Path, report_dir: Path) -> Dict[str, Any]:
    ...
    if set(metrics["split"]) != {"val"}:
        raise RuntimeError("C44 Validation decision requires Validation-only metrics")
    comparisons, positive, inversions = base.validation_comparisons(config, run_dir, metrics)
    comparisons = rename_c41_columns(comparisons, "C41", "C44")
    positive = rename_c41_columns(positive, "c41", "c44")
    inversions = rename_c41_columns(inversions, "C41", "C44")
    ...
    shortcuts["combination"] = "C44-MISE"
    ...
    positive_pass = bool(
        float(positive["c44_minus_c17_sensitivity"].min()) >= -0.10
        and int(positive["c17_tp_to_c44_fn"].sum()) <= int(positive["c17_fn_to_c44_tp"].sum()) + 3
    )
    mean_c27_inversions = float(inversions["C27_inversions"].mean())
    mean_c44_inversions = float(inversions["C44_inversions"].mean())
    ranking_pass = bool(
        (mean_c44_inversions - mean_c27_inversions) / max(mean_c27_inversions, 1.0) <= 0.10
        and int(inversions["C44_minus_C27_inversions"].max()) <= 20
    )
    ...
    for path in sorted((run_dir / "seed_runs").glob("seed_*/reports/run_config.json")):
        ...
        capacity_pass &= int(runtime["trainable_parameter_count"]) <= int(config["c44"]["trainable_parameter_limit"])
    if not capacity_pass or not health_pass:
        label = "C44_TRAINING_INVALID"
    elif not shortcut_pass:
        label = "C44_SHORTCUT_CONCERN"
    elif not positive_pass:
        label = "C44_POSITIVE_DAMAGE"
    elif not ranking_pass:
        label = "C44_RANKING_DAMAGE"
    elif not auc_pass:
        label = "C44_NO_AUC_GAIN"
    else:
    ...
    decision = {
        ...
        "strict_best": "C44_MISE" if promoted else "KEEP_DEMA_C17_STRICT_BEST",
    ...
    comparisons.to_csv(report_dir / "c44_metrics_by_seed.csv", index=False)
    pd.DataFrame([{"split": "val", "AUC_mean": mean_auc, "AUC_std": std_auc, "C17_AUC_mean": float(comparisons["C17_AUC"].mean()), "C27_AUC_mean": float(comparisons["C27_AUC"].mean()), "C44_minus_C27_AUC_mean": float(comparisons["C44_minus_C27_AUC"].mean())}]).to_csv(report_dir / "c44_metrics_summary.csv", index=False)
    epoch.to_csv(report_dir / "c44_metrics_by_epoch.csv", index=False)
    pd.read_csv(run_dir / "reports" / "parameter_drift.csv").to_csv(report_dir / "c44_parameter_drift.csv", index=False)
    pd.read_csv(run_dir / "reports" / "patient_diagnostics_val.csv").to_csv(report_dir / "c44_patient_diagnostics_val.csv", index=False)
    health.to_csv(report_dir / "c44_training_health.csv", index=False)
    positive.to_csv(report_dir / "c44_positive_preservation.csv", index=False)
    inversions.to_csv(report_dir / "c44_pairwise_inversion_summary.csv", index=False)
    shortcuts.to_csv(report_dir / "c44_shortcut_audit.csv", index=False)
    (report_dir / "c44_validation_decision.json").write_text(json.dumps(decision, indent=2) + "\n", encoding="utf-8")
    (report_dir / "c44_route_decision.md").write_text(
        "\n".join(
            [
                "# C44-MISE Validation Decision",
                "",
                f"- Decision: `{label}`.",
...
def write_final_report(config: Mapping[str, Any], run_dir: Path, report_dir: Path) -> Dict[str, Any]:
    decision = json.loads((report_dir / "c44_validation_decision.json").read_text(encoding="utf-8"))
    metrics = pd.read_csv(run_dir / "reports" / "metrics_by_seed.csv")
    if set(metrics["split"]) != {"val", "test"}:
        raise RuntimeError("C44 final report requires Validation and reporting-only rows")
    test = metrics[metrics["split"] == "test"]
    summary = pd.read_csv(report_dir / "c44_metrics_summary.csv")
    summary = pd.concat([summary, pd.DataFrame([{"split": "test", "AUC_mean": float(test["AUC"].mean()), "AUC_std": float(test["AUC"].std(ddof=1)), "Sensitivity_mean": float(test["Sensitivity"].mean()), "Specificity_mean": float(test["Specificity"].mean()), "Balanced_ACC_mean": float(test["Balanced_ACC"].mean())}])], ignore_index=True, sort=False)
    summary.to_csv(report_dir / "c44_metrics_summary.csv", index=False)
    positive = pd.read_csv(report_dir / "c44_positive_preservation.csv")
    inversions = pd.read_csv(report_dir / "c44_pairwise_inversion_summary.csv")
    health = pd.read_csv(report_dir / "c44_training_health.csv")
    shortcut = pd.read_csv(report_dir / "c44_shortcut_audit.csv")
    lines = [
        "# DMEA-HT Phase C44-MISE Final Report",
        "",
        ...
        f"- Aggregate C17 TP-to-C44 FN / FN-to-C44 TP: `{int(positive['c17_tp_to_c44_fn'].sum())}`/`{int(positive['c17_fn_to_c44_tp'].sum())}`.",
        f"- C27-to-C44 repaired/introduced pairs: `{int(inversions['C27_to_C44_repaired'].sum())}`/`{int(inversions['C27_to_C44_introduced'].sum())}`.",
    ...
    (report_dir / "phase_c44_dema_final_report.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
    return decision
...
def main() -> None:
    ...
    if str(config.get("phase", "")).lower() != "c44":
        raise RuntimeError("C44 report requires the formal C44 config")
    ...
    if args.stage == "validation":
        ...
        print(json.dumps({"status": "C44_VALIDATION_DECISION_FROZEN", "decision": decision["decision_label"]}))
    else:
        decision = write_final_report(config, run_dir, report_dir)
        print(json.dumps({"status": "C44_FINAL_REPORT_COMPLETE", "decision": decision["decision_label"]}))

#!/usr/bin/env python3
"""Authorize C49-CREB direct formal execution with a strict contract gate."""
...
from dmea_ht.c49_creb import C49CREBModel, HEAD_PREFIXES, trainable_parameter_count  # noqa: E402
...
def main() -> None:
    ...
    source_text = inspect.getsource(C49CREBModel)
    ...
    scope_pass &= bool(trainable_counts) and max(trainable_counts.values()) <= int(config["c49"]["trainable_parameter_limit"])
    train_source = (REPO_ROOT / "scripts" / "train_phase_c49.py").read_text(encoding="utf-8")
    base_train_source = (REPO_ROOT / "scripts" / "train_phase_c40.py").read_text(encoding="utf-8")
    collector_source = (REPO_ROOT / "scripts" / "collect_phase_c49_report.py").read_text(encoding="utf-8")
    ...
    ) and bool(config["c49"]["shortcut_fields_used_as_inputs"] is False)
    route_pass = (
        all(term in source_text for term in ("binding_encoder", "bound_stream_encoder", "raw", "aligned", "bound_input", "discordance"))
        and all(term in inherited_text for term in ("latest_mask", "history_mask", "variance", "log2", "raw_image", "aligned_image"))
        and list(config["c49"]["stream_order"]) == ["image", "text", "bio"]
        ...
        and config["c49"]["learned_visit_score"] is False
        and config["c49"]["temporal_attention"] is False
        and config["c49"]["router"] is False
    )
    test_blocked = (
        "C49 Validation decision must be frozen before reporting-only Test" in train_source
        and 'set(metrics["split"]) != {"val"}' in train_source
        and "c49_validation_decision.json" in train_source
    )
    direct_contract = (
    ...
    checks = [
        ...
        ("06_binding_gradients_and_frozen_source", gradient_pass),
        ("07_shortcut_fields_excluded", shortcut_pass),
        ("08_cross_resolution_binding_contract", route_pass),
    ...
    pd.DataFrame(gradient_rows).to_csv(report_dir / "c49_gradient_audit.csv", index=False)
    passed = sum(bool(value) for _, value in checks)
    status = "C49_CREB_DIRECT_MULTI_SEED_AUTHORIZED" if passed == len(checks) else "C49_PATH_GATE_FAIL"
    payload = {
        "phase": "C49-CREB",
        "status": status,
        "passed": passed,
    ...
    (report_dir / "c49_gate.json").write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"status": status, "passed": passed, "total": len(checks)}))
    if status != "C49_CREB_DIRECT_MULTI_SEED_AUTHORIZED":
        raise SystemExit(2)

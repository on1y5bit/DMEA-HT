#!/usr/bin/env python3
"""Freeze and report C41-MELR Validation and reporting-only Test evidence."""
...
from scripts.train_phase_c41 import SEEDS  # noqa: E402


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--config", default="configs/dema_ht_c41_melr_multiseed.yaml")
...
def ensure_audit_shortcut_columns(frame: pd.DataFrame, config: Mapping[str, Any]) -> pd.DataFrame:
    ...
    for row in read_jsonl(config["project"]["manifest"]):
        ...
        derived[patient_id] = {
            ...
            "dated_bio_visit_count": int(sum(visit.get("dated_bio_row_id") is not None for visit in visits)),
        }
    result = frame.copy()
    ...
    if any(field not in result.columns or result[field].isna().all() for field in missing):
        raise RuntimeError(f"C41 audit-only shortcut columns unavailable: {missing}")
    return result
...
def aligned_validation(config: Mapping[str, Any], run_dir: Path, seed: int) -> Tuple[pd.DataFrame, Dict[str, pd.DataFrame], np.ndarray]:
    c41 = read_prediction(run_dir / "predictions" / f"val_predictions_seed_{seed}.csv")
    baselines = {
        "C17": read_prediction(Path(config["c17"]["c17_run_dir"]) / "predictions" / f"val_predictions_seed_{seed}.csv"),
        "C27": read_prediction(Path(config["c27"]["c27_run_dir"]) / "predictions" / f"val_predictions_seed_{seed}.csv"),
    }
    ids = c41["patient_id"].to_numpy(dtype=str)
    labels = c41["label"].to_numpy(dtype=int)
    if len(c41) != 94 or int((labels == 1).sum()) != 47:
        raise RuntimeError(f"C41 Validation balance failed for seed {seed}")
    for name, frame in baselines.items():
        if not np.array_equal(ids, frame["patient_id"].to_numpy(dtype=str)):
            raise RuntimeError(f"C41 {name} patient alignment failed for seed {seed}")
        if not np.array_equal(labels, frame["label"].to_numpy(dtype=int)):
            raise RuntimeError(f"C41 {name} label alignment failed for seed {seed}")
    return c41, baselines, labels


def validation_comparisons(config: Mapping[str, Any], run_dir: Path, metrics: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    metric_rows: List[Dict[str, Any]] = []
    positive_rows: List[Dict[str, Any]] = []
    inversion_rows: List[Dict[str, Any]] = []
    for seed in SEEDS:
        c41, baselines, labels = aligned_validation(config, run_dir, seed)
        probabilities = {
            "C41": c41[probability_column(c41)].to_numpy(dtype=float),
            **{name: frame[probability_column(frame)].to_numpy(dtype=float) for name, frame in baselines.items()},
        }
        metric = metrics[(metrics["seed"].astype(int) == seed) & (metrics["split"] == "val")]
        if len(metric) != 1:
            raise RuntimeError(f"C41 Validation metric row missing for seed {seed}")
        row = metric.iloc[0].to_dict()
        metric_rows.append(
            {
                **row,
                **{f"{name}_AUC": auc(labels, value) for name, value in probabilities.items()},
                **{f"C41_minus_{name}_AUC": float(row["AUC"]) - auc(labels, value) for name, value in probabilities.items() if name != "C41"},
            }
        )
        c17_class = probabilities["C17"] >= 0.5
        c41_class = probabilities["C41"] >= 0.5
        c17_counts = binary_counts(labels, probabilities["C17"])
        c41_counts = binary_counts(labels, probabilities["C41"])
        positive = labels == 1
        positive_rows.append(
            {
                "seed": seed,
                "c17_tp_to_c41_fn": int((positive & c17_class & ~c41_class).sum()),
                "c17_fn_to_c41_tp": int((positive & ~c17_class & c41_class).sum()),
                "c17_sensitivity": c17_counts["Sensitivity"],
                "c41_sensitivity": c41_counts["Sensitivity"],
                "c41_minus_c17_sensitivity": c41_counts["Sensitivity"] - c17_counts["Sensitivity"],
            }
        )
        c27_inversions = inversion_vector(labels, probabilities["C27"])
        c41_inversions = inversion_vector(labels, probabilities["C41"])
        inversion_rows.append(
            {
                ...
                "C41_inversions": int(c41_inversions.sum()),
                "C41_minus_C27_inversions": int(c41_inversions.sum() - c27_inversions.sum()),
                "C41_inversion_ratio_vs_C27": float((c41_inversions.sum() - c27_inversions.sum()) / max(c27_inversions.sum(), 1)),
                "C27_to_C41_repaired": int((c27_inversions & ~c41_inversions).sum()),
                "C27_to_C41_introduced": int((~c27_inversions & c41_inversions).sum()),
            }
        )
...
def training_health(run_dir: Path, epoch: pd.DataFrame) -> Tuple[pd.DataFrame, bool]:
    ...
    diagnostics = pd.read_csv(run_dir / "reports" / "patient_diagnostics_val.csv", dtype={"patient_id": str})
    ...
    for seed in SEEDS:
        ...
        state_std = float(diag["patient_state_component_std"].to_numpy(dtype=float).std()) if len(diag) else 0.0
        prediction_ok = len(diag) == 94 and np.isfinite(probabilities).all() and float(probabilities.std()) > 0.0 and float(probabilities.max() - probabilities.min()) > 1e-4
        state_ok = len(diag) == 94 and np.isfinite(diag["patient_state_norm"].to_numpy(dtype=float)).all() and float(diag["patient_state_norm"].to_numpy(dtype=float).std()) > 0.0
        ...
        rows.extend(
            [
                ...
                {"seed": seed, "category": "patient_state_health", "state_std": state_std, "health_pass": state_ok},
...
def shortcut_audit(config: Mapping[str, Any], run_dir: Path) -> pd.DataFrame:
    rows: List[Dict[str, Any]] = []
    for seed in SEEDS:
        frame = ensure_audit_shortcut_columns(read_prediction(run_dir / "predictions" / f"val_predictions_seed_{seed}.csv"), config)
        probability = frame[probability_column(frame)].to_numpy(dtype=float)
        correlations = {field: safe_spearman(probability, pd.to_numeric(frame[field], errors="coerce").to_numpy(dtype=float)) for field in SELECTED_SHORTCUT_FIELDS}
        ...
        rows.append(
            {
                "seed": seed,
                "combination": "C41-MELR",
                "selected_structure_shortcut_only_label_AUC": shortcut_only_auc(frame),
                "max_abs_prediction_selected_structure_spearman": maximum,
                "shortcut_safety_pass": shortcut_only_auc(frame) <= 0.55 and maximum <= 0.35,
...
def freeze_validation_decision(config: Mapping[str, Any], run_dir: Path, report_dir: Path) -> Dict[str, Any]:
    ...
    if set(metrics["split"]) != {"val"}:
        raise RuntimeError("C41 Validation decision requires Validation-only metrics")
    ...
    positive_pass = bool(float(positive["c41_minus_c17_sensitivity"].min()) >= -0.10 and int(positive["c17_tp_to_c41_fn"].sum()) <= int(positive["c17_fn_to_c41_tp"].sum()) + 3)
    mean_c27_inversions = float(inversions["C27_inversions"].mean())
    mean_c41_inversions = float(inversions["C41_inversions"].mean())
    ranking_pass = bool((mean_c41_inversions - mean_c27_inversions) / max(mean_c27_inversions, 1.0) <= 0.10 and int(inversions["C41_minus_C27_inversions"].max()) <= 20)
    ...
    for path in sorted((run_dir / "seed_runs").glob("seed_*/reports/run_config.json")):
        ...
        capacity_pass &= int(runtime["trainable_parameter_count"]) <= int(config["c41"]["trainable_parameter_limit"])
    if not capacity_pass or not health_pass:
        label = "C41_TRAINING_INVALID"
    elif not shortcut_pass:
        label = "C41_SHORTCUT_CONCERN"
    elif not positive_pass:
        label = "C41_POSITIVE_DAMAGE"
    elif not ranking_pass:
        label = "C41_RANKING_DAMAGE"
    elif not auc_pass:
        label = "C41_NO_AUC_GAIN"
    else:
    ...
    decision = {
        ...
        "strict_best": "C41_MELR" if promoted else "KEEP_DEMA_C17_STRICT_BEST",
    ...
    comparisons.to_csv(report_dir / "c41_metrics_by_seed.csv", index=False)
    pd.DataFrame([{"split": "val", "AUC_mean": mean_auc, "AUC_std": std_auc, "C17_AUC_mean": float(comparisons["C17_AUC"].mean()), "C27_AUC_mean": float(comparisons["C27_AUC"].mean()), "C41_minus_C27_AUC_mean": float(comparisons["C41_minus_C27_AUC"].mean())}]).to_csv(report_dir / "c41_metrics_summary.csv", index=False)
    epoch.to_csv(report_dir / "c41_metrics_by_epoch.csv", index=False)
    pd.read_csv(run_dir / "reports" / "parameter_drift.csv").to_csv(report_dir / "c41_parameter_drift.csv", index=False)
    pd.read_csv(run_dir / "reports" / "patient_diagnostics_val.csv").to_csv(report_dir / "c41_patient_diagnostics_val.csv", index=False)
    health.to_csv(report_dir / "c41_training_health.csv", index=False)
    positive.to_csv(report_dir / "c41_positive_preservation.csv", index=False)
    inversions.to_csv(report_dir / "c41_pairwise_inversion_summary.csv", index=False)
    shortcuts.to_csv(report_dir / "c41_shortcut_audit.csv", index=False)
    (report_dir / "c41_validation_decision.json").write_text(json.dumps(decision, indent=2) + "\n", encoding="utf-8")
    (report_dir / "c41_route_decision.md").write_text(
        "\n".join(
            [
                "# C41-MELR Validation Decision",
                "",
                f"- Decision: `{label}`.",
...
def write_final_report(config: Mapping[str, Any], run_dir: Path, report_dir: Path) -> Dict[str, Any]:
    decision = json.loads((report_dir / "c41_validation_decision.json").read_text(encoding="utf-8"))
    metrics = pd.read_csv(run_dir / "reports" / "metrics_by_seed.csv")
    if set(metrics["split"]) != {"val", "test"}:
        raise RuntimeError("C41 final report requires Validation and reporting-only rows")
    test = metrics[metrics["split"] == "test"]
    summary = pd.read_csv(report_dir / "c41_metrics_summary.csv")
    summary = pd.concat(
        [summary, pd.DataFrame([{"split": "test", "AUC_mean": float(test["AUC"].mean()), "AUC_std": float(test["AUC"].std(ddof=1)), "Sensitivity_mean": float(test["Sensitivity"].mean()), "Specificity_mean": float(test["Specificity"].mean()), "Balanced_ACC_mean": float(test["Balanced_ACC"].mean())}])],
        ignore_index=True,
        sort=False,
    )
    summary.to_csv(report_dir / "c41_metrics_summary.csv", index=False)
    positive = pd.read_csv(report_dir / "c41_positive_preservation.csv")
    inversions = pd.read_csv(report_dir / "c41_pairwise_inversion_summary.csv")
    health = pd.read_csv(report_dir / "c41_training_health.csv")
    shortcut = pd.read_csv(report_dir / "c41_shortcut_audit.csv")
    lines = [
        "# DMEA-HT Phase C41-MELR Final Report",
        "",
        ...
        f"- Aggregate C17 TP-to-C41 FN / FN-to-C41 TP: `{int(positive['c17_tp_to_c41_fn'].sum())}`/`{int(positive['c17_fn_to_c41_tp'].sum())}`.",
        f"- C27-to-C41 repaired/introduced pairs: `{int(inversions['C27_to_C41_repaired'].sum())}`/`{int(inversions['C27_to_C41_introduced'].sum())}`.",
    ...
    (report_dir / "phase_c41_dema_final_report.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
    return decision
...
def main() -> None:
    ...
    if str(config.get("phase", "")).lower() != "c41":
        raise RuntimeError("C41 report requires the formal C41 config")
    ...
    if args.stage == "validation":
        ...
        print(json.dumps({"status": "C41_VALIDATION_DECISION_FROZEN", "decision": decision["decision_label"]}))
    else:
        decision = write_final_report(config, run_dir, report_dir)
        print(json.dumps({"status": "C41_FINAL_REPORT_COMPLETE", "decision": decision["decision_label"]}))

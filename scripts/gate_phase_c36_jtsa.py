#!/usr/bin/env python3
"""Authorize direct C36-JTSA formal execution with exactly 20 checks."""
...
from scripts.train_phase_c36 import SEEDS, trainable_gradient_norms  # noqa: E402
...
def run_runtime_checks(
    ...
    runtime = {
        ...
        "embedding_shape": True,
        "patient_state_shape": True,
    ...
    for seed in SEEDS:
        set_seed(seed)
        checkpoint_path = Path(str(config["c36"]["c17_checkpoint"]).replace("{seed}", str(seed)))
        ...
        runtime["trainable_scope"] &= bool(trainable_names) and all(
            C36JTSAModel.is_trainable_parameter(name) for name in trainable_names
        )
        runtime["frozen_representation"] &= all(
            not parameter.requires_grad for parameter in model.frozen_sources.parameters()
        )
        projector_ids = [
            tuple(id(parameter) for parameter in model.mechanism_visit_projectors[name].parameters())
            for name in MECHANISM_NAMES
        ]
        embedder_ids = [
            tuple(id(parameter) for parameter in model.mechanism_trajectory_embedders[name].parameters())
            for name in MECHANISM_NAMES
        ]
        runtime["mechanism_mapping"] &= (
            tuple(MECHANISM_LABELS)
            == ("M1_morphology", "M2_immune", "M3_function", "M4_opposition", "M5_temporal_text")
        )
        ...
        runtime["mechanism_mapping"] &= len(set(embedder_ids)) == len(MECHANISM_NAMES)
        ...
        embeddings = first_output["mechanism_embedding"]
        patient_state = first_output["patient_disease_state"]
        ...
        runtime["embedding_shape"] &= (
            embeddings.ndim == 3
            and tuple(embeddings.shape[1:]) == (5, 32)
            and bool(torch.isfinite(embeddings).all())
        )
        runtime["patient_state_shape"] &= (
            patient_state.ndim == 2
            and tuple(patient_state.shape[1:]) == (32,)
            and bool(torch.isfinite(patient_state).all())
            and bool(torch.isfinite(first_output["logit"]).all())
        )
        ...
        runtime["fallback_finite"] &= bool(
            ...
            and torch.isfinite(fallback_output["mechanism_embedding"]).all()
            and torch.isfinite(fallback_output["patient_disease_state"]).all()
        ...
        runtime["anchor_contract"] &= bool(
            ...
            and torch.allclose(
                first_output["anchor_distance"], torch.tensor(2.0, device=device), atol=1e-5
            )
            and torch.allclose((anchor_non_ht + anchor_ht) / 2.0, anchor_center, atol=1e-6)
            and torch.allclose(
                torch.linalg.vector_norm(anchor_direction), torch.ones((), device=device), atol=1e-5
            )
        )

        train_batch = move_batch(next(iter(build_loaders(config, rows, ("train",))["train"])), device)
        ...
        for probe_index, probe_batch in enumerate(probe_batches):
            ...
            if not bool(torch.isfinite(loss)):
                ...
                probe_details.append({"probe": probe_index, "loss": float("nan")})
                continue
            loss.backward()
        ...
        details["fallback_gradient_norms_by_seed"][str(seed)] = {
            "mechanism_fallbacks": norms["mechanism_fallbacks"]
        }
        ...
        runtime["gradient_contract"] &= all(
            parameter.grad is None for parameter in model.frozen_sources.parameters()
        )
        runtime["gradient_contract"] &= all(
        ...
        if before != state_digest(model):
            raise RuntimeError("C36 gate changed model state without an optimizer step")
        del model
...
def main() -> None:
    ...
    train_source = (REPO_ROOT / "scripts" / "train_phase_c36.py").read_text(encoding="utf-8")
    collector_path = REPO_ROOT / "scripts" / "collect_phase_c36_report.py"
    collector_source = collector_path.read_text(encoding="utf-8")
    model_source = inspect.getsource(C36JTSAModel) + inspect.getsource(FrozenC17EvidenceRepresentation)
    ...
    no_audit_fields = not any(field in model_source for field in audit_fields)
    forbidden_modules = (
    ...
    no_forbidden_modules = not any(field in model_source.lower() for field in forbidden_modules)
    test_blocked = (
        not decision_path.exists()
        and "C36 validation decision must be frozen before reporting-only test" in train_source
        and "validation_decision_frozen_before_test" in train_source
    )
    checks = [
        ("01_canonical_main_clean", canonical and branch == "main" and clean),
        ("02_manifest_sha256_exact", file_sha256(resolve_path(config["project"]["manifest"])) == str(config["c36"]["manifest_sha256"])),
        ...
        ("06_only_c36_scope_trainable", runtime["trainable_scope"]),
        ...
        ("09_fixed_single_multi_trajectory_shape_bv596", runtime["trajectory_shape"]),
        ("10_mechanism_embedding_shape_b532", runtime["embedding_shape"]),
        ("11_joint_patient_state_shape_b32", runtime["patient_state_shape"]),
        ("12_symmetric_anchor_radius_one_distance_two", runtime["anchor_contract"]),
        ("13_missing_mechanism_fallback_finite", runtime["fallback_finite"]),
        ("14_real_activated_gradients_finite_nonzero", runtime["gradient_contract"]),
        ("15_trainable_parameter_count_within_limit", int(audit.loc[audit["trainable"].astype(bool), "parameter_count"].sum()) <= int(config["c36"]["trainable_parameter_limit"])),
        ("16_audit_fields_do_not_enter_model", no_audit_fields),
        ("17_bce_only_and_no_secondary_metric", bool(config["loss"]["bce_only"]) and train_source.count("binary_cross_entropy_with_logits") == 1 and "positive_preservation_loss" not in train_source and "pairwise_ranking_loss" not in train_source and "auxiliary_loss" not in train_source and disabled_metric not in train_source and disabled_metric not in collector_source and disabled_metric not in config_path.read_text(encoding="utf-8")),
        ("18_single_model_independent_direct_multiseed", config["deployment"] == {"one_checkpoint": True, "one_model": True, "one_forward": True, "ensemble": False} and "seed_runs" in train_source and "subprocess.Popen" in train_source and 'f"seed_{seed}_best.pt"' in train_source and len(SEEDS) == 3),
        ("19_test_blocked_before_validation_decision", test_blocked),
        ("20_forbidden_propagation_and_saved_prediction_modules_absent", no_forbidden_modules),
    ]
    if len(checks) != 20:
        raise RuntimeError(f"C36 gate must contain exactly 20 checks, found {len(checks)}")
    report_dir = resolve_path(config["project"]["report_dir"])
    report_dir.mkdir(parents=True, exist_ok=True)
    audit.to_csv(report_dir / "c36_trainable_parameter_audit.csv", index=False)
    checkpoints.to_csv(report_dir / "c36_c17_checkpoint_audit.csv", index=False)
    passed = sum(bool(value) for _, value in checks)
    status = "C36_JTSA_DIRECT_MULTI_SEED_AUTHORIZED" if passed == len(checks) else "DEMA_C36_PATH_GATE_FAIL"
    ...
    payload = {
        ...
        "mechanism_embedding_shape": ["B", 5, 32],
        "patient_state_shape": ["B", 32],
        "anchor_radius": float(config["c36"]["anchor_radius"]),
        "anchor_distance": 2.0,
    ...
    (report_dir / "c36_gate.json").write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"status": status, "passed": passed, "total": len(checks)}))
    if status != "C36_JTSA_DIRECT_MULTI_SEED_AUTHORIZED":
        raise SystemExit(2)

#!/usr/bin/env python3
"""Freeze the C32 validation decision and collect its reporting-only result."""
...
def aligned_validation(
    ...
    c32 = read_prediction(run_dir / "predictions" / f"val_predictions_seed_{seed}.csv")
    c27 = read_prediction(
    ...
    c17 = read_prediction(
        resolve_path(config["c32"]["c17_run_dir"])
        / "predictions"
        / f"val_predictions_seed_{seed}.csv"
    )
    ids = c32["patient_id"].to_numpy(dtype=str)
    labels = c32["label"].to_numpy(dtype=int)
    for name, frame in (("C27", c27), ("C17", c17)):
        if not np.array_equal(ids, frame["patient_id"].to_numpy(dtype=str)):
            raise RuntimeError(f"C32 {name} patient alignment failed for seed {seed}")
        if not np.array_equal(labels, frame["label"].to_numpy(dtype=int)):
            raise RuntimeError(f"C32 {name} label alignment failed for seed {seed}")
    if len(c32) != 94 or int((labels == 1).sum()) != 47:
        raise RuntimeError(f"C32 validation balance failed for seed {seed}")
    return c32, c27, c17
...
def validation_comparisons(
    ...
    for seed in SEEDS:
        c32, c27, c17 = aligned_validation(config, run_dir, seed)
        labels = c32["label"].to_numpy(dtype=int)
        ...
        if len(metric) != 1:
            raise RuntimeError(f"C32 validation metric row missing for seed {seed}")
        metric_row = metric.iloc[0].to_dict()
        c32_auc = float(metric_row["AUC"])
        ...
        metric_rows.append(
            {
                **metric_row,
                "C17_AUC": c17_auc,
                "C27_AUC": c27_auc,
                "C32_minus_C17_AUC": c32_auc - c17_auc,
                "C32_minus_C27_AUC": c32_auc - c27_auc,
                "minor_AUC_variation_vs_C27": abs(c32_auc - c27_auc) < 0.003,
            }
        )
        ...
        positive_rows.append(
            {
                "seed": seed,
                "c17_tp_to_c32_fn": int((positive & c17_class & ~c32_class).sum()),
                "c17_fn_to_c32_tp": int((positive & ~c17_class & c32_class).sum()),
                "c32_sensitivity": c32_counts["Sensitivity"],
                "c17_sensitivity": c17_counts["Sensitivity"],
                "c27_sensitivity": c27_counts["Sensitivity"],
                "c32_minus_c17_sensitivity": (
                    c32_counts["Sensitivity"] - c17_counts["Sensitivity"]
                ),
                "c27_material_positive_damage_count": int(c27_damage.sum()),
                "c32_material_positive_damage_count": int(c32_damage.sum()),
                "c32_minus_c27_material_damage": int(
                    c32_damage.sum() - c27_damage.sum()
                ),
            }
        )
        ...
        inversion_rows.append(
            {
                "seed": seed,
                "total_pairs": int(len(inv32)),
                "C17_inversions": int(inv17.sum()),
                "C27_inversions": int(inv27.sum()),
                "C32_inversions": int(inv32.sum()),
                "C32_minus_C27_inversions": int(inv32.sum() - inv27.sum()),
                "C27_to_C32_repaired": int((inv27 & ~inv32).sum()),
                "C27_to_C32_introduced": int((~inv27 & inv32).sum()),
                "minor_ranking_variation_vs_C27": abs(int(inv32.sum() - inv27.sum())) <= 3,
            }
        )
    return (
...
def projector_health(
    run_dir: Path,
) -> Tuple[pd.DataFrame, pd.DataFrame, bool]:
    drift = pd.read_csv(run_dir / "reports" / "projector_drift.csv")
    diagnostics = pd.read_csv(
        run_dir / "reports" / "patient_projector_diagnostics_val.csv",
    ...
    for seed in SEEDS:
        for modality in ("image", "text", "bio"):
            frame = drift[
                (drift["seed"].astype(int) == seed) & (drift["modality"] == modality)
            ]
            values = frame["relative_parameter_drift"].to_numpy(dtype=float)
            diag = diagnostics[
                (diagnostics["seed"].astype(int) == seed)
                & (diagnostics["modality"] == modality)
                & diagnostics["available"].astype(str).str.lower().eq("true")
            ]
            gradient_column = f"{modality}_projector_grad_norm"
            gradient = float(selected.loc[selected["seed"].astype(int) == seed, gradient_column].iloc[0])
            cosine = diag["original_vs_adapted_cosine"].to_numpy(dtype=float)
            l2_delta = diag["original_vs_adapted_l2_delta"].to_numpy(dtype=float)
            row_valid = bool(
                len(values)
                ...
                and np.isfinite(l2_delta).all()
                and float(np.max(l2_delta)) > 0.0
                and float(np.std(l2_delta)) > 0.0
            )
            valid &= row_valid
            rows.append(
                {
                    ...
                    "representation_cosine_mean": float(cosine.mean()),
                    "representation_cosine_minimum": float(cosine.min()),
                    "representation_l2_delta_mean": float(l2_delta.mean()),
                    "representation_l2_delta_maximum": float(l2_delta.max()),
...
def shortcut_audit(run_dir: Path) -> pd.DataFrame:
    ...
    for seed in SEEDS:
        ...
        correlations = {
            field: safe_spearman(
                frame[probability_column(frame)].to_numpy(dtype=float),
...
def freeze_validation_decision(
    ...
    if set(metrics["split"]) != {"val"}:
        raise RuntimeError("C32 validation decision requires validation-only metrics")
    comparison, positive, inversions = validation_comparisons(config, run_dir, metrics)
    drift_summary, diagnostics, health_pass = projector_health(run_dir)
    shortcuts = shortcut_audit(run_dir)

    c32_auc = comparison["AUC"].to_numpy(dtype=float)
    c27_auc = comparison["C27_AUC"].to_numpy(dtype=float)
    mean_auc = float(c32_auc.mean())
    std_auc = float(c32_auc.std(ddof=1))
    auc_pass = bool(
        mean_auc >= C27_MEAN + PROMOTION_GAIN
        and int((c32_auc > c27_auc).sum()) >= 2
        and float((c32_auc - c27_auc).min()) >= -0.01
        and std_auc <= 0.025
    )
    positive_pass = bool(
        int(positive["c17_tp_to_c32_fn"].sum())
        <= int(positive["c17_fn_to_c32_tp"].sum())
        and float(positive["c32_minus_c17_sensitivity"].min()) >= -0.05
        and int(positive["c32_material_positive_damage_count"].sum())
        <= int(positive["c27_material_positive_damage_count"].sum())
    )
    ranking_pass = bool(
        float(inversions["C32_inversions"].mean())
        <= float(inversions["C27_inversions"].mean())
        and int(inversions["C27_to_C32_repaired"].sum())
        >= int(inversions["C27_to_C32_introduced"].sum())
        and int(inversions["C32_minus_C27_inversions"].max()) <= 10
    )
    shortcut_pass = bool(
        shortcuts["shortcut_safety_pass"].astype(str).str.lower().eq("true").all()
    )
    if not health_pass:
        label = "DEMA_C32_TRAINING_INVALID"
    elif not shortcut_pass:
        label = "DEMA_C32_SHORTCUT_CONCERN"
    elif not positive_pass:
        label = "DEMA_C32_POSITIVE_DAMAGE"
    elif not ranking_pass:
        label = "DEMA_C32_RANKING_WORSENING"
    elif not auc_pass:
        label = "DEMA_C32_NO_AUC_GAIN"
    else:
        label = "PROMOTE_DEMA_C32_VPA"
    promoted = label == "PROMOTE_DEMA_C32_VPA"
    median_seed = int(
    ...
    decision = {
        ...
        "promoted": promoted,
        "strict_best": "DEMA_C32_VPA" if promoted else "KEEP_DEMA_C17_STRICT_BEST",
        "stop_label": None if promoted else "STOP_C32_VPA_TUNING",
    ...
    comparison.to_csv(report_dir / "c32_metrics_by_seed.csv", index=False)
        ...
        [
            {
                ...
                "C32_minus_C17_AUC_mean": float(comparison["C32_minus_C17_AUC"].mean()),
                "C32_minus_C27_AUC_mean": float(comparison["C32_minus_C27_AUC"].mean()),
            }
        ]
    )
    summary.to_csv(report_dir / "c32_metrics_summary.csv", index=False)
    pd.read_csv(run_dir / "reports" / "metrics_by_epoch.csv").to_csv(
        report_dir / "c32_metrics_by_epoch.csv", index=False
    )
    drift_summary.to_csv(report_dir / "c32_projector_drift.csv", index=False)
    diagnostics.to_csv(report_dir / "c32_patient_diagnostics_val.csv", index=False)
    positive.to_csv(report_dir / "c32_positive_preservation.csv", index=False)
    inversions.to_csv(report_dir / "c32_pairwise_inversion_summary.csv", index=False)
    shortcuts.to_csv(report_dir / "c32_shortcut_audit.csv", index=False)
    (report_dir / "c32_validation_decision.json").write_text(
        json.dumps(decision, indent=2) + "\n", encoding="utf-8"
    )
    route_lines = [
        "# C32-VPA Validation Route Decision",
        "",
        f"- Decision: `{label}`.",
    ...
    (report_dir / "c32_route_decision.md").write_text(
        "\n".join(route_lines) + "\n", encoding="utf-8"
    )
    return decision
...
def write_final_report(
    ...
    decision = json.loads(
        (report_dir / "c32_validation_decision.json").read_text(encoding="utf-8")
    )
    metrics = pd.read_csv(run_dir / "reports" / "metrics_by_seed.csv")
    if set(metrics["split"]) != {"val", "test"}:
        raise RuntimeError("C32 final report requires validation and reporting-only rows")
    val = pd.read_csv(report_dir / "c32_metrics_by_seed.csv")
    test = metrics[metrics["split"] == "test"].sort_values("seed")
    combined = pd.concat([val, test], ignore_index=True, sort=False)
    combined.to_csv(report_dir / "c32_metrics_by_seed.csv", index=False)
    summary = pd.read_csv(report_dir / "c32_metrics_summary.csv")
    summary = pd.concat(
        [
            summary,
            pd.DataFrame(
    ...
    summary.to_csv(report_dir / "c32_metrics_summary.csv", index=False)
    positive = pd.read_csv(report_dir / "c32_positive_preservation.csv")
    inversions = pd.read_csv(report_dir / "c32_pairwise_inversion_summary.csv")
    drift = pd.read_csv(report_dir / "c32_projector_drift.csv")
    shortcut = pd.read_csv(report_dir / "c32_shortcut_audit.csv")
    lines = [
        ...
        f"- Aggregate C17 TP->C32 FN / FN->C32 TP: `{int(positive['c17_tp_to_c32_fn'].sum())}`/`{int(positive['c17_fn_to_c32_tp'].sum())}`.",
        f"- Aggregate C27->C32 repaired/introduced pairs: `{int(inversions['C27_to_C32_repaired'].sum())}`/`{int(inversions['C27_to_C32_introduced'].sum())}`.",
        f"- Projector health rows passed: "
        f"`{int(drift['health_pass'].astype(str).str.lower().eq('true').sum())}/{len(drift)}`.",
    ...
    (report_dir / "phase_c32_dema_final_report.md").write_text(
        "\n".join(lines) + "\n", encoding="utf-8"
    )
    return decision
...
def main() -> None:
    ...
    if str(config.get("phase", "")).lower() != "c32":
        raise RuntimeError("C32 report requires the formal C32 config")
    ...
    if args.stage == "validation":
        ...
        status = "C32_VALIDATION_DECISION_FROZEN"
    else:
        decision = write_final_report(config, run_dir, report_dir)
        status = "C32_FINAL_REPORT_COMPLETE"

#!/usr/bin/env python3
"""Authorize C56-PPEK direct formal execution with a strict contract gate."""
...
from dmea_ht.c56_ppek import C56PPEKModel, HEAD_PREFIXES, trainable_parameter_count  # noqa: E402
from dmea_ht.config import load_config  # noqa: E402
from dmea_ht.visit_data import read_jsonl  # noqa: E402
from scripts.gate_phase_c54_lrra import clone_batch, finite_tensors, git_output, missing_batches, sha256_file  # noqa: E402
...
def parse_args() -> Any:
    ...
    parser.add_argument("--config", default="configs/dema_ht_c56_ppek_multiseed.yaml")
...
def main() -> None:
    ...
    source_text = inspect.getsource(C56PPEKModel)
    inherited_text = inspect.getsource(C47DRFEModel)
    route_text = source_text + inherited_text
    for seed in SEEDS:
        set_seed(seed)
        model = C56PPEKModel(config, seed).to(device)
    ...
    scope_pass &= bool(trainable_counts) and max(trainable_counts.values()) <= int(config["c56"]["trainable_parameter_limit"])
    train_source = (REPO_ROOT / "scripts" / "train_phase_c56.py").read_text(encoding="utf-8")
    ...
    ) and bool(config["c56"]["shortcut_fields_used_as_inputs"] is False)
    route_pass = (
        all(term in source_text for term in ("C56PPEKModel", "_fixed_kernel_features", "support_readout", "support_classifier", "opposition_head", "opposition_penalty", "softplus", "tanh"))
        and all(term in inherited_text for term in ("latest_mask", "history_mask", "variance", "log2", "raw_image", "aligned_image"))
        and list(config["c56"]["stream_order"]) == ["raw_image", "raw_text", "raw_bio", "aligned_image", "aligned_text", "aligned_bio"]
        and [tuple(pair) for pair in config["c56"]["interaction_pairs"]] == [(0, 1), (0, 2), (1, 2)]
        and abs(float(config["c56"]["opposition_penalty_bound"]) - 0.75) <= 1e-12
        and "visit_score" not in route_text
        and "temporal" not in route_text.lower()
        and "router" not in route_text.lower()
        and all(term not in route_text for term in ("C27", "C37", "C38", "C39", "C40", "C42", "C43", "C44", "MultiheadAttention"))
        and config["c56"]["learned_visit_score"] is False
        and config["c56"]["temporal_attention"] is False
        and config["c56"]["router"] is False
    )
    test_blocked = (
        "C56 Validation decision must be frozen before reporting-only Test" in train_source
        and 'set(metrics["split"]) != {"val"}' in train_source
        and "c56_validation_decision.json" in train_source
    )
    direct_contract = (
    ...
    checks = [
        ...
        ("06_polarity_kernel_gradients_and_frozen_source", gradient_pass),
        ("07_shortcut_fields_excluded", shortcut_pass),
        ("08_polarity_preserving_kernel_contract", route_pass),
    ...
    pd.DataFrame(gradient_rows).to_csv(report_dir / "c56_gradient_audit.csv", index=False)
    passed = sum(bool(value) for _, value in checks)
    status = "C56_PPEK_DIRECT_MULTI_SEED_AUTHORIZED" if passed == len(checks) else "C56_PATH_GATE_FAIL"
    payload = {
        "phase": "C56-PPEK",
        "status": status,
        "passed": passed,
    ...
    (report_dir / "c56_gate.json").write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"status": status, "passed": passed, "total": len(checks)}))
    if status != "C56_PPEK_DIRECT_MULTI_SEED_AUTHORIZED":
        raise SystemExit(2)

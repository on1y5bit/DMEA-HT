#!/usr/bin/env python3
"""Freeze and report C51-HRME Validation and reporting-only Test evidence."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--config", default="configs/dema_ht_c51_hrme_multiseed.yaml")
...
def freeze_validation_decision(
    ...
    if set(metrics["split"]) != {"val"}:
        raise RuntimeError("C51 Validation decision requires Validation-only metrics")
    comparisons, positive, inversions = base.validation_comparisons(config, run_dir, metrics)
    comparisons = rename_c41_columns(comparisons, "C41", "C51")
    positive = rename_c41_columns(positive, "c41", "c51")
    inversions = rename_c41_columns(inversions, "C41", "C51")
    ...
    shortcuts["combination"] = "C51-HRME"
    ...
    positive_pass = bool(
        float(positive["c51_minus_c17_sensitivity"].min()) >= -0.10
        and int(positive["c17_tp_to_c51_fn"].sum())
        <= int(positive["c17_fn_to_c51_tp"].sum()) + 3
    )
    mean_c27_inversions = float(inversions["C27_inversions"].mean())
    mean_c51_inversions = float(inversions["C51_inversions"].mean())
    ranking_pass = bool(
        (mean_c51_inversions - mean_c27_inversions) / max(mean_c27_inversions, 1.0) <= 0.10
        and int(inversions["C51_minus_C27_inversions"].max()) <= 20
    )
    ...
    for path in sorted((run_dir / "seed_runs").glob("seed_*/reports/run_config.json")):
        ...
        capacity_pass &= int(runtime["trainable_parameter_count"]) <= int(config["c51"]["trainable_parameter_limit"])
    if not capacity_pass or not health_pass:
        label = "C51_TRAINING_INVALID"
    elif not shortcut_pass:
        label = "C51_SHORTCUT_CONCERN"
    elif not positive_pass:
        label = "C51_POSITIVE_DAMAGE"
    elif not ranking_pass:
        label = "C51_RANKING_DAMAGE"
    elif not auc_pass:
        label = "C51_NO_AUC_GAIN"
    else:
    ...
    decision = {
        ...
        "strict_best": "C51_HRME" if promoted else "KEEP_DEMA_C17_STRICT_BEST",
    ...
    comparisons.to_csv(report_dir / "c51_metrics_by_seed.csv", index=False)
    pd.DataFrame(
        [
            {
                "split": "val",
                ...
                "C51_minus_C27_AUC_mean": float(comparisons["C51_minus_C27_AUC"].mean()),
            }
        ]
    ).to_csv(report_dir / "c51_metrics_summary.csv", index=False)
    epoch.to_csv(report_dir / "c51_metrics_by_epoch.csv", index=False)
    pd.read_csv(run_dir / "reports" / "parameter_drift.csv").to_csv(report_dir / "c51_parameter_drift.csv", index=False)
    pd.read_csv(run_dir / "reports" / "patient_diagnostics_val.csv").to_csv(report_dir / "c51_patient_diagnostics_val.csv", index=False)
    health.to_csv(report_dir / "c51_training_health.csv", index=False)
    positive.to_csv(report_dir / "c51_positive_preservation.csv", index=False)
    inversions.to_csv(report_dir / "c51_pairwise_inversion_summary.csv", index=False)
    shortcuts.to_csv(report_dir / "c51_shortcut_audit.csv", index=False)
    (report_dir / "c51_validation_decision.json").write_text(json.dumps(decision, indent=2) + "\n", encoding="utf-8")
    (report_dir / "c51_route_decision.md").write_text(
        "\n".join(
            [
                "# C51-HRME Validation Decision",
                "",
                f"- Decision: `{label}`.",
...
def write_final_report(config: Mapping[str, Any], run_dir: Path, report_dir: Path) -> Dict[str, Any]:
    decision = json.loads((report_dir / "c51_validation_decision.json").read_text(encoding="utf-8"))
    metrics = pd.read_csv(run_dir / "reports" / "metrics_by_seed.csv")
    if set(metrics["split"]) != {"val", "test"}:
        raise RuntimeError("C51 final report requires Validation and reporting-only Test rows")
    test = metrics[metrics["split"] == "test"]
    summary = pd.read_csv(report_dir / "c51_metrics_summary.csv")
    summary = pd.concat(
        [
            summary,
            pd.DataFrame(
    ...
    summary.to_csv(report_dir / "c51_metrics_summary.csv", index=False)
    positive = pd.read_csv(report_dir / "c51_positive_preservation.csv")
    inversions = pd.read_csv(report_dir / "c51_pairwise_inversion_summary.csv")
    health = pd.read_csv(report_dir / "c51_training_health.csv")
    shortcut = pd.read_csv(report_dir / "c51_shortcut_audit.csv")
    lines = [
        "# DMEA-HT Phase C51-HRME Final Report",
        "",
        ...
        f"- Aggregate C17 TP-to-C51 FN / FN-to-C51 TP: `{int(positive['c17_tp_to_c51_fn'].sum())}`/`{int(positive['c17_fn_to_c51_tp'].sum())}`.",
        f"- C27-to-C51 repaired/introduced pairs: `{int(inversions['C27_to_C51_repaired'].sum())}`/`{int(inversions['C27_to_C51_introduced'].sum())}`.",
    ...
    (report_dir / "phase_c51_dema_final_report.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
    return decision
...
def main() -> None:
    ...
    if str(config.get("phase", "")).lower() != "c51":
        raise RuntimeError("C51 report requires the formal C51 config")
    ...
    if args.stage == "validation":
        ...
        print(json.dumps({"status": "C51_VALIDATION_DECISION_FROZEN", "decision": decision["decision_label"]}))
    else:
        decision = write_final_report(config, run_dir, report_dir)
        print(json.dumps({"status": "C51_FINAL_REPORT_COMPLETE", "decision": decision["decision_label"]}))

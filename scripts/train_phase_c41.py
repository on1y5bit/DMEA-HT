#!/usr/bin/env python3
"""Train C41-MELR as direct, independent formal validation shards."""
...
from dmea_ht.c41_melr import (  # noqa: E402
    C41MELRModel,
    HEAD_PREFIXES,
    MODALITY_NAMES,
...
def build_loaders(config: Mapping[str, Any], rows: Sequence[Dict[str, Any]], splits: Sequence[str]) -> Dict[str, DataLoader]:
...
def gradient_norm(model: C41MELRModel) -> float:
    total = 0.0
...
def parameter_drift_rows(model: C41MELRModel, initial_state: Mapping[str, torch.Tensor], seed: int) -> List[Dict[str, Any]]:
...
def run_epoch(
    ...
    modality_states: List[np.ndarray] = []
    routing_states: List[np.ndarray] = []
    ...
    for batch in loader:
        ...
        with torch.set_grad_enabled(is_train):
            ...
            if not bool(torch.isfinite(loss)):
                raise RuntimeError("C41 non-finite BCE loss")
            if is_train:
        ...
        modality_states.append(arrays["modality_tokens"])
        routing_states.append(arrays["routing_weights"])
        for index, patient_id in enumerate(batch["patient_id"]):
            row: Dict[str, Any] = {
                ...
                "evidence_logit": float(arrays["evidence_logit"][index]),
                "consensus_logit": float(arrays["consensus_logit"][index]),
            }
            for modality_index, modality_name in enumerate(MODALITY_NAMES):
                row[f"routing_weight_{modality_name}"] = float(arrays["routing_weights"][index, modality_index])
                row[f"modality_logit_{modality_name}"] = float(arrays["modality_logits"][index, modality_index])
                row[f"trajectory_available_{modality_name}"] = bool(arrays["trajectory_available"][index, modality_index])
                row[f"modality_state_norm_{modality_name}"] = float(np.linalg.norm(arrays["modality_tokens"][index, modality_index]))
    ...
    metrics.update(
        {
            ...
            "modality_state_component_std": float(np.concatenate(modality_states, axis=0).std()),
            "routing_component_std": float(np.concatenate(routing_states, axis=0).std()),
    ...
    return {
        ...
        "modality_states": np.concatenate(modality_states, axis=0),
        "routing_states": np.concatenate(routing_states, axis=0),
...
def train_seed(config: Dict[str, Any], rows: Sequence[Dict[str, Any]], seed: int, seed_dir: Path, device: torch.device) -> Dict[str, Any]:
    ...
    model = C41MELRModel(config, seed).to(device)
    names = trainable_parameter_names(model)
    if not names or any(not name.startswith(HEAD_PREFIXES) for name in names):
        raise RuntimeError(f"C41 trainable scope violation: {names}")
    if any(parameter.requires_grad for name, parameter in model.named_parameters() if name.startswith("sources.")):
        raise RuntimeError("C41 source encoders and projectors must remain frozen")
    count = trainable_parameter_count(model)
    if count > int(config["c41"]["trainable_parameter_limit"]):
        raise RuntimeError(f"C41 capacity contract failed: {count}")
    ...
    if best_state is None:
        raise RuntimeError(f"C41 seed {seed} produced no checkpoint")
    model.load_state_dict(best_state, strict=True)
    for row in epoch_rows:
        row["selected_by_val_auc"] = int(row["epoch"]) == best_epoch
    val_result = run_epoch(model, loaders["val"], None, device)
    if val_result["metrics"]["prediction_std"] <= 0.0:
        raise RuntimeError(f"C41 seed {seed} produced constant validation predictions")
...
def save_split(result: Dict[str, Any], out_dir: Path, split: str) -> Dict[str, Any]:
    ...
    np.savez_compressed(
        ...
        modality_states=split_result["modality_states"][order].astype(np.float32),
        routing_states=split_result["routing_states"][order].astype(np.float32),
    )
...
def validation_seed_stage(config: Dict[str, Any], rows: Sequence[Dict[str, Any]], seed: int, out_dir: Path, device: torch.device) -> None:
    ...
    if seed_dir.exists():
        raise RuntimeError(f"C41 seed output already exists: {seed_dir}")
    ...
    status_path.write_text(json.dumps(status, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"status": "C41_VALIDATION_SEED_COMPLETE", "seed": seed}))
...
def validation_finalize_stage(config: Dict[str, Any], out_dir: Path, device: torch.device) -> None:
    ...
    for seed in SEEDS:
        ...
        if status.get("status") != "COMPLETE":
            raise RuntimeError(f"C41 seed {seed} shard incomplete")
    ...
    print(json.dumps({"status": "C41_VALIDATION_COMPLETE", "seeds": list(SEEDS)}))
...
def checkpoint_payload(path: Path) -> Mapping[str, Any]:
    ...
    if not isinstance(payload, Mapping):
        raise RuntimeError(f"Invalid C41 checkpoint payload: {path}")
    return payload


def reporting_test_stage(config: Dict[str, Any], rows: Sequence[Dict[str, Any]], out_dir: Path, device: torch.device) -> None:
    decision_path = resolve_path(config["project"]["report_dir"]) / "c41_validation_decision.json"
    if not decision_path.exists():
        raise RuntimeError("C41 Validation decision must be frozen before reporting-only Test")
    ...
    if not decision.get("validation_decision_frozen_before_test", False) or decision.get("test_used_for_decision", True) or decision.get("ensemble_used", True):
        raise RuntimeError("C41 Validation/Test isolation contract failed")
    metrics_path = out_dir / "reports" / "metrics_by_seed.csv"
    metrics = pd.read_csv(metrics_path)
    if set(metrics["split"]) != {"val"}:
        raise RuntimeError("C41 reporting-only Test requires Validation-only metrics")
    loader = build_loaders(config, rows, ("test",))["test"]
    for seed in SEEDS:
        model = C41MELRModel(config, seed).to(device)
        payload = checkpoint_payload(out_dir / "checkpoints" / f"seed_{seed}_best.pt")
        if int(payload.get("seed", -1)) != seed:
            raise RuntimeError(f"C41 checkpoint seed mismatch for {seed}")
    ...
    status_path.write_text(json.dumps(status, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"status": "C41_REPORTING_TEST_COMPLETE", "seeds": list(SEEDS)}))
...
def direct_multiseed_stage(config_path: Path, config: Dict[str, Any], rows: Sequence[Dict[str, Any]], out_dir: Path, device: torch.device) -> None:
    gate_path = resolve_path(config["project"]["report_dir"]) / "c41_gate.json"
    if not gate_path.exists():
        raise RuntimeError("C41 direct execution requires the completed gate")
    gate = json.loads(gate_path.read_text(encoding="utf-8"))
    if gate.get("status") != "C41_MELR_DIRECT_MULTI_SEED_AUTHORIZED" or int(gate.get("passed", 0)) != int(gate.get("total", 0)):
        raise RuntimeError("C41 direct execution requires an authorized gate")
    if (out_dir / "seed_runs").exists():
        raise RuntimeError("C41 formal seed outputs already exist")
    for child in ("reports", "predictions", "checkpoints", "representations"):
    ...
    if any(code != 0 for code in codes):
        raise RuntimeError(f"C41 validation shard failed: {codes}")
    subprocess.run([sys.executable, str(script), "--config", str(config_path), "--stage", "validation-finalize"], check=True)
    collector = REPO_ROOT / "scripts" / "collect_phase_c41_report.py"
    ...
    print(json.dumps({"status": "C41_DIRECT_MULTI_SEED_COMPLETE", "seeds": list(SEEDS)}))
...
def main() -> None:
    ...
    if str(config.get("phase", "")).lower() != "c41":
        raise RuntimeError("C41 phase contract is missing")
    if [int(seed) for seed in config["training"]["seeds"]] != list(SEEDS):
        raise RuntimeError("C41 formal seeds must remain [0, 42, 3407]")
    ...
    if args.stage == "validation-seed":
        if args.seed not in SEEDS:
            raise RuntimeError(f"Unsupported C41 seed: {args.seed}")

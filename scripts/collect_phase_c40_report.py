#!/usr/bin/env python3
"""Freeze and report C40-BCEQ Validation and reporting-only Test evidence."""
...
from scripts.train_phase_c40 import SEEDS  # noqa: E402


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--config", default="configs/dema_ht_c40_bceq_multiseed.yaml")
...
def ensure_audit_shortcut_columns(frame: pd.DataFrame, config: Mapping[str, Any]) -> pd.DataFrame:
    ...
    if any(field not in result.columns or result[field].isna().all() for field in missing):
        raise RuntimeError(f"C40 audit-only shortcut columns unavailable: {missing}")
    return result
...
def aligned_validation(config: Mapping[str, Any], run_dir: Path, seed: int) -> Tuple[pd.DataFrame, Dict[str, pd.DataFrame], np.ndarray]:
    c40 = read_prediction(run_dir / "predictions" / f"val_predictions_seed_{seed}.csv")
    baselines = {
    ...
    ids = c40["patient_id"].to_numpy(dtype=str)
    labels = c40["label"].to_numpy(dtype=int)
    if len(c40) != 94 or int((labels == 1).sum()) != 47:
        raise RuntimeError(f"C40 validation balance failed for seed {seed}")
    for name, frame in baselines.items():
        if not np.array_equal(ids, frame["patient_id"].to_numpy(dtype=str)):
            raise RuntimeError(f"C40 {name} patient alignment failed for seed {seed}")
        if not np.array_equal(labels, frame["label"].to_numpy(dtype=int)):
            raise RuntimeError(f"C40 {name} label alignment failed for seed {seed}")
    return c40, baselines, labels
...
def validation_comparisons(config: Mapping[str, Any], run_dir: Path, metrics: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    for seed in SEEDS:
        c40, baselines, labels = aligned_validation(config, run_dir, seed)
        probabilities = {
            "C40": c40[probability_column(c40)].to_numpy(dtype=float),
        ...
        if len(metric) != 1:
            raise RuntimeError(f"C40 validation metric row missing for seed {seed}")
        row = metric.iloc[0].to_dict()
        metric_rows.append(
            {
                **row,
                **{f"{name}_AUC": auc(labels, value) for name, value in probabilities.items()},
                **{f"C40_minus_{name}_AUC": float(row["AUC"]) - auc(labels, value) for name, value in probabilities.items() if name != "C40"},
            }
        )
        c17_class = probabilities["C17"] >= 0.5
        c40_class = probabilities["C40"] >= 0.5
        c17_counts = binary_counts(labels, probabilities["C17"])
        c40_counts = binary_counts(labels, probabilities["C40"])
        positive = labels == 1
        positive_rows.append(
            {
                "seed": seed,
                "c17_tp_to_c40_fn": int((positive & c17_class & ~c40_class).sum()),
                "c17_fn_to_c40_tp": int((positive & ~c17_class & c40_class).sum()),
                "c17_sensitivity": c17_counts["Sensitivity"],
                "c40_sensitivity": c40_counts["Sensitivity"],
                "c40_minus_c17_sensitivity": c40_counts["Sensitivity"] - c17_counts["Sensitivity"],
            }
        )
        c27_inversions = inversion_vector(labels, probabilities["C27"])
        c40_inversions = inversion_vector(labels, probabilities["C40"])
        inversion_rows.append(
            {
                ...
                "C40_inversions": int(c40_inversions.sum()),
                "C40_minus_C27_inversions": int(c40_inversions.sum() - c27_inversions.sum()),
                "C40_inversion_ratio_vs_C27": float((c40_inversions.sum() - c27_inversions.sum()) / max(c27_inversions.sum(), 1)),
                "C27_to_C40_repaired": int((c27_inversions & ~c40_inversions).sum()),
                "C27_to_C40_introduced": int((~c27_inversions & c40_inversions).sum()),
            }
        )
...
def freeze_validation_decision(config: Mapping[str, Any], run_dir: Path, report_dir: Path) -> Dict[str, Any]:
    ...
    if set(metrics["split"]) != {"val"}:
        raise RuntimeError("C40 validation decision requires validation-only metrics")
    ...
    positive_pass = bool(float(positive["c40_minus_c17_sensitivity"].min()) >= -0.10 and int(positive["c17_tp_to_c40_fn"].sum()) <= int(positive["c17_fn_to_c40_tp"].sum()) + 3)
    mean_c27_inversions = float(inversions["C27_inversions"].mean())
    mean_c40_inversions = float(inversions["C40_inversions"].mean())
    ranking_pass = bool((mean_c40_inversions - mean_c27_inversions) / max(mean_c27_inversions, 1.0) <= 0.10 and int(inversions["C40_minus_C27_inversions"].max()) <= 20)
    ...
    for path in sorted((run_dir / "seed_runs").glob("seed_*/reports/run_config.json")):
        ...
        capacity_pass &= int(runtime["trainable_parameter_count"]) <= int(config["c40"]["trainable_parameter_limit"])
    if not capacity_pass or not health_pass:
        label = "C40_TRAINING_INVALID"
    elif not shortcut_pass:
        label = "C40_SHORTCUT_CONCERN"
    elif not positive_pass:
        label = "C40_POSITIVE_DAMAGE"
    elif not ranking_pass:
        label = "C40_RANKING_DAMAGE"
    elif not auc_pass:
        label = "C40_NO_AUC_GAIN"
    else:
    ...
    decision = {
        ...
        "strict_best": "C40_BCEQ" if promoted else "KEEP_DEMA_C17_STRICT_BEST",
    ...
    comparisons.to_csv(report_dir / "c40_metrics_by_seed.csv", index=False)
    pd.DataFrame([{"split": "val", "AUC_mean": mean_auc, "AUC_std": std_auc, "C17_AUC_mean": float(comparisons["C17_AUC"].mean()), "C27_AUC_mean": float(comparisons["C27_AUC"].mean()), "C40_minus_C27_AUC_mean": float(comparisons["C40_minus_C27_AUC"].mean())}]).to_csv(report_dir / "c40_metrics_summary.csv", index=False)
    epoch.to_csv(report_dir / "c40_metrics_by_epoch.csv", index=False)
    pd.read_csv(run_dir / "reports" / "parameter_drift.csv").to_csv(report_dir / "c40_parameter_drift.csv", index=False)
    pd.read_csv(run_dir / "reports" / "patient_diagnostics_val.csv").to_csv(report_dir / "c40_patient_diagnostics_val.csv", index=False)
    health.to_csv(report_dir / "c40_training_health.csv", index=False)
    positive.to_csv(report_dir / "c40_positive_preservation.csv", index=False)
    inversions.to_csv(report_dir / "c40_pairwise_inversion_summary.csv", index=False)
    shortcuts.to_csv(report_dir / "c40_shortcut_audit.csv", index=False)
    (report_dir / "c40_validation_decision.json").write_text(json.dumps(decision, indent=2) + "\n", encoding="utf-8")
    (report_dir / "c40_route_decision.md").write_text(
        "\n".join(
            [
                "# C40-BCEQ Validation Decision",
                "",
                f"- Decision: `{label}`.",
...
def write_final_report(config: Mapping[str, Any], run_dir: Path, report_dir: Path) -> Dict[str, Any]:
    decision = json.loads((report_dir / "c40_validation_decision.json").read_text(encoding="utf-8"))
    metrics = pd.read_csv(run_dir / "reports" / "metrics_by_seed.csv")
    if set(metrics["split"]) != {"val", "test"}:
        raise RuntimeError("C40 final report requires validation and reporting-only rows")
    test = metrics[metrics["split"] == "test"]
    summary = pd.read_csv(report_dir / "c40_metrics_summary.csv")
    summary = pd.concat(
    ...
    summary.to_csv(report_dir / "c40_metrics_summary.csv", index=False)
    positive = pd.read_csv(report_dir / "c40_positive_preservation.csv")
    inversions = pd.read_csv(report_dir / "c40_pairwise_inversion_summary.csv")
    health = pd.read_csv(report_dir / "c40_training_health.csv")
    shortcut = pd.read_csv(report_dir / "c40_shortcut_audit.csv")
    lines = [
        "# DMEA-HT Phase C40-BCEQ Final Report",
        "",
        ...
        f"- Aggregate C17 TP-to-C40 FN / FN-to-C40 TP: `{int(positive['c17_tp_to_c40_fn'].sum())}`/`{int(positive['c17_fn_to_c40_tp'].sum())}`.",
        f"- C27-to-C40 repaired/introduced pairs: `{int(inversions['C27_to_C40_repaired'].sum())}`/`{int(inversions['C27_to_C40_introduced'].sum())}`.",
    ...
    (report_dir / "phase_c40_dema_final_report.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
    return decision
...
def main() -> None:
    ...
    if str(config.get("phase", "")).lower() != "c40":
        raise RuntimeError("C40 report requires the formal C40 config")
    ...
    if args.stage == "validation":
        ...
        print(json.dumps({"status": "C40_VALIDATION_DECISION_FROZEN", "decision": decision["decision_label"]}))
    else:
        decision = write_final_report(config, run_dir, report_dir)
        print(json.dumps({"status": "C40_FINAL_REPORT_COMPLETE", "decision": decision["decision_label"]}))

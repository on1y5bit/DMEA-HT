#!/usr/bin/env python3
"""Authorize C41-MELR direct formal execution with a compact contract gate."""
...
from dmea_ht.c41_melr import C41MELRModel, HEAD_PREFIXES, trainable_parameter_count  # noqa: E402
...
from scripts.train_phase_c41 import SEEDS, build_loaders, move_batch, set_seed  # noqa: E402
...
def main() -> None:
    ...
    source_text = inspect.getsource(C41MELRModel)
    for seed in SEEDS:
        set_seed(seed)
    ...
    trainable_scope_pass = bool(trainable_counts) and max(trainable_counts.values()) <= int(config["c41"]["trainable_parameter_limit"])
    config_text = config_path.read_text(encoding="utf-8")
    train_source = (REPO_ROOT / "scripts" / "train_phase_c41.py").read_text(encoding="utf-8")
    collector_source = (REPO_ROOT / "scripts" / "collect_phase_c41_report.py").read_text(encoding="utf-8")
    shortcut_pass = not any(field in source_text for field in ("patient_id", "selected_n_visits", "raw_n_visits", "used_images", "raw_n_images", "image_padding_count", "report_length", "source_folder")) and bool(config["c41"]["shortcut_fields_used_as_inputs"] is False)
    route_pass = (
        "latest" in source_text
        and "history" in source_text
        and "dispersion" in source_text
        and "log2" in source_text
        and "routing_weights" in source_text
        and "consensus_head" in source_text
        and "C27" not in source_text
        and "C37" not in source_text
        and "C38" not in source_text
        and "C39" not in source_text
        and "C40" not in source_text
        and "visit_score" not in source_text
        and "temporal_linear" not in source_text
    )
    test_blocked = "Validation decision must be frozen before reporting-only Test" in train_source and "set(metrics[\"split\"]) != {\"val\"}" in train_source and "c41_validation_decision.json" in train_source
    ...
    loss_contract = bool(config["loss"]["bce_only"]) and train_source.count("binary_cross_entropy_with_logits") == 1 and "scheduler" not in train_source.lower() and "threshold" not in train_source.lower()
    ...
    checks = [
        ...
        ("08_fixed_trajectory_dispersion_and_evidence_routing", route_pass and config["c41"]["fixed_recency_kernel"] == "inverse_log2_age" and config["c41"]["learned_visit_score"] is False),
    ...
    pd.DataFrame(gradient_rows).to_csv(report_dir / "c41_gradient_audit.csv", index=False)
    pd.DataFrame(checkpoint_rows).to_csv(report_dir / "c41_c17_checkpoint_audit.csv", index=False)
    passed = sum(bool(value) for _, value in checks)
    status = "C41_MELR_DIRECT_MULTI_SEED_AUTHORIZED" if passed == len(checks) else "C41_PATH_GATE_FAIL"
    payload = {"phase": "C41-MELR", "status": status, "passed": passed, "total": len(checks), "git_commit": git_output("rev-parse", "HEAD"), "branch": branch, "device": str(device), "gpu": torch.cuda.get_device_name(0) if torch.cuda.is_available() else "cpu", "trainable_parameter_count_by_seed": trainable_counts, "checks": [{"name": name, "passed": bool(value)} for name, value in checks]}
    (report_dir / "c41_gate.json").write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"status": status, "passed": passed, "total": len(checks)}))
    if status != "C41_MELR_DIRECT_MULTI_SEED_AUTHORIZED":
        raise SystemExit(2)

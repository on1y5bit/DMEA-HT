#!/usr/bin/env python3
"""Train C44-MISE as direct, independent formal validation shards."""
...
from dmea_ht.c44_mise import (  # noqa: E402
    C44MISEModel,
    HEAD_PREFIXES,
...
def train_seed(
    config: Dict[str, Any], rows: Sequence[Dict[str, Any]], seed: int, seed_dir: Path, device: torch.device
) -> Dict[str, Any]:
    core.set_seed(seed)
    ...
    if not names or any(not name.startswith(HEAD_PREFIXES) for name in names):
        raise RuntimeError(f"C44 trainable scope violation: {names}")
    if any(parameter.requires_grad for name, parameter in model.named_parameters() if name.startswith("sources.")):
        raise RuntimeError("C44 C17 sources must remain frozen")
    count = trainable_parameter_count(model)
    if count > int(config["c44"]["trainable_parameter_limit"]):
        raise RuntimeError(f"C44 capacity contract failed: {count}")
    initial_state = {
    ...
    for epoch in range(1, int(config["training"]["epochs"]) + 1):
        train_result = core.run_epoch(model, loaders["train"], optimizer, device)
    ...
    if best_state is None:
        raise RuntimeError(f"C44 seed {seed} produced no checkpoint")
    model.load_state_dict(best_state, strict=True)
    for row in epoch_rows:
        row["selected_by_val_auc"] = int(row["epoch"]) == best_epoch
    val_result = core.run_epoch(model, loaders["val"], None, device)
    if val_result["metrics"]["prediction_std"] <= 0.0:
        raise RuntimeError(f"C44 seed {seed} produced constant validation predictions")
    torch.save(
        {
...
def validation_seed_stage(config: Dict[str, Any], rows: Sequence[Dict[str, Any]], seed: int, out_dir: Path, device: torch.device) -> None:
    ...
    if seed_dir.exists():
        raise RuntimeError(f"C44 seed output already exists: {seed_dir}")
    ...
    status_path.write_text(json.dumps(status, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"status": "C44_VALIDATION_SEED_COMPLETE", "seed": seed}))
...
def validation_finalize_stage(config: Dict[str, Any], out_dir: Path, device: torch.device) -> None:
    ...
    for seed in SEEDS:
        ...
        if status.get("status") != "COMPLETE":
            raise RuntimeError(f"C44 seed {seed} shard incomplete")
    ...
    (out_dir / "reports" / "run_status.json").write_text(json.dumps({"phase": "C44-MISE", "status": "VALIDATION_COMPLETE", "started_at": min(item["started_at"] for item in statuses), "finished_at": core.timestamp(), "completed_seeds": list(SEEDS), "parallel_seed_training": True, "device": str(device), "gpu": torch.cuda.get_device_name(0) if torch.cuda.is_available() else "cpu"}, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"status": "C44_VALIDATION_COMPLETE", "seeds": list(SEEDS)}))


def reporting_test_stage(config: Dict[str, Any], rows: Sequence[Dict[str, Any]], out_dir: Path, device: torch.device) -> None:
    decision_path = resolve_path(config["project"]["report_dir"]) / "c44_validation_decision.json"
    if not decision_path.exists():
        raise RuntimeError("C44 Validation decision must be frozen before reporting-only Test")
    ...
    if not decision.get("validation_decision_frozen_before_test", False) or decision.get("test_used_for_decision", True) or decision.get("ensemble_used", True):
        raise RuntimeError("C44 Validation/Test isolation contract failed")
    metrics_path = out_dir / "reports" / "metrics_by_seed.csv"
    metrics = pd.read_csv(metrics_path)
    if set(metrics["split"]) != {"val"}:
        raise RuntimeError("C44 reporting-only Test requires Validation-only metrics")
    ...
    for seed in SEEDS:
        ...
        if int(payload.get("seed", -1)) != seed:
            raise RuntimeError(f"C44 checkpoint seed mismatch for {seed}")
        model.load_state_dict(payload["model"], strict=True)
        result = core.run_epoch(model, loader, None, device)
        metrics = pd.concat([metrics, pd.DataFrame([core.save_split({"seed": seed, "best_epoch": payload["best_epoch"], "test": result}, out_dir, "test")])], ignore_index=True)
        del model
    ...
    status_path.write_text(json.dumps(status, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"status": "C44_REPORTING_TEST_COMPLETE", "seeds": list(SEEDS)}))
...
def direct_multiseed_stage(config_path: Path, config: Dict[str, Any], rows: Sequence[Dict[str, Any]], out_dir: Path, device: torch.device) -> None:
    gate_path = resolve_path(config["project"]["report_dir"]) / "c44_gate.json"
    if not gate_path.exists():
        raise RuntimeError("C44 direct execution requires the completed gate")
    gate = json.loads(gate_path.read_text(encoding="utf-8"))
    if gate.get("status") != "C44_MISE_DIRECT_MULTI_SEED_AUTHORIZED" or int(gate.get("passed", 0)) != int(gate.get("total", 0)):
        raise RuntimeError("C44 direct execution requires an authorized gate")
    if (out_dir / "seed_runs").exists():
        raise RuntimeError("C44 formal seed outputs already exist")
    for child in ("reports", "predictions", "checkpoints", "representations"):
    ...
    if any(code != 0 for code in codes):
        raise RuntimeError(f"C44 validation shard failed: {codes}")
    subprocess.run([sys.executable, str(script), "--config", str(config_path), "--stage", "validation-finalize"], check=True)
    collector = REPO_ROOT / "scripts" / "collect_phase_c44_report.py"
    ...
    print(json.dumps({"status": "C44_DIRECT_MULTI_SEED_COMPLETE", "seeds": list(SEEDS)}))
...
def main() -> None:
    ...
    if str(config.get("phase", "")).lower() != "c44":
        raise RuntimeError("C44 phase contract is missing")
    if [int(seed) for seed in config["training"]["seeds"]] != list(SEEDS):
        raise RuntimeError("C44 formal seeds must remain [0, 42, 3407]")
    ...
    if args.stage == "validation-seed":
        if args.seed not in SEEDS:
            raise RuntimeError(f"Unsupported C44 seed: {args.seed}")

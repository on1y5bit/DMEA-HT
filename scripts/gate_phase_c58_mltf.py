#!/usr/bin/env python3
"""Authorize C58-MLTF direct formal execution with a strict contract gate."""
...
from dmea_ht.c58_mltf import C58MLTFModel, HEAD_PREFIXES, trainable_parameter_count  # noqa: E402
...
def main() -> None:
    ...
    source_text = inspect.getsource(C58MLTFModel)
    ...
    scope_pass &= bool(trainable_counts) and max(trainable_counts.values()) <= int(config["c58"]["trainable_parameter_limit"])
    train_source = (REPO_ROOT / "scripts" / "train_phase_c58.py").read_text(encoding="utf-8")
    ...
    ) and bool(config["c58"]["shortcut_fields_used_as_inputs"] is False)
    route_pass = (
        all(
            term in source_text
            for term in (
        ...
        and list(config["c58"]["stream_order"]) == ["raw_image", "raw_text", "raw_bio", "aligned_image", "aligned_text", "aligned_bio"]
        and int(config["c58"]["fusion_rank"]) == 32
        ...
        and config["c58"]["learned_visit_score"] is False
        and config["c58"]["temporal_attention"] is False
        and config["c58"]["router"] is False
    )
    test_blocked = (
        "C58 Validation decision must be frozen before reporting-only Test" in train_source
        and 'set(metrics["split"]) != {"val"}' in train_source
        and "c58_validation_decision.json" in train_source
    )
    direct_contract = (
    ...
    checks = [
        ...
        ("06_multimodal_tensor_fusion_gradients_and_frozen_source", gradient_pass),
        ("07_shortcut_fields_excluded", shortcut_pass),
        ("08_low_rank_multimodal_tensor_fusion_contract", route_pass),
    ...
    pd.DataFrame(gradient_rows).to_csv(report_dir / "c58_gradient_audit.csv", index=False)
    passed = sum(bool(value) for _, value in checks)
    status = "C58_MLTF_DIRECT_MULTI_SEED_AUTHORIZED" if passed == len(checks) else "C58_PATH_GATE_FAIL"
    payload = {
        "phase": "C58-MLTF",
        "status": status,
        "passed": passed,
    ...
    (report_dir / "c58_gate.json").write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"status": status, "passed": passed, "total": len(checks)}))
    if status != "C58_MLTF_DIRECT_MULTI_SEED_AUTHORIZED":
        raise SystemExit(2)

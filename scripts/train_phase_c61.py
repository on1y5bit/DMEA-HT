#!/usr/bin/env python3
"""Train C61-CBPI as direct, independent formal validation shards."""
...
from dmea_ht.c61_cbpi import C61CBPIModel, HEAD_PREFIXES  # noqa: E402
...
SEEDS = (0, 42, 3407)
shared.C54LRRAModel = C61CBPIModel
...
def runtime_config(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    translated["c54"] = dict(config["c61"])
    return translated


def mark_phase(path: Path, phase: str = "C61-CBPI") -> None:
    if not path.exists():
        return
...
def reporting_test_stage(
    ...
    decision_path = resolve_path(config["project"]["report_dir"]) / "c61_validation_decision.json"
    if not decision_path.exists():
        raise RuntimeError("C61 Validation decision must be frozen before reporting-only Test")
    ...
    if (
        ...
        raise RuntimeError("C61 Validation/Test isolation contract failed")
    metrics_path = out_dir / "reports" / "metrics_by_seed.csv"
    metrics = pd.read_csv(metrics_path)
    if set(metrics["split"]) != {"val"}:
        raise RuntimeError("C61 reporting-only Test requires Validation-only metrics")
    ...
    for seed in SEEDS:
        ...
        if int(payload.get("seed", -1)) != seed:
            raise RuntimeError(f"C61 checkpoint seed mismatch for {seed}")
    ...
    print(json.dumps({"status": "C61_REPORTING_TEST_COMPLETE", "seeds": list(SEEDS)}))
...
def direct_multiseed_stage(
    ...
    gate_path = resolve_path(config["project"]["report_dir"]) / "c61_gate.json"
    if not gate_path.exists():
        raise RuntimeError("C61 direct execution requires the completed gate")
    gate = json.loads(gate_path.read_text(encoding="utf-8"))
    if gate.get("status") != "C61_CBPI_DIRECT_MULTI_SEED_AUTHORIZED" or int(gate.get("passed", 0)) != int(gate.get("total", 0)):
        raise RuntimeError("C61 direct execution requires an authorized gate")
    if (out_dir / "seed_runs").exists():
        raise RuntimeError("C61 formal seed outputs already exist")
    ...
    if any(code != 0 for code in codes):
        raise RuntimeError(f"C61 validation shard failed: {codes}")
    validation_finalize_stage(config, out_dir, device)
    collector = REPO_ROOT / "scripts" / "collect_phase_c61_report.py"
    ...
    print(json.dumps({"status": "C61_DIRECT_MULTI_SEED_COMPLETE", "seeds": list(SEEDS)}))
...
def main() -> None:
    ...
    if str(config.get("phase", "")).lower() != "c61":
        raise RuntimeError("C61 phase contract is missing")
    if [int(seed) for seed in config["training"]["seeds"]] != list(SEEDS):
        raise RuntimeError("C61 formal seeds must remain [0, 42, 3407]")
    ...
    if args.stage == "validation-seed":
        if args.seed not in SEEDS:
            raise RuntimeError(f"Unsupported C61 seed: {args.seed}")

#!/usr/bin/env python3
"""Train C56-PPEK as direct, independent formal validation shards."""
...
from dmea_ht.c56_ppek import C56PPEKModel, HEAD_PREFIXES  # noqa: E402
...
SEEDS = (0, 42, 3407)
shared.C54LRRAModel = C56PPEKModel
...
def runtime_config(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    translated["c54"] = dict(config["c56"])
    return translated


def mark_phase(path: Path, phase: str = "C56-PPEK") -> None:
    if not path.exists():
        return
...
def reporting_test_stage(
    ...
    decision_path = resolve_path(config["project"]["report_dir"]) / "c56_validation_decision.json"
    if not decision_path.exists():
        raise RuntimeError("C56 Validation decision must be frozen before reporting-only Test")
    ...
    if (
        ...
        raise RuntimeError("C56 Validation/Test isolation contract failed")
    metrics_path = out_dir / "reports" / "metrics_by_seed.csv"
    metrics = pd.read_csv(metrics_path)
    if set(metrics["split"]) != {"val"}:
        raise RuntimeError("C56 reporting-only Test requires Validation-only metrics")
    ...
    for seed in SEEDS:
        ...
        if int(payload.get("seed", -1)) != seed:
            raise RuntimeError(f"C56 checkpoint seed mismatch for {seed}")
    ...
    status.update({"phase": "C56-PPEK", "status": "COMPLETE", "test_started_after_validation_decision": True, "finished_at": core.timestamp()})
    status_path.write_text(json.dumps(status, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"status": "C56_REPORTING_TEST_COMPLETE", "seeds": list(SEEDS)}))
...
def direct_multiseed_stage(
    ...
    gate_path = resolve_path(config["project"]["report_dir"]) / "c56_gate.json"
    if not gate_path.exists():
        raise RuntimeError("C56 direct execution requires the completed gate")
    gate = json.loads(gate_path.read_text(encoding="utf-8"))
    if gate.get("status") != "C56_PPEK_DIRECT_MULTI_SEED_AUTHORIZED" or int(gate.get("passed", 0)) != int(gate.get("total", 0)):
        raise RuntimeError("C56 direct execution requires an authorized gate")
    if (out_dir / "seed_runs").exists():
        raise RuntimeError("C56 formal seed outputs already exist")
    ...
    if any(code != 0 for code in codes):
        raise RuntimeError(f"C56 validation shard failed: {codes}")
    validation_finalize_stage(config, out_dir, device)
    collector = REPO_ROOT / "scripts" / "collect_phase_c56_report.py"
    ...
    print(json.dumps({"status": "C56_DIRECT_MULTI_SEED_COMPLETE", "seeds": list(SEEDS)}))
...
def main() -> None:
    ...
    if str(config.get("phase", "")).lower() != "c56":
        raise RuntimeError("C56 phase contract is missing")
    if [int(seed) for seed in config["training"]["seeds"]] != list(SEEDS):
        raise RuntimeError("C56 formal seeds must remain [0, 42, 3407]")
    ...
    if args.stage == "validation-seed":
        if args.seed not in SEEDS:
            raise RuntimeError(f"Unsupported C56 seed: {args.seed}")

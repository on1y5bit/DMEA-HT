#!/usr/bin/env python3
"""Authorize C42-E2E-PET direct formal execution with a compact contract gate."""
...
from dmea_ht.c42_e2e_pet import C42E2EPETModel, trainable_parameter_count  # noqa: E402
...
def main() -> None:
    ...
    source_text = inspect.getsource(C42E2EPETModel)
    for seed in SEEDS:
        set_seed(seed)
        model = C42E2EPETModel(config, seed).to(device)
        trainable_counts[str(seed)] = trainable_parameter_count(model)
        scope_pass &= all(parameter.requires_grad for parameter in model.parameters())
        model.train(True)
            ...
            for name, parameter in model.named_parameters():
                ...
                if finite and norm > 0.0:
                    if name.startswith("sources."):
                        ...
                        graph_gradient_count += 1
            gradient_pass &= source_gradient_count > 0 and graph_gradient_count >= 3
            gradient_rows.append({"seed": seed, "probe": probe_index, "source_gradient_tensors": source_gradient_count, "graph_gradient_tensors": graph_gradient_count})
        del model
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
    scope_pass &= bool(trainable_counts) and max(trainable_counts.values()) <= int(config["c42"]["trainable_parameter_limit"])
    train_source = (REPO_ROOT / "scripts" / "train_phase_c42.py").read_text(encoding="utf-8")
    base_train_source = (REPO_ROOT / "scripts" / "train_phase_c41.py").read_text(encoding="utf-8")
    collector_source = (REPO_ROOT / "scripts" / "collect_phase_c42_report.py").read_text(encoding="utf-8")
    shortcut_pass = not any(field in source_text for field in ("patient_id", "selected_n_visits", "raw_n_visits", "used_images", "raw_n_images", "image_padding_count", "report_length", "source_folder")) and bool(config["c42"]["shortcut_fields_used_as_inputs"] is False)
    route_pass = (
        "evidence_graph" in source_text
        and "patient_query" in source_text
        and "dispersion" in source_text
        and "log2" in source_text
        and "requires_grad_(True)" in source_text
        and "torch.no_grad" not in source_text
    ...
    checks = [
        ...
        ("04_end_to_end_scope_and_capacity", scope_pass),
    ...
    pd.DataFrame(gradient_rows).to_csv(report_dir / "c42_gradient_audit.csv", index=False)
    passed = sum(bool(value) for _, value in checks)
    status = "C42_E2E_PET_DIRECT_MULTI_SEED_AUTHORIZED" if passed == len(checks) else "C42_PATH_GATE_FAIL"
    payload = {"phase": "C42-E2E-PET", "status": status, "passed": passed, "total": len(checks), "git_commit": git_output("rev-parse", "HEAD"), "branch": branch, "device": str(device), "gpu": torch.cuda.get_device_name(0) if torch.cuda.is_available() else "cpu", "trainable_parameter_count_by_seed": trainable_counts, "checks": [{"name": name, "passed": bool(value)} for name, value in checks]}
    (report_dir / "c42_gate.json").write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"status": status, "passed": passed, "total": len(checks)}))
    if status != "C42_E2E_PET_DIRECT_MULTI_SEED_AUTHORIZED":
        raise SystemExit(2)

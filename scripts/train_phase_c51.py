#!/usr/bin/env python3
"""Train C51-HRME as direct, independent formal validation shards."""
...
from dmea_ht.c51_hrme import (  # noqa: E402
    C51HRMEModel,
    HEAD_PREFIXES,
...
def train_seed(
    ...
    model = C51HRMEModel(config, seed).to(device)
    names = trainable_parameter_names(model)
    if not names or any(not name.startswith(HEAD_PREFIXES) for name in names):
        raise RuntimeError(f"C51 trainable scope violation: {names}")
    if any(parameter.requires_grad for name, parameter in model.named_parameters() if name.startswith("sources.")):
        raise RuntimeError("C51 C17 sources must remain frozen")
    count = trainable_parameter_count(model)
    if count > int(config["c51"]["trainable_parameter_limit"]):
        raise RuntimeError(f"C51 capacity contract failed: {count}")
    initial_state = {
    ...
    if best_state is None:
        raise RuntimeError(f"C51 seed {seed} produced no checkpoint")
    model.load_state_dict(best_state, strict=True)
    for row in epoch_rows:
        row["selected_by_val_auc"] = int(row["epoch"]) == best_epoch
    val_result = core.run_epoch(model, loaders["val"], None, device)
    if val_result["metrics"]["prediction_std"] <= 0.0:
        raise RuntimeError(f"C51 seed {seed} produced constant validation predictions")
    torch.save(
        {
...
def validation_seed_stage(
    ...
    if seed_dir.exists():
        raise RuntimeError(f"C51 seed output already exists: {seed_dir}")
    ...
    status_path.write_text(json.dumps(status, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"status": "C51_VALIDATION_SEED_COMPLETE", "seed": seed}))
...
def validation_finalize_stage(config: Dict[str, Any], out_dir: Path, device: torch.device) -> None:
    ...
    for seed in SEEDS:
        ...
        if status.get("status") != "COMPLETE":
            raise RuntimeError(f"C51 seed {seed} shard incomplete")
    ...
    print(json.dumps({"status": "C51_VALIDATION_COMPLETE", "seeds": list(SEEDS)}))


def reporting_test_stage(
    config: Dict[str, Any], rows: Sequence[Dict[str, Any]], out_dir: Path, device: torch.device
) -> None:
    decision_path = resolve_path(config["project"]["report_dir"]) / "c51_validation_decision.json"
    if not decision_path.exists():
        raise RuntimeError("C51 Validation decision must be frozen before reporting-only Test")
    ...
    if (
        ...
        raise RuntimeError("C51 Validation/Test isolation contract failed")
    metrics_path = out_dir / "reports" / "metrics_by_seed.csv"
    metrics = pd.read_csv(metrics_path)
    if set(metrics["split"]) != {"val"}:
        raise RuntimeError("C51 reporting-only Test requires Validation-only metrics")
    ...
    for seed in SEEDS:
        ...
        if int(payload.get("seed", -1)) != seed:
            raise RuntimeError(f"C51 checkpoint seed mismatch for {seed}")
    ...
    status_path.write_text(json.dumps(status, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"status": "C51_REPORTING_TEST_COMPLETE", "seeds": list(SEEDS)}))
...
def direct_multiseed_stage(
    ...
    gate_path = resolve_path(config["project"]["report_dir"]) / "c51_gate.json"
    if not gate_path.exists():
        raise RuntimeError("C51 direct execution requires the completed gate")
    gate = json.loads(gate_path.read_text(encoding="utf-8"))
    if gate.get("status") != "C51_HRME_DIRECT_MULTI_SEED_AUTHORIZED" or int(gate.get("passed", 0)) != int(gate.get("total", 0)):
        raise RuntimeError("C51 direct execution requires an authorized gate")
    if (out_dir / "seed_runs").exists():
        raise RuntimeError("C51 formal seed outputs already exist")
    for child in ("reports", "predictions", "checkpoints", "representations"):
    ...
    if any(code != 0 for code in codes):
        raise RuntimeError(f"C51 validation shard failed: {codes}")
    subprocess.run([sys.executable, str(script), "--config", str(config_path), "--stage", "validation-finalize"], check=True)
    collector = REPO_ROOT / "scripts" / "collect_phase_c51_report.py"
    ...
    print(json.dumps({"status": "C51_DIRECT_MULTI_SEED_COMPLETE", "seeds": list(SEEDS)}))
...
def main() -> None:
    ...
    if str(config.get("phase", "")).lower() != "c51":
        raise RuntimeError("C51 phase contract is missing")
    if [int(seed) for seed in config["training"]["seeds"]] != list(SEEDS):
        raise RuntimeError("C51 formal seeds must remain [0, 42, 3407]")
    ...
    if args.stage == "validation-seed":
        if args.seed not in SEEDS:
            raise RuntimeError(f"Unsupported C51 seed: {args.seed}")

#!/usr/bin/env python3
"""Train C40-BCEQ as direct, independent formal validation shards."""
...
from dmea_ht.c40_bceq import (  # noqa: E402
    C40BCEQModel,
    EVIDENCE_NAMES,
    HEAD_PREFIXES,
...
def gradient_norm(model: C40BCEQModel) -> float:
    total = 0.0
...
def parameter_drift_rows(
    model: C40BCEQModel, initial_state: Mapping[str, torch.Tensor], seed: int
) -> List[Dict[str, Any]]:
    ...
    for batch in loader:
        ...
        with torch.set_grad_enabled(is_train):
            ...
            if not bool(torch.isfinite(loss)):
                raise RuntimeError("C40 non-finite BCE loss")
            if is_train:
        ...
        for index, patient_id in enumerate(batch["patient_id"]):
            ...
            attention = arrays["attention"][index]
            attention_entropy = float(
                -(attention.clip(1e-8, 1.0) * np.log(attention.clip(1e-8, 1.0))).sum()
            )
            row: Dict[str, Any] = {
                "patient_id": str(patient_id),
                "label": int(labels[index]),
                "visit_count_audit_only": int(visit_mask[index].sum()),
                "evidence_valid_count": int(arrays["evidence_valid"][index].sum()),
                "bio_valid_count": int(
                    (arrays["latest_bio_valid"][index] | arrays["history_bio_valid"][index]).sum()
                ),
                ...
                "evidence_attention_entropy": attention_entropy,
            }
            for evidence_index, evidence_name in enumerate(EVIDENCE_NAMES):
                row[f"evidence_norm_{evidence_name}"] = float(
                    np.linalg.norm(arrays["evidence_tokens"][index, evidence_index])
                )
                row[f"evidence_valid_{evidence_name}"] = bool(
                    arrays["evidence_valid"][index, evidence_index]
                )
...
def train_seed(
    ...
    if not names or any(not name.startswith(HEAD_PREFIXES) for name in names):
        raise RuntimeError(f"C40 trainable scope violation: {names}")
    if any(parameter.requires_grad for name, parameter in model.named_parameters() if name.startswith("sources.")):
        raise RuntimeError("C40 image/text sources must remain frozen")
    count = trainable_parameter_count(model)
    if count > int(config["c40"]["trainable_parameter_limit"]):
        raise RuntimeError(f"C40 capacity contract failed: {count}")
    initial_state = {
    ...
    if best_state is None:
        raise RuntimeError(f"C40 seed {seed} produced no checkpoint")
    model.load_state_dict(best_state, strict=True)
    for row in epoch_rows:
        row["selected_by_val_auc"] = int(row["epoch"]) == best_epoch
    val_result = run_epoch(model, loaders["val"], None, device)
    if val_result["metrics"]["prediction_std"] <= 0.0:
        raise RuntimeError(f"C40 seed {seed} produced constant validation predictions")
...
def validation_seed_stage(
    ...
    if seed_dir.exists():
        raise RuntimeError(f"C40 seed output already exists: {seed_dir}")
    ...
    status_path.write_text(json.dumps(status, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"status": "C40_VALIDATION_SEED_COMPLETE", "seed": seed}))
...
def validation_finalize_stage(config: Dict[str, Any], out_dir: Path, device: torch.device) -> None:
    ...
    for seed in SEEDS:
        ...
        if status.get("status") != "COMPLETE":
            raise RuntimeError(f"C40 seed {seed} shard incomplete")
    ...
    print(json.dumps({"status": "C40_VALIDATION_COMPLETE", "seeds": list(SEEDS)}))


def reporting_test_stage(config: Dict[str, Any], rows: Sequence[Dict[str, Any]], out_dir: Path, device: torch.device) -> None:
    decision_path = resolve_path(config["project"]["report_dir"]) / "c40_validation_decision.json"
    if not decision_path.exists():
        raise RuntimeError("C40 validation decision must be frozen before reporting-only test")
    ...
    if (
        ...
        raise RuntimeError("C40 validation/test isolation contract failed")
    metrics_path = out_dir / "reports" / "metrics_by_seed.csv"
    metrics = pd.read_csv(metrics_path)
    if set(metrics["split"]) != {"val"}:
        raise RuntimeError("C40 reporting-only test requires validation-only metrics")
    loader = build_loaders(config, rows, ("test",))["test"]
    for seed in SEEDS:
        model = C40BCEQModel(config, seed).to(device)
        payload = checkpoint_payload(out_dir / "checkpoints" / f"seed_{seed}_best.pt")
        if int(payload.get("seed", -1)) != seed:
            raise RuntimeError(f"C40 checkpoint seed mismatch for {seed}")
    ...
    status_path.write_text(json.dumps(status, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"status": "C40_REPORTING_TEST_COMPLETE", "seeds": list(SEEDS)}))
...
def direct_multiseed_stage(config_path: Path, config: Dict[str, Any], rows: Sequence[Dict[str, Any]], out_dir: Path, device: torch.device) -> None:
    gate_path = resolve_path(config["project"]["report_dir"]) / "c40_gate.json"
    if not gate_path.exists():
        raise RuntimeError("C40 direct execution requires the completed gate")
    gate = json.loads(gate_path.read_text(encoding="utf-8"))
    if gate.get("status") != "C40_BCEQ_DIRECT_MULTI_SEED_AUTHORIZED" or int(gate.get("passed", 0)) != int(gate.get("total", 0)):
        raise RuntimeError("C40 direct execution requires an authorized gate")
    if (out_dir / "seed_runs").exists():
        raise RuntimeError("C40 formal seed outputs already exist")
    for child in ("reports", "predictions", "checkpoints", "representations"):
    ...
    if any(code != 0 for code in codes):
        raise RuntimeError(f"C40 validation shard failed: {codes}")
    subprocess.run([sys.executable, str(script), "--config", str(config_path), "--stage", "validation-finalize"], check=True)
    collector = REPO_ROOT / "scripts" / "collect_phase_c40_report.py"
    ...
    print(json.dumps({"status": "C40_DIRECT_MULTI_SEED_COMPLETE", "seeds": list(SEEDS)}))
...
def main() -> None:
    ...
    if str(config.get("phase", "")).lower() != "c40":
        raise RuntimeError("C40 phase contract is missing")
    if [int(seed) for seed in config["training"]["seeds"]] != list(SEEDS):
        raise RuntimeError("C40 formal seeds must remain [0, 42, 3407]")
    ...
    if args.stage == "validation-seed":
        if args.seed not in SEEDS:
            raise RuntimeError(f"Unsupported C40 seed: {args.seed}")

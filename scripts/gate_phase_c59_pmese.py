#!/usr/bin/env python3
"""Authorize C59-PMESE direct formal execution with a strict contract gate."""
...
from dmea_ht.c59_pmese import C59PMESEModel, HEAD_PREFIXES, trainable_parameter_count  # noqa: E402
...
def main() -> None:
    ...
    source_text = inspect.getsource(C59PMESEModel)
    ...
    scope_pass &= bool(trainable_counts) and max(trainable_counts.values()) <= int(config["c59"]["trainable_parameter_limit"])
    train_source = (REPO_ROOT / "scripts" / "train_phase_c59.py").read_text(encoding="utf-8")
    ...
    ) and bool(config["c59"]["shortcut_fields_used_as_inputs"] is False)
    route_pass = (
        all(
            term in source_text
            for term in (
        ...
        and list(config["c59"]["stream_order"]) == ["raw_image", "raw_text", "raw_bio", "aligned_image", "aligned_text", "aligned_bio"]
        and int(config["c59"]["instance_dim"]) == 128
        ...
        and config["c59"]["learned_visit_score"] is False
        and config["c59"]["temporal_attention"] is False
        and config["c59"]["router"] is False
        and config["c59"]["visit_selector"] is False
    )
    test_blocked = (
        "C59 Validation decision must be frozen before reporting-only Test" in train_source
        and 'set(metrics["split"]) != {"val"}' in train_source
        and "c59_validation_decision.json" in train_source
    )
    direct_contract = (
    ...
    checks = [
        ...
        ("06_multimodal_instance_gradients_and_frozen_source", gradient_pass),
        ("07_shortcut_fields_excluded", shortcut_pass),
        ("08_patient_level_multimodal_set_contract", route_pass),
    ...
    pd.DataFrame(gradient_rows).to_csv(report_dir / "c59_gradient_audit.csv", index=False)
    passed = sum(bool(value) for _, value in checks)
    status = "C59_PMESE_DIRECT_MULTI_SEED_AUTHORIZED" if passed == len(checks) else "C59_PATH_GATE_FAIL"
    payload = {
        "phase": "C59-PMESE",
        "status": status,
        "passed": passed,
    ...
    (report_dir / "c59_gate.json").write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"status": status, "passed": passed, "total": len(checks)}))
    if status != "C59_PMESE_DIRECT_MULTI_SEED_AUTHORIZED":
        raise SystemExit(2)

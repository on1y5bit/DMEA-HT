#!/usr/bin/env python3
"""Authorize C38-MPES direct formal execution with a compact contract gate."""
...
from dmea_ht.c38_mpes import C38MPESModel, HEAD_PREFIXES, trainable_parameter_count  # noqa: E402
...
from scripts.train_phase_c38 import SEEDS, build_loaders, move_batch, set_seed  # noqa: E402
...
def main() -> None:
    ...
    source_text = inspect.getsource(C38MPESModel)
    for seed in SEEDS:
        set_seed(seed)
    ...
    trainable_scope_pass = bool(trainable_counts) and max(trainable_counts.values()) <= int(config["c38"]["trainable_parameter_limit"])
    config_text = config_path.read_text(encoding="utf-8")
    train_source = (REPO_ROOT / "scripts" / "train_phase_c38.py").read_text(encoding="utf-8")
    collector_source = (REPO_ROOT / "scripts" / "collect_phase_c38_report.py").read_text(encoding="utf-8")
    ...
    ) and bool(config["c38"]["shortcut_fields_used_as_inputs"] is False)
    static_contract_pass = (
        "C27" not in source_text
        and "C37" not in source_text
        and "learned visit score" not in source_text.lower()
    ...
    test_blocked = (
        ...
        and "c38_validation_decision.json" in train_source
    )
    direct_contract = (
    ...
    for seed in SEEDS:
        ...
        row_ok = bool(
            ...
            and any(str(key).startswith("mechanism_evidence_alignment.") for key in state)
        )
    ...
    checks = [
        ...
        ("08_new_patient_set_architecture", static_contract_pass and bool(config["c38"]["latest_history_set_partition"]) and config["c38"]["learned_visit_score"] is False),
    ...
    pd.DataFrame(gradient_rows).to_csv(report_dir / "c38_gradient_audit.csv", index=False)
    pd.DataFrame(checkpoint_rows).to_csv(report_dir / "c38_c17_checkpoint_audit.csv", index=False)
    passed = sum(bool(value) for _, value in checks)
    status = "C38_MPES_DIRECT_MULTI_SEED_AUTHORIZED" if passed == len(checks) else "C38_PATH_GATE_FAIL"
    payload = {
        "phase": "C38-MPES",
        "status": status,
        "passed": passed,
    ...
    (report_dir / "c38_gate.json").write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"status": status, "passed": passed, "total": len(checks)}))
    if status != "C38_MPES_DIRECT_MULTI_SEED_AUTHORIZED":
        raise SystemExit(2)

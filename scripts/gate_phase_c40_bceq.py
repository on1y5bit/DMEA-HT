#!/usr/bin/env python3
"""Authorize C40-BCEQ direct formal execution with a compact contract gate."""
...
from dmea_ht.c40_bceq import C40BCEQModel, HEAD_PREFIXES, trainable_parameter_count  # noqa: E402
...
def checkpoint_payload(path: Path) -> Mapping[str, Any]:
    ...
    if not isinstance(payload, Mapping):
        raise RuntimeError(f"Invalid C40 checkpoint payload: {path}")
    return payload
...
def split_contract(rows: List[Dict[str, Any]]) -> bool:
    ...
    for split, (count, positives, negatives) in expected.items():
        ...
        if len(selected) != count or len(ids) != count or int((labels == 1).sum()) != positives or int((labels == 0).sum()) != negatives:
            return False
        ids_by_split[split] = ids
    return not (ids_by_split["train"] & ids_by_split["val"] or ids_by_split["train"] & ids_by_split["test"] or ids_by_split["val"] & ids_by_split["test"])
...
def main() -> None:
    ...
    source_text = inspect.getsource(C40BCEQModel)
    for seed in SEEDS:
        set_seed(seed)
        model = C40BCEQModel(config, seed).to(device)
        trainable_counts[str(seed)] = trainable_parameter_count(model)
        frozen_scope_pass &= all(name.startswith(HEAD_PREFIXES) for name, parameter in model.named_parameters() if parameter.requires_grad)
        frozen_scope_pass &= not any(parameter.requires_grad for name, parameter in model.named_parameters() if name.startswith("sources."))
        model.train(True)
        for probe in [train_batch, *missing_batches(train_batch)]:
            ...
            gradient_rows.append({"seed": seed, "finite_head_gradient_tensors": finite_gradient_count, "nonzero_head_gradient_tensors": nonzero_gradient_count, "source_gradient_tensors": source_gradient_count})
            gradient_pass &= nonzero_gradient_count >= 3
        del model
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
    trainable_scope_pass = bool(trainable_counts) and max(trainable_counts.values()) <= int(config["c40"]["trainable_parameter_limit"])
    config_text = config_path.read_text(encoding="utf-8")
    train_source = (REPO_ROOT / "scripts" / "train_phase_c40.py").read_text(encoding="utf-8")
    collector_source = (REPO_ROOT / "scripts" / "collect_phase_c40_report.py").read_text(encoding="utf-8")
    disabled_metric = "AUP" + "RC"
    shortcut_pass = not any(field in source_text for field in ("patient_id", "selected_n_visits", "raw_n_visits", "used_images", "raw_n_images", "image_padding_count", "report_length", "source_folder")) and bool(config["c40"]["shortcut_fields_used_as_inputs"] is False)
    static_contract_pass = (
        "C27" not in source_text
        and "C37" not in source_text
        and "C38" not in source_text
        and "C39" not in source_text
        and "visit_score" not in source_text
        and "temporal_linear" not in source_text
        and "log2" in source_text
        and "bio_query" in source_text
        and "evidence_attention" in source_text
    )
    test_blocked = "validation decision must be frozen before reporting-only test" in train_source and "set(metrics[\"split\"]) != {\"val\"}" in train_source and "c40_validation_decision.json" in train_source
    ...
    loss_contract = bool(config["loss"]["bce_only"]) and train_source.count("binary_cross_entropy_with_logits") == 1 and "scheduler" not in train_source.lower() and disabled_metric not in train_source and disabled_metric not in collector_source and disabled_metric not in config_text
    ...
    for seed in SEEDS:
        ...
        row_ok = bool(path.exists() and int(payload.get("seed", -1)) == seed and any(str(key).startswith("base_model.image_encoder.") for key in state) and any(str(key).startswith("base_model.text_encoder.") for key in state) and any(str(key).startswith("mechanism_evidence_alignment.image.") for key in state) and any(str(key).startswith("mechanism_evidence_alignment.text.") for key in state))
    ...
    checks = [
        ...
        ("08_continuous_bio_query_evidence_memory", static_contract_pass and config["c40"]["fixed_recency_kernel"] == "inverse_log2_age" and config["c40"]["learned_visit_score"] is False),
    ...
    pd.DataFrame(gradient_rows).to_csv(report_dir / "c40_gradient_audit.csv", index=False)
    pd.DataFrame(checkpoint_rows).to_csv(report_dir / "c40_c17_checkpoint_audit.csv", index=False)
    passed = sum(bool(value) for _, value in checks)
    status = "C40_BCEQ_DIRECT_MULTI_SEED_AUTHORIZED" if passed == len(checks) else "C40_PATH_GATE_FAIL"
    payload = {"phase": "C40-BCEQ", "status": status, "passed": passed, "total": len(checks), "git_commit": git_output("rev-parse", "HEAD"), "branch": branch, "device": str(device), "gpu": torch.cuda.get_device_name(0) if torch.cuda.is_available() else "cpu", "trainable_parameter_count_by_seed": trainable_counts, "checks": [{"name": name, "passed": bool(value)} for name, value in checks]}
    (report_dir / "c40_gate.json").write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"status": status, "passed": passed, "total": len(checks)}))
    if status != "C40_BCEQ_DIRECT_MULTI_SEED_AUTHORIZED":
        raise SystemExit(2)

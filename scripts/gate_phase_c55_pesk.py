#!/usr/bin/env python3
"""Authorize C55-PESK direct formal execution with a strict contract gate."""
...
from dmea_ht.c55_pesk import C55PESKModel, HEAD_PREFIXES, trainable_parameter_count  # noqa: E402
...
def parse_args() -> Any:
    ...
    parser.add_argument("--config", default="configs/dema_ht_c55_pesk_multiseed.yaml")
...
def main() -> None:
    ...
    source_text = inspect.getsource(C55PESKModel)
    inherited_text = inspect.getsource(C47DRFEModel)
    route_text = source_text + inherited_text
    for seed in SEEDS:
        set_seed(seed)
        model = C55PESKModel(config, seed).to(device)
    ...
    scope_pass &= bool(trainable_counts) and max(trainable_counts.values()) <= int(config["c55"]["trainable_parameter_limit"])
    train_source = (REPO_ROOT / "scripts" / "train_phase_c55.py").read_text(encoding="utf-8")
    ...
    ) and bool(config["c55"]["shortcut_fields_used_as_inputs"] is False)
    route_pass = (
        all(term in source_text for term in ("C55PESKModel", "_fixed_trajectory_statistics", "interaction_states", "disagreement_states", "kernel_readout", "F.layer_norm"))
        and all(term in inherited_text for term in ("latest_mask", "history_mask", "variance", "log2", "raw_image", "aligned_image"))
        and list(config["c55"]["stream_order"]) == ["raw_image", "raw_text", "raw_bio", "aligned_image", "aligned_text", "aligned_bio"]
        and [tuple(pair) for pair in config["c55"]["interaction_pairs"]] == [(0, 1), (0, 2), (1, 2)]
        ...
        and config["c55"]["learned_visit_score"] is False
        and config["c55"]["temporal_attention"] is False
        and config["c55"]["router"] is False
    )
    test_blocked = (
        "C55 Validation decision must be frozen before reporting-only Test" in train_source
        and 'set(metrics["split"]) != {"val"}' in train_source
        and "c55_validation_decision.json" in train_source
    )
    direct_contract = (
    ...
    checks = [
        ...
        ("06_fixed_kernel_gradients_and_frozen_source", gradient_pass),
        ("07_shortcut_fields_excluded", shortcut_pass),
        ("08_patient_evidence_set_kernel_contract", route_pass),
    ...
    pd.DataFrame(gradient_rows).to_csv(report_dir / "c55_gradient_audit.csv", index=False)
    passed = sum(bool(value) for _, value in checks)
    status = "C55_PESK_DIRECT_MULTI_SEED_AUTHORIZED" if passed == len(checks) else "C55_PATH_GATE_FAIL"
    payload = {
        "phase": "C55-PESK",
        "status": status,
        "passed": passed,
    ...
    (report_dir / "c55_gate.json").write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"status": status, "passed": passed, "total": len(checks)}))
    if status != "C55_PESK_DIRECT_MULTI_SEED_AUTHORIZED":
        raise SystemExit(2)

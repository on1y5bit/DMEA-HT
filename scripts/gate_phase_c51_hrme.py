#!/usr/bin/env python3
"""Authorize C51-HRME direct formal execution with a strict contract gate."""
...
from dmea_ht.c51_hrme import C51HRMEModel, HEAD_PREFIXES, trainable_parameter_count  # noqa: E402
...
def main() -> None:
    ...
    source_text = inspect.getsource(C51HRMEModel)
    ...
    scope_pass &= bool(trainable_counts) and max(trainable_counts.values()) <= int(config["c51"]["trainable_parameter_limit"])
    train_source = (REPO_ROOT / "scripts" / "train_phase_c51.py").read_text(encoding="utf-8")
    base_train_source = (REPO_ROOT / "scripts" / "train_phase_c40.py").read_text(encoding="utf-8")
    collector_source = (REPO_ROOT / "scripts" / "collect_phase_c51_report.py").read_text(encoding="utf-8")
    ...
    ) and bool(config["c51"]["shortcut_fields_used_as_inputs"] is False)
    route_pass = (
        all(
            term in source_text
            for term in (
                "distribution_encoder",
                "_distributional_summary",
                "nanquantile",
                "history_q50",
                "history_iqr",
                "set_mean",
        ...
        and list(config["c51"]["stream_order"]) == ["raw_image", "raw_text", "raw_bio", "aligned_image", "aligned_text", "aligned_bio"]
        ...
        and config["c51"]["learned_visit_score"] is False
        and config["c51"]["temporal_attention"] is False
        and config["c51"]["router"] is False
    )
    test_blocked = (
        "C51 Validation decision must be frozen before reporting-only Test" in train_source
        and 'set(metrics["split"]) != {"val"}' in train_source
        and "c51_validation_decision.json" in train_source
    )
    direct_contract = (
    ...
    checks = [
        ...
        ("06_distribution_gradients_and_frozen_source", gradient_pass),
        ("07_shortcut_fields_excluded", shortcut_pass),
        ("08_distributional_patient_set_contract", route_pass),
    ...
    pd.DataFrame(gradient_rows).to_csv(report_dir / "c51_gradient_audit.csv", index=False)
    passed = sum(bool(value) for _, value in checks)
    status = "C51_HRME_DIRECT_MULTI_SEED_AUTHORIZED" if passed == len(checks) else "C51_PATH_GATE_FAIL"
    payload = {
        "phase": "C51-HRME",
        "status": status,
        "passed": passed,
    ...
    (report_dir / "c51_gate.json").write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"status": status, "passed": passed, "total": len(checks)}))
    if status != "C51_HRME_DIRECT_MULTI_SEED_AUTHORIZED":
        raise SystemExit(2)

#!/usr/bin/env python3
"""Freeze and report C60-EMASE Validation and reporting-only Test evidence."""
...
def transform(value: Any) -> Any:
    ...
    if isinstance(value, str):
        return value.replace("C54", "C60").replace("c54", "c60").replace("LRRA", "EMASE")
    return value


def materialize_c60(report_dir: Path) -> None:
    for source_name in REPORT_FILES:
        source = report_dir / source_name
        if not source.exists():
            continue
        target_name = source_name.replace("c54", "c60").replace("C54", "C60")
...
def main() -> None:
    ...
    if str(config.get("phase", "")).lower() != "c60":
        raise RuntimeError("C60 report requires the formal C60 config")
    ...
    if args.stage == "validation":
        ...
        materialize_c60(report_dir)
        print(json.dumps({"status": "C60_VALIDATION_DECISION_FROZEN", "decision": transform(decision["decision_label"])}))
    else:
        decision = shared.write_final_report(translated, run_dir, report_dir)
        materialize_c60(report_dir)
        print(json.dumps({"status": "C60_FINAL_REPORT_COMPLETE", "decision": transform(decision["decision_label"])}))

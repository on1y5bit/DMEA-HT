#!/usr/bin/env python3
"""Authorize C46-MCFS direct formal execution with a strict contract gate."""
...
from dmea_ht.c46_mcfs import C46MCFSModel, HEAD_PREFIXES, trainable_parameter_count  # noqa: E402
...
def main() -> None:
    ...
    source_text = inspect.getsource(C46MCFSModel)
    for seed in SEEDS:
        set_seed(seed)
    ...
    scope_pass &= bool(trainable_counts) and max(trainable_counts.values()) <= int(config["c46"]["trainable_parameter_limit"])
    train_source = (REPO_ROOT / "scripts" / "train_phase_c46.py").read_text(encoding="utf-8")
    base_train_source = (REPO_ROOT / "scripts" / "train_phase_c40.py").read_text(encoding="utf-8")
    collector_source = (REPO_ROOT / "scripts" / "collect_phase_c46_report.py").read_text(encoding="utf-8")
    ...
    ) and bool(config["c46"]["shortcut_fields_used_as_inputs"] is False)
    route_pass = (
        all(
            term in source_text
            for term in (
        ...
        and config["c46"]["learned_visit_score"] is False
        and config["c46"]["temporal_attention"] is False
        and config["c46"]["router"] is False
    )
    test_blocked = (
        "C46 Validation decision must be frozen before reporting-only Test" in train_source
        and 'set(metrics["split"]) != {"val"}' in train_source
        and "c46_validation_decision.json" in train_source
    )
    direct_contract = (
    ...
    checks = [
        ...
        ("06_shared_mechanism_gradients_and_frozen_source", gradient_pass),
        ("07_shortcut_fields_excluded", shortcut_pass),
        ("08_fixed_mechanism_slot_contract", route_pass),
    ...
    pd.DataFrame(gradient_rows).to_csv(report_dir / "c46_gradient_audit.csv", index=False)
    passed = sum(bool(value) for _, value in checks)
    status = "C46_MCFS_DIRECT_MULTI_SEED_AUTHORIZED" if passed == len(checks) else "C46_PATH_GATE_FAIL"
    payload = {
        "phase": "C46-MCFS",
        "status": status,
        "passed": passed,
    ...
    (report_dir / "c46_gate.json").write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"status": status, "passed": passed, "total": len(checks)}))
    if status != "C46_MCFS_DIRECT_MULTI_SEED_AUTHORIZED":
        raise SystemExit(2)

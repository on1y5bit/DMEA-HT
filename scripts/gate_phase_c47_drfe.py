#!/usr/bin/env python3
"""Authorize C47-DRFE direct formal execution with a strict contract gate."""
...
from dmea_ht.c47_drfe import C47DRFEModel, HEAD_PREFIXES, trainable_parameter_count  # noqa: E402
...
def main() -> None:
    ...
    source_text = inspect.getsource(C47DRFEModel)
    for seed in SEEDS:
        set_seed(seed)
    ...
    scope_pass &= bool(trainable_counts) and max(trainable_counts.values()) <= int(config["c47"]["trainable_parameter_limit"])
    train_source = (REPO_ROOT / "scripts" / "train_phase_c47.py").read_text(encoding="utf-8")
    base_train_source = (REPO_ROOT / "scripts" / "train_phase_c40.py").read_text(encoding="utf-8")
    collector_source = (REPO_ROOT / "scripts" / "collect_phase_c47_report.py").read_text(encoding="utf-8")
    ...
    ) and bool(config["c47"]["shortcut_fields_used_as_inputs"] is False)
    route_pass = (
        all(
            term in source_text
            for term in (
        ...
        and "temporal" not in source_text.lower()
        and "router" not in source_text.lower()
        and all(term not in source_text for term in ("C27", "C37", "C38", "C39", "C40", "C42", "C43", "C44", "MultiheadAttention"))
        and config["c47"]["learned_visit_score"] is False
        and config["c47"]["temporal_attention"] is False
        and config["c47"]["router"] is False
    )
    test_blocked = (
        "C47 Validation decision must be frozen before reporting-only Test" in train_source
        and 'set(metrics["split"]) != {"val"}' in train_source
        and "c47_validation_decision.json" in train_source
    )
    direct_contract = (
    ...
    checks = [
        ...
        ("06_shared_stream_gradients_and_frozen_source", gradient_pass),
        ("07_shortcut_fields_excluded", shortcut_pass),
        ("08_dual_resolution_fixed_stream_contract", route_pass),
    ...
    pd.DataFrame(gradient_rows).to_csv(report_dir / "c47_gradient_audit.csv", index=False)
    passed = sum(bool(value) for _, value in checks)
    status = "C47_DRFE_DIRECT_MULTI_SEED_AUTHORIZED" if passed == len(checks) else "C47_PATH_GATE_FAIL"
    payload = {
        "phase": "C47-DRFE",
        "status": status,
        "passed": passed,
    ...
    (report_dir / "c47_gate.json").write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"status": status, "passed": passed, "total": len(checks)}))
    if status != "C47_DRFE_DIRECT_MULTI_SEED_AUTHORIZED":
        raise SystemExit(2)

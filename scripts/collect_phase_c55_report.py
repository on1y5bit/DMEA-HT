#!/usr/bin/env python3
"""Freeze and report C55-PESK Validation and reporting-only Test evidence."""
...
def transform(value: Any) -> Any:
    ...
    if isinstance(value, str):
        return value.replace("C54", "C55").replace("c54", "c55").replace("LRRA", "PESK")
    return value


def materialize_c55(report_dir: Path) -> None:
    for source_name in REPORT_FILES:
        source = report_dir / source_name
        if not source.exists():
            continue
        target_name = source_name.replace("c54", "c55").replace("C54", "C55")
...
def main() -> None:
    ...
    if str(config.get("phase", "")).lower() != "c55":
        raise RuntimeError("C55 report requires the formal C55 config")
    ...
    if args.stage == "validation":
        ...
        materialize_c55(report_dir)
        print(json.dumps({"status": "C55_VALIDATION_DECISION_FROZEN", "decision": transform(decision["decision_label"])}))
    else:
        decision = shared.write_final_report(translated, run_dir, report_dir)
        materialize_c55(report_dir)
        print(json.dumps({"status": "C55_FINAL_REPORT_COMPLETE", "decision": transform(decision["decision_label"])}))

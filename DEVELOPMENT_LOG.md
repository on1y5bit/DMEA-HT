...
### Gate, Formal Run, and Final Decision

- The first server gate correctly blocked at `17/18` because the zero-variance branch produced `NaN` gradients: `sqrt(variance)` at zero was followed by a zero-gradient `where` branch. The forward contract was unchanged; clamping the square-root input to `1e-12` stabilized the backward pass. The corrected gate passed `18/18` at commit `22593e6` on CUDA RTX 5090.
- The authorized gate reported `259,266` trainable parameters and `38,742,912` frozen parameters. All three encoders were frozen, single-visit and multi-visit trajectories were observed, the five source state outputs were finite and in range, and no audit-only shortcut field entered the model.
- Formal parallel Validation started at `2026-07-15T19:05:42+08:00` and all seeds completed normally. Validation-selected epochs were seed `0: 30`, seed `42: 13`, and seed `3407: 1`.
- C34 Validation AUC was seed `0: 0.7496604799`, seed `42: 0.7564508828`, and seed `3407: 0.6889995473`, mean/std `0.7317036366 +/- 0.0371383471`. The mean change was `-0.1379206277` versus C17 and `-0.1505960465` versus C27; mean Validation AUC `0.90` was not reached.
- Source-state means for image/text-support/text-opposition/bio-immune/bio-function were respectively `0.545967/0.331864/0.012402/0.000708/0.851968` for seed `0`, `0.643164/0.393461/0.657461/0.617647/0.654899` for seed `42`, and `0.372712/0.073106/0.260453/0.591194/0.877933` for seed `3407`. The source and trajectory state health gate passed; no all-constant state collapse was observed.
- The selected-epoch training health audit passed `30/33` rows. The three failing rows were the `empty_visit_state` gradient rows for seeds `0/42/3407`: the formal manifest contained no real visit with zero available evidence, so this fallback branch received no observed training gradient. The implementation gate had already verified the branch with deterministic BCE probes, but the formal selected-checkpoint health gate remains invalid under the strict contract.
- Positive preservation failed: aggregate C17 TP-to-C34-FN versus FN-to-C34-TP was `53/12`, with per-seed transitions `14/6`, `2/6`, and `37/0`. Seed `3407` sensitivity dropped by `-0.7872340426`; C34 material positive damage was `29/33/37` versus C27 `12/21/9`.
- Pairwise ranking failed: C34 inversions were `553/538/687` versus C27 `217/284/279`; C27-to-C34 repaired/introduced pairs were `390/1388`, with per-seed inversion increases `336/254/408`.
- Shortcut safety passed all seeds. Selected-structure shortcut-only label AUC was `0.2833861476`; maximum absolute prediction/selected-structure Spearman was `0.2357695527`. Raw visit/image association warnings remained audit-only.
...
### Implementation, Gate, and Formal Result

- C35 implementation was committed and pushed at `4d8ca3a`; the canonical server fast-forwarded from `b853925` to `4d8ca3a`. The server gate ran in `/home/linruixin/chen/conda/envs/ma` on the RTX 5090 and passed exactly `18/18` with `C35_MTSA_DIRECT_MULTI_SEED_AUTHORIZED`.
- The gate reported `184,845` trainable parameters and `41,724,291` frozen parameters. It observed both single-visit and multi-visit patients, verified `[B,V,5,32]` mechanism visit states, `[B,5,96]` fixed trajectories, `[B,5]` patient coordinates, finite missing-evidence fallbacks, symmetric anchors with distance `2.0`, and finite nonzero gradients for the real activated mechanism projectors, coordinate heads, and anchor direction.
- Formal Validation started at `2026-07-15T20:50:49+08:00` and finished at `20:54:38+08:00` with seeds `[0,42,3407]` running in parallel on the NVIDIA RTX 5090. There was no smoke, pilot, variant, sweep, ensemble, checkpoint/prediction averaging, secondary metric, calibration, threshold tuning, or Test-based selection.
- Validation-selected epochs were seed `0: 11`, seed `42: 8`, and seed `3407: 9`. Validation AUC was `0.8243549117 / 0.8320507017 / 0.8320507017`, mean/std `0.8294854384 +/- 0.0044431664`. C35-C27 was `-0.0774105930 / -0.0393843368 / -0.0416478044`, mean `-0.0528142448`; C35-C17 was `-0.0457220462 / -0.0448166591 / -0.0298777727`, mean `-0.0401388260`. The mean `0.90` target was not reached.
- Coordinate health failed because M5 temporal-text coordinates were constant at seed `42` (`std = 0.0`); seeds `0` and `3407` were also numerically near-constant (`8.36e-09` and `4.62e-09`). M1-M4 remained varying, anchor health passed, training health passed, and the aggregate state/training audit had `35/36` rows passing.
- Positive preservation failed: aggregate C17 TP-to-C35-FN versus FN-to-C35-TP was `29/12`; C35 material positive damage was `78` versus C27 `42`. Sensitivity change versus C17 was `+0.0425531915 / -0.2553191489 / -0.1489361702` for seeds `0/42/3407`.
- Ranking safety failed: C35 inversions were `388/371/371` versus C27 `217/284/279`; aggregate C27-to-C35 repaired/introduced pairs were `403/753`.
- Shortcut safety passed all seeds. Selected-structure shortcut-only label AUC was `0.2833861476`; maximum absolute prediction/selected-structure Spearman was `0.2550160468`.
...
### Implementation, Gate, Formal Run, and Final Decision

- C36 implementation was committed and pushed at `c0cee67`; the canonical server fast-forwarded from `7655bd9` to `c0cee67`. The server used `/home/linruixin/chen/conda/envs/ma` on the NVIDIA RTX 5090 and the authoritative data root `/data/csb/DMEA-HT/HT_2025.12_25`.
- The C36 gate passed exactly `20/20` with `C36_JTSA_DIRECT_MULTI_SEED_AUTHORIZED`. It reported `222,816` trainable parameters and `41,724,291` frozen parameters. It verified the frozen C17 encoder/projector prefixes, the five mechanisms, `[B,V,5,32]` visit states, `[B,5,96]` trajectories, `[B,5,32]` mechanism embeddings, `[B,32]` patient state, symmetric anchors with radius `1.0` and distance `2.0`, missing-evidence fallback finiteness, finite nonzero activated gradients, BCE-only training, and Validation/Test isolation.
- Formal Validation started at `2026-07-15T21:40:36+08:00` and finished at `21:46:21+08:00`; seeds `[0,42,3407]` ran as independent parallel single-model replicates on the RTX 5090. Reporting-only Test finished at `21:46:27+08:00`. No smoke, pilot, variant, sweep, ensemble, averaging, secondary metric, AUPRC, calibration, threshold tuning, or Test-based selection was used.
- Validation-selected epochs were seed `0: 21`, seed `42: 15`, and seed `3407: 18`. Validation AUC was `0.8356722499 / 0.8175645088 / 0.8234495247`, mean/std `0.8255620945 +/- 0.0092368709`. C36-C17 AUC was `-0.0344047080 / -0.0593028520 / -0.0384789498`, mean `-0.0440621699`; C36-C27 was `-0.0660932549 / -0.0538705297 / -0.0502489814`, mean `-0.0567375887`; C36-C35 was `+0.0113173382 / -0.0144861928 / -0.0086011770`, mean `-0.0039233439`. The mean `0.90` target was not reached.
- Patient-state and mechanism health passed `36/36` rows. Patient state variation, all five mechanism embedding variations, symmetric anchor geometry, finite nonzero selected-epoch gradients, and parameter drift all passed. The validation artifacts contain `1,410` mechanism embedding rows and `282` patient state/anchor rows.
- Positive preservation failed: C17 TP-to-C36-FN versus FN-to-C36-TP transitions were seed `0: 25/0`, seed `42: 6/3`, and seed `3407: 5/7`, aggregate `36/10`. Sensitivity change versus C17 was `-0.5319148936 / -0.0638297872 / +0.0425531915`. C36 material positive damage was `35/32/26`, aggregate `93`, versus C27 `12/21/9`, aggregate `42`.
- Ranking safety failed: C36 inversions were `363/403/390` versus C27 `217/284/279`; C27-to-C36 repaired/introduced pairs were `121/267`, `131/250`, and `106/217`, aggregate `358/734`. Inversion increases versus C27 were `146/119/111`.
- Shortcut safety passed all seeds. Selected-structure shortcut-only label AUC was `0.2833861476`; maximum absolute prediction/selected-structure Spearman was `0.1796339449`. Raw visit/image association warnings remained audit-only.
- Validation decision was frozen before Test as `DEMA_C36_POSITIVE_DAMAGE`. The AUC gate also failed because the Validation mean was below C27, and the positive/ranking gates failed despite state and shortcut health passing. The strict best remains `KEEP_DEMA_C17_STRICT_BEST`; `STOP_DISEASE_STATE_ANCHOR_ROUTE` is binding and no C36 deployment checkpoint was selected.
- Reporting-only Test AUC was `0.7868480726 / 0.8112244898 / 0.8044217687`, mean/std `0.8008314437 +/- 0.0125785633`. Test did not alter the frozen decision, architecture, checkpoint, threshold, or deployment selection. Final server artifacts are under `runs/dema_ht_c36_jtsa_multiseed/` and `analysis_reports/phase_c36_dema/`, including `phase_c36_dema_final_report.md`, `c36_validation_decision.json`, the state/embedding/anchor health audits, positive-preservation audit, inversion summary, and shortcut audit.

## 2026-07-15 Phase C37-E2E-VRL End-to-End Visit Representation Learning

### Pre-Edit Contract

- C36-JTSA completed with mean Validation AUC `0.8255620945 +/- 0.0092368709`. Patient-state, mechanism-embedding, anchor, training, and shortcut health passed, but AUC, positive preservation, and pairwise ranking failed materially. The frozen decision is `DEMA_C36_POSITIVE_DAMAGE`.
- `KEEP_DEMA_C17_STRICT_BEST` and `STOP_DISEASE_STATE_ANCHOR_ROUTE` are binding. C34 source-state scalar, C35 mechanism-coordinate, C36 joint-anchor, C36-v2, larger-anchor, prototype, contrastive, and anchor-auxiliary routes are permanently closed.
- C37 tests the remaining full visit-level hypothesis: exact C27 visit reconstruction, mechanism definitions, temporal scorer, fixed recency prior, conflict formula, patient projection, and classifier are reused without adding architecture. Each seed initializes from its corresponding validation-selected C27 checkpoint.
- The only boundary change is end-to-end optimization. Image/text/bio encoders, C17 pre-propagation evidence projectors, and the complete C27 temporal/patient prediction path train jointly from epoch 1 with fixed learning-rate scales `0.01 / 0.05 / 0.10` relative to the C27 base rate. C27 behavior remains unchanged when the C37 switch is absent.
- Formal seeds remain `[0,42,3407]`, each an independent single-model replicate with Validation-AUC checkpoint selection. The gate must pass exactly `16/16`; after authorization, all three seeds start directly in parallel with no smoke or pilot.
- BCE with logits is the only loss. No new head, adapter, residual, anchor, prototype, graph, attention, RNN, Transformer, auxiliary branch, per-visit loss, ranking loss, distillation, ensemble, averaging, calibration, threshold tuning, AUPRC, or Test-based selection is allowed.
- Test remains reporting-only after the Validation decision is frozen. If C37 fails promotion, `KEEP_DEMA_C17_STRICT_BEST` and `STOP_VISIT_LEVEL_MODEL_ROUTE` become binding; no C37-v2, partial unfreezing, encoder subset, or learning-rate variant is authorized.
- Starting implementation commit: `8f5e9cd`. C37 implementation, gate, formal results, final decision, deployment status, and final commit will be recorded below after execution.

### Implementation, Gate, Formal Run, and Final Decision

- C37 implementation was committed and pushed at `64b522c`; the canonical server fast-forwarded to this commit on `main`. The run used `/home/linruixin/chen/conda/envs/ma` on the NVIDIA GeForce RTX 5090 and the authoritative data root `/data/csb/DMEA-HT/HT_2025.12_25`.
- The exact C37 gate passed `16/16` with `C37_E2E_VRL_DIRECT_MULTI_SEED_AUTHORIZED`. Initial C27 reproduction passed for all seeds: maximum absolute logit errors were `9.5367431641e-07`, `4.7683715820e-07`, and `4.7683715820e-07`; validation AUC, patient IDs, labels, and threshold classes matched the official C27 predictions. The gate verified finite nonzero gradients for all encoders, evidence projectors, and C27 prediction-path groups, finite missing-modality outputs, BCE-only training, validation/test isolation, and exact architecture reuse.
- C37 jointly trained all C17 encoders, all C17 pre-propagation evidence projectors, and the complete C27 temporal/patient/classifier path from each C27 validation-selected checkpoint. The run had `43,114,377` trainable parameters and no frozen parameters in the C37 model. Relative learning-rate scales were encoders `0.01` (`1e-6`), projectors `0.05` (`5e-6`), and prediction path `0.10` (`1e-5`) from the `1e-4` base rate; the recency prior remained fixed at `log(2)`.
- Formal Validation ran directly as independent parallel seeds `[0,42,3407]` from `2026-07-15T22:23:27+08:00` through `22:33:43+08:00`; selected epochs were seed `0: 1`, seed `42: 10`, and seed `3407: 14`. Validation AUC was `0.9026708918 / 0.8610230874 / 0.8655500226`, mean/std `0.8764146673 +/- 0.0228509361`. C37-C27 AUC differences were `+0.0009053871 / -0.0104119511 / -0.0081484835`, mean `-0.0058850158`; C37-C17 differences were `+0.0325939339 / -0.0158442734 / +0.0036215482`, mean `+0.0067904029`. The C27 promotion threshold was not reached.
- Training health passed all `33/33` rows, including nonconstant predictions and patient states, finite parameter drift, and finite nonzero selected-epoch gradients across all nine trainable parameter groups. The selected-structure shortcut audit passed all seeds: shortcut-only label AUC `0.2833861476`, maximum absolute prediction/selected-structure Spearman `0.1587408244`; audit-only raw visit/image association warnings were not model inputs.
- Positive preservation did not pass: aggregate C17 TP-to-C37-FN versus FN-to-C37-TP transitions were `16/16`, but sensitivity changes versus C17 were `+0.1063829787 / -0.1276595745 / +0.0212765957`, below the allowed `-0.05` floor for seed `42`. C37 material positive damage was `11/20/10`, aggregate `41`, versus C27 `12/21/9`, aggregate `42`.
- Pairwise ranking safety did not pass. C37 inversions were `215/307/297` versus C27 `217/284/279`; repaired/introduced pairs were `31/29`, `29/52`, and `44/62`, aggregate `104/143`, with a maximum inversion increase of `23`.
- Validation decision was frozen before Test as `DEMA_C37_POSITIVE_DAMAGE`. The AUC, positive-preservation, and ranking gates failed while training health, capacity, and shortcut safety passed. The strict best remains `KEEP_DEMA_C17_STRICT_BEST`; `STOP_VISIT_LEVEL_MODEL_ROUTE` is binding. No C37 deployment checkpoint was selected, and no C37-v2, partial-unfreezing, encoder-subset, or learning-rate variant is authorized.
- Reporting-only Test completed at `2026-07-15T22:33:53+08:00`. Test AUC was `0.8265306122 / 0.8679138322 / 0.8446712018`, mean/std `0.8463718821 +/- 0.0207439620`. Test did not alter the frozen decision, checkpoint, threshold, deployment seed, or model route. The canonical server is complete with `runs/dema_ht_c37_e2e_vrl_multiseed/` and `analysis_reports/phase_c37_dema/` artifacts; the final log was synchronized to the server by checksum-verified SCP after the final GitHub pull timed out, without reactivating the preserved server stash.

## 2026-07-16 Goal DEMA_HT_AUC_090_PLUS: Phase C38-MPES

### Pre-Edit Contract

- Goal `DEMA_HT_AUC_090_PLUS` is active. The current strict best remains C17 with mean Validation AUC `0.8696242644`; C27 remains the strongest unpromoted candidate at `0.8822996831`, with seed `0` at `0.9017655048` but seeds `42` and `3407` near `0.87`. C37 E2E optimization failed with mean `0.8764146673` and is closed by `DEMA_C37_POSITIVE_DAMAGE` and `STOP_VISIT_LEVEL_MODEL_ROUTE`.
- C38 tests a new patient-level multi-instance hypothesis: each historical visit is converted into five clinically defined mechanism evidence nodes using the C17 image/text/bio encoders and mechanism evidence projectors; the model then treats these nodes as a patient evidence set, with separate learned query pooling for the latest visit and the pre-latest history set, followed by mechanism-wise latest/history interaction and patient-level cross-mechanism readout. It does not modify or fine-tune C27's temporal scorer, does not use a visit-level adapter, and does not score visits with a learned temporal MLP.
- The C17 source encoders and complete C17 mechanism evidence alignment are loaded from each C17 validation-selected checkpoint and frozen. Only the new mechanism-conditioned set pooling, latest/history interaction, patient readout, and classifier are trainable. No C17 post-hoc residual, C27/C37 prediction path, disease-state anchor route, hidden-size-only change, or learning-rate variant is used.
- The latest/history partition is derived only from chronological manifest order to identify the final record; visit count, dates, patient ID, raw image/report counts, padding, source folders, or other shortcut fields are not passed as model features. BCE with logits is the only loss. Formal seeds remain `[0,42,3407]`, with one checkpoint, one model, one forward, no ensemble, no AUPRC, and Test reporting-only after Validation decision freeze.
- C38 promotion requires mean Validation AUC `>= 0.9000`, at least `2/3` seeds `>= 0.9000`, std `<= 0.025`, no seed sensitivity drop versus C17 greater than `0.10`, aggregate C17 TP-to-candidate-FN no more than aggregate FN-to-candidate-TP plus `3`, mean inversion increase versus C27 no more than `10%`, no per-seed inversion increase above `20`, and shortcut/leakage/split safety. If C38 fails, this is one distinct hypothesis toward the goal's three-hypothesis data-limit audit; no C38 micro-variants are authorized.
- Starting commit for C38 implementation: `e720e4c`. C38 implementation, gate, formal results, final decision, and next-hypothesis route will be recorded below after execution.

### C38 Formal Result And Decision

- C38 implementation was committed at `9638cdf`, with the latest broadcast and audit-report fixes at `0987b7b` and `4cb40c1`. The canonical server used `/home/linruixin/chen/conda/envs/ma` on the NVIDIA GeForce RTX 5090 and the authoritative manifest/data root. The initial gate caught one latest-visit mask broadcasting defect before training; a first post-gate launch caught only a missing diagnostic import and exited before a usable checkpoint. Both failed artifacts were preserved separately; the authoritative run began only after the corrected `11/11` gate.
- The corrected formal run used direct parallel seeds `[0, 42, 3407]`, one independent C38 model/checkpoint per seed, Validation-AUC checkpoint selection, and reporting-only Test after the Validation decision was frozen. Validation-selected epochs were `12`, `4`, and `13`.
- C38 Validation AUC was `0.8628338615 / 0.8583069262 / 0.8813942961`, mean/std `0.8675116946 +/- 0.0122338977`. C38-C17 was `-0.0072430964 / -0.0185604346 / +0.0194658216`, mean `-0.0021125698`; C38-C27 was `-0.0389316433 / -0.0131281123 / +0.0076957900`, mean `-0.0147879885`. The `0.90` target was not reached and no seed reached `0.90`.
- Training health passed `9/9`, all three seeds had nonconstant patient states and finite nonzero new-head gradients, positive preservation passed with aggregate C17 TP-to-C38-FN / FN-to-C38-TP `7/17`, and selected-structure shortcut-only label AUC was `0.2833861476` for every seed. Test was reporting-only with mean/std AUC `0.7970521542 +/- 0.0197682492`.
- Pairwise ranking safety failed. C27-to-C38 inversions were `217 -> 303`, `284 -> 313`, and `279 -> 262`; repaired/introduced counts were `120/206`, `92/121`, and `101/84`. Mean inversions increased from `260.0` to `292.7` (`+12.6%`), and seed `0` increased by `86` (`+39.6%`). The frozen decision is `C38_RANKING_DAMAGE`; strict best remains `KEEP_DEMA_C17_STRICT_BEST`, with no C38 deployment checkpoint.

## 2026-07-16 Goal DEMA_HT_AUC_090_PLUS: Phase C39-CMEQ

### Pre-Edit Contract

- C38-MPES was a healthy executable but did not improve C17 and introduced material C27 ranking damage. Its learned latest/history query pooling and cross-mechanism patient attention are therefore closed; no C38 micro-variant is authorized.
- C39 tests a new cross-modal clinical evidence organization hypothesis: frozen C17 pre-propagation image, text, and biochemical evidence projectors first form three patient-level continuous trajectories using a fixed ordinal-recency kernel. Each trajectory retains latest evidence, fixed historical evidence, and latest-history change without learning a visit score.
- The new patient model then builds explicit pairwise relation tokens for image-text, image-biochemistry, and text-biochemistry evidence. A low-capacity patient readout uses the three modality states and the three cross-modal relation states in fixed order. This tests whether clinically meaningful cross-modal agreement/disagreement is the missing stable signal, while avoiding C38's five-node set competition and avoiding C34-C36 disease-state scalar/joint anchoring.
- Only the frozen C17 image/text/bio encoders and their pre-propagation evidence projectors are loaded from the corresponding C17 validation-selected checkpoint. C17 propagation/readout, C27 temporal scorer, C37 end-to-end route, C38 modules, saved predictions, dates, patient IDs, visit counts, image/report counts, padding fields, and missingness shortcut fields are excluded from predictor inputs. Evidence validity is used only to mask unavailable source nodes, never as a classifier feature.
- The sole training loss is BCE with logits. Formal seeds remain `[0, 42, 3407]`; each seed has one independent model, optimizer, Validation-AUC-selected checkpoint, and one forward at deployment. No ensemble, averaging, AUPRC, calibration, threshold tuning, Test-based selection, smoke, pilot, or sweep is permitted. Test remains blocked until the Validation decision is frozen.
- C39 promotion uses the goal gates: mean Validation AUC `>= 0.9000`, at least `2/3` seeds `>= 0.9000`, std `<= 0.025`, positive safety versus C17, ranking safety versus C27, shortcut safety, finite/noncollapsed training health, and patient-level split/label/test isolation. If C39 fails, it remains one distinct whole-model hypothesis and the next phase must change the evidence organization again.
- Starting implementation commit for C39 is `4cb40c1`. The C39 implementation, minimal gate, formal run, final decision, and next route will be recorded below.

### C39 Formal Result And Decision

- C39 implementation was committed and pushed at `bf41eff`. The canonical server ran the corrected `11/11` gate and direct parallel seeds `[0, 42, 3407]` in `/home/linruixin/chen/conda/envs/ma` on the NVIDIA GeForce RTX 5090. Validation-selected epochs were `2`, `6`, and `16`; the run completed normally and Test was evaluated only after the frozen Validation decision.
- C39 Validation AUC was `0.8524219104 / 0.8718877320 / 0.8691715708`, mean/std `0.8644937377 +/- 0.0105423501`. C39-C17 was `-0.0176550475 / -0.0049796288 / +0.0072430964`, mean `-0.0051305266`; C39-C27 was `-0.0493435944 / +0.0004526935 / -0.0045269353`, mean `-0.0178059454`. The `0.90` target was not reached.
- Training health passed `9/9`, the selected-structure shortcut-only label AUC was `0.2833861476` for every seed, and Test remained reporting-only with mean/std AUC `0.8148148148 +/- 0.0185117686`.
- Positive preservation failed materially: C17 TP-to-C39-FN / FN-to-C39-TP was `3/8`, `23/0`, and `14/1`; aggregate `40/9`. Sensitivity changes versus C17 were `+0.1063829787`, `-0.4893617021`, and `-0.2765957447`, with seed `42` breaching the material-damage threshold. Ranking also failed: C27-to-C39 inversions were `217 -> 326`, `284 -> 283`, and `279 -> 289`; repaired/introduced counts were `85/194`, `119/118`, and `102/112`, with mean inversion increase over `10%` and seed `0` increasing by `109`.
- The frozen decision is `C39_POSITIVE_DAMAGE`; strict best remains `KEEP_DEMA_C17_STRICT_BEST`, with no C39 deployment checkpoint. C39's trainable pairwise cross-modal relation stack is closed; no C39-v2 or relation-layer sweep is authorized.

## 2026-07-16 Goal DEMA_HT_AUC_090_PLUS: Phase C40-BCEQ

### Pre-Edit Contract

- C39 showed that a trainable three-way modality relation stack is unstable: it produced a severe seed-42 positive collapse and did not improve the frozen C17 evidence route. The next bottleneck is whether continuous biochemical measurements are being used only as projected evidence rather than as a patient-level clinical condition.
- C40 tests Biochemical-Conditioned Evidence Query (BCEQ), a new patient-level mechanism: raw continuous biochemical values from the longitudinal record form a nonlinear latest/history/change state, which becomes a clinical query over a fixed image/report evidence memory. The query learns which visual morphology and report evidence are concordant with the biochemical state; it is not a visit scorer and does not form pairwise relation tokens.
- Frozen C17 image and text encoders plus their pre-propagation evidence projectors create four fixed evidence tokens: latest image, historical image, latest report, and historical report, with fixed inverse-log2 ordinal recency for the historical summaries. A trainable biochemical state encoder uses only `bio_values` and availability masks for aggregation, not missingness as a classifier feature. One biochemical query attends to the four evidence tokens, followed by a low-capacity patient readout and BCE classifier.
- C17 mechanism propagation/readout, C27 temporal scorer, C37 end-to-end route, C38/C39 modules, saved predictions, dates, patient IDs, visit counts, image/report counts, padding fields, and shortcut fields are excluded from predictor inputs. Evidence validity and biochemical observation masks are used only to form finite clinical summaries and attention masks; no validity scalar is concatenated to the classifier.
- The sole training loss is BCE with logits. Formal seeds remain `[0, 42, 3407]`, each with one independent model, optimizer, Validation-AUC-selected checkpoint, and one deployment forward. No ensemble, averaging, AUPRC, calibration, threshold tuning, Test-based selection, smoke, pilot, or sweep is permitted. Test remains blocked until the Validation decision is frozen.
- C40 promotion uses the goal gates: mean Validation AUC `>= 0.9000`, at least `2/3` seeds `>= 0.9000`, std `<= 0.025`, positive safety versus C17, ranking safety versus C27, shortcut safety, finite/noncollapsed training health, and patient-level split/label/test isolation. If C40 fails, it is the third distinct post-goal whole-model hypothesis and the later data-limit audit may only begin after its results are combined with C38/C39 evidence.
- Starting implementation commit for C40 is `bf41eff`. The C40 implementation, minimal gate, formal run, final decision, and next route will be recorded below.

### C40 Formal Result And Decision

- C40 implementation was committed and pushed at `e946e00`. The canonical server pulled that commit, compiled it in `/home/linruixin/chen/conda/envs/ma`, passed the exact `11/11` gate with `C40_BCEQ_DIRECT_MULTI_SEED_AUTHORIZED`, and completed direct parallel Validation seeds `[0, 42, 3407]` on the NVIDIA GeForce RTX 5090. The run status was `COMPLETE`; no smoke or pilot was used.
- Validation-selected epochs were seed `0: 1`, seed `42: 4`, and seed `3407: 4`. C40 Validation AUC was `0.8469895880 / 0.8610230874 / 0.8641919421`, mean/std `0.8574015392 +/- 0.0091551600`. C40-C17 was `-0.0230873699 / -0.0158442734 / +0.0022634677`, mean `-0.0122227252`; C40-C27 was `-0.0547759168 / -0.0104119511 / -0.0095065640`, mean `-0.0248981440`. The `0.90` target was not reached.
- Training health passed `9/9`, capacity passed, and selected-structure shortcut safety passed for all seeds. The selected-structure shortcut-only label AUC was `0.2833861476`; audit-only raw visit/image association warnings were not model inputs. Reporting-only Test was evaluated only after the Validation decision and had mean/std AUC `0.8002645503 +/- 0.0051438615`.
- Positive preservation failed: C17 TP-to-C40-FN / FN-to-C40-TP was `16/0`, `8/1`, and `2/6`, aggregate `26/7`. Sensitivity changes versus C17 were `-0.3404255319 / -0.1489361702 / +0.0851063830`; seeds `0` and `42` showed substantive positive damage.
- Pairwise ranking failed: C17/C27/C40 inversion counts were `287/217/338`, `272/284/307`, and `305/279/300`. Relative to C27, C40 introduced `198`, `135`, and `105` pairs after repairing `77`, `112`, and `84`; aggregate repaired/introduced counts were `273/438`, and mean inversions increased from `260.0` to `315.0` (`+21.15%`).
- The Validation decision was frozen before Test as `C40_POSITIVE_DAMAGE`; `auc_gate_pass`, `positive_safety_pass`, and `ranking_safety_pass` were false, while `shortcut_safety_pass`, `training_health_pass`, and `capacity_gate_pass` were true. The strict best remains `KEEP_DEMA_C17_STRICT_BEST`; no C40 deployment checkpoint was selected.
- C38, C39, and C40 now provide three genuinely different whole-model hypotheses. All three completed with healthy training and shortcut safety, but their mean Validation AUC values were `0.8675116946`, `0.8644937377`, and `0.8574015392`, all below `0.89`. This satisfies the entry condition for a data-limit audit, but not yet the stronger conclusion that missing, weak, or semantically ambiguous evidence explains the common errors. No new model route is authorized before that audit.

## 2026-07-16 Goal DEMA_HT_AUC_090_PLUS: Validation-Only Data-Limit Audit

### Audit Contract

- The audit uses only the fixed Validation predictions from C17, C27, C38, C39, and C40 for seeds `[0, 42, 3407]`. It never opens a Test prediction, Test label, or Test-derived artifact, and it does not train a classifier or alter any model.
- It verifies the canonical manifest SHA-256, patient-level split counts, label alignment, prediction patient alignment, Validation-only filenames, and the frozen decision flags for the three post-goal hypotheses. It then measures common false-negative/false-positive patients, pairwise ranking burden, error concentration, and audit-only evidence strata from the manifest.
- Evidence fields are descriptive only: follow-up coverage, dated biochemical availability, weak evidence labels, report availability, and visit/image counts are never passed to a predictor. The audit can conclude `DEMA_HT_AUC_090_DATA_LIMIT_SUSPECTED` only if the three healthy safe hypotheses remain below `0.89`, common errors concentrate in the same patients, and the evidence/label audit provides a reproducible ambiguity or missing-evidence signal while all split, leakage, and selection checks pass.
- Starting audit implementation will be recorded below with its server report, conclusion, and the next goal decision.

### Audit Result And C41 Route

- Audit implementation was committed and pushed at `4608290`; the canonical server fast-forwarded to that commit and completed `GOAL_DATA_LIMIT_AUDIT` in `/home/linruixin/chen/conda/envs/ma`.
- The manifest SHA-256 matched `cc19e7d1088a5df79b937fc8db4196300796a2adbfe2cb49f42be0f99b4a5b9b`; all C17/C27/C38/C39/C40 Validation patient IDs and labels aligned, all three candidate decision JSONs passed health, shortcut, and Validation/Test isolation checks, and no Test artifact was opened. The structural audit therefore passed.
- Candidate means remained C38 `0.8675116946`, C39 `0.8644937377`, and C40 `0.8574015392`. Common errors were concentrated in the top quartile (`0.7407407407` of all error counts), but only `8/47` positives (`0.1702127660`) were common hard false negatives and only `16/94` patients (`0.1702127660`) met the common-hard threshold. Evidence conflict was `0.375` among common hard positives versus `0.3589743590` among other positives, with no enriched evidence stratum. The formal conclusion is `DATA_LIMIT_AUDIT_INCONCLUSIVE`, not `DEMA_HT_AUC_090_DATA_LIMIT_SUSPECTED`.
- The audit does not satisfy the goal's common-patient and evidence-ambiguity conditions, so the goal remains active and the strict best remains `KEEP_DEMA_C17_STRICT_BEST`. The audit artifacts are under server-side `analysis_reports/goal_auc_090/data_limit_audit/`.

## 2026-07-16 Goal DEMA_HT_AUC_090_PLUS: Phase C41-MELR

### Pre-Edit Contract

- C38-C40 tested patient evidence-set query pooling, explicit cross-modal relation tokens, and a biochemical-conditioned evidence query. They were healthy and shortcut-safe but all remained below `0.89`; the Validation-only data-limit audit was inconclusive because common hard patients and evidence enrichment were insufficient.
- C41 tests a new longitudinal organization: each frozen C17 modality evidence stream is summarized by four fixed patient-level trajectory statistics, namely latest evidence, inverse-log2 historical mean, latest-history change, and historical dispersion. This represents uncertainty and trajectory heterogeneity without learning a visit score or selecting visits.
- Three modality trajectory encoders produce image, report, and biochemical evidence states. Each state has an independent clinical evidence logit. A simplex router assigns nonnegative evidence weights inside one model, and an additive consensus readout captures agreement among the routed states. This is a new evidence-logit routing mechanism, not C38 query pooling, C39 pairwise relation stacking, C40 biochemical querying, a residual, or a visit-level scorer.
- Only the frozen C17 image/text/bio encoders and pre-propagation evidence projectors provide inputs. Dates, patient IDs, visit counts, raw image/report counts, padding, missingness shortcut fields, saved predictions, and Test artifacts are excluded from predictor inputs. Missing evidence is used only to mask a trajectory token and never concatenated as a classifier feature.
...
### C45 Formal Result And Decision

- The C45 gate passed exactly `11/11`; direct parallel formal seeds `[0, 42, 3407]` completed with training-only modality dropout and same-model EMA validation selection. Validation-selected epochs were seed `0: 3`, seed `42: 9`, and seed `3407: 5`. Validation AUC was `0.8365776369 / 0.8746038932 / 0.8646446356`, mean/std `0.8586087219 +/- 0.0197186011`; the `0.9000` target was not reached.
- C45 was below C17 by `-0.0110155425` mean AUC and below C27 by `-0.0236909612`. Positive preservation passed with C17 TP-to-C45-FN / FN-to-C45-TP of `3/8`, `3/3`, and `1/8`; sensitivity changes were `+0.1063829787`, `0.0`, and `+0.1489361702`.
- Ranking safety failed because C27/C45 inversion counts were `217/361`, `284/277`, and `279/299`; repaired/introduced pairs were `76/220`, `115/108`, and `100/120`, aggregate `291/448`. Seed 0 introduced `144` additional inversions. Training health passed `9/9`, capacity passed, and selected-structure shortcut safety passed for all seeds with shortcut-only label AUC `0.2833861476`.
- Validation was frozen before reporting-only Test as `C45_RANKING_DAMAGE`; Test AUC mean/std was `0.8335222978 +/- 0.0121099546`. No checkpoint was deployed, and no ensemble, threshold tuning, or Test-based selection was used. C45 is not data-limit-safe; the strict best remains `KEEP_DEMA_C17_STRICT_BEST`, and the goal remains active.

## 2026-07-16 Goal DEMA_HT_AUC_090_PLUS: Phase C46-MCFS

### Pre-Edit Contract

- C45 showed that a three-modality shared consensus state is too lossy and destabilizes patient ranking, even though its positive sensitivity and shortcut behavior were safe. C46 therefore changes the evidence unit rather than tuning C45: it preserves the five fixed mechanism slots exposed by the frozen C17 evidence projectors.
- C46 tests Mechanism-Conditioned Fixed-State readout. For every patient, the five mechanism streams are built from the existing C17 evidence nodes using the established morphology, biochemical, and report-role mapping. Each mechanism uses fixed latest, inverse-log2 historical mean, latest-history delta, and historical dispersion. A shared mechanism encoder maps the five mechanism summaries; fixed-order mechanism states, global consensus, and cross-mechanism discordance are read by one patient classifier.
- The mechanism order is semantic and fixed; there is no learned visit score, temporal attention, visit selector, router, patient ID, date, visit-count feature, image/report count, padding field, source path, saved prediction, missingness classifier feature, or Test artifact. Missing evidence only masks a mechanism state before fixed pooling.
- C46 uses BCE with logits only, one independent model/checkpoint per seed, formal seeds `[0, 42, 3407]`, Validation-AUC checkpoint selection, direct parallel execution after an exact gate, and reporting-only Test after the Validation decision. No ensemble, averaging, calibration, threshold tuning, secondary metric, smoke, pilot, sweep, EMA, or closed-route micro-variant is authorized.
- The trainable scope is limited to the shared mechanism encoder, fixed-order patient readout, and classifier; all C17 encoders and pre-propagation evidence projectors remain frozen. The declared trainable parameter limit is `5,000,000`.
- Promotion requires the goal AUC gates plus positive preservation versus C17, ranking safety versus C27, shortcut safety, finite training health, capacity, and patient-level split/Test isolation. If C46 fails, record the formal result and continue to the next genuinely different whole-model hypothesis unless the complete data-limit stop criteria are evidenced.
- Starting implementation for C46-MCFS is authorized after this contract; local static checks, exact gate, direct formal seeds, Validation freeze, and reporting-only Test are required.

### C46 Formal Result And Decision

- The C46 gate passed exactly `11/11`; direct parallel formal seeds `[0, 42, 3407]` completed with fixed mechanism-slot trajectories and one Validation-AUC-selected model per seed. Validation-selected epochs were seed `0: 6`, seed `42: 9`, and seed `3407: 8`. Validation AUC was `0.8709823450 / 0.8619284744 / 0.8669081032`, mean/std `0.8666063075 +/- 0.0045344739`; the `0.9000` target was not reached.
- C46 was below C17 by `-0.0030179568` mean AUC and below C27 by `-0.0156933756`. Positive preservation failed: C17 TP-to-C46-FN / FN-to-C46-TP was `3/10`, `17/1`, and `3/5`; seed 42 sensitivity changed by `-0.3404255319`, with aggregate counts `23/16`.
- Ranking safety failed. C27/C46 inversion counts were `217/285`, `284/305`, and `279/294`; repaired/introduced pairs were `98/166`, `129/150`, and `94/109`, aggregate `321/425`. Training health passed `9/9`, capacity passed, and selected-structure shortcut safety passed for all seeds with shortcut-only label AUC `0.2833861476`.
- Validation was frozen before reporting-only Test as `C46_POSITIVE_DAMAGE`; Test AUC mean/std was `0.8036659108 +/- 0.0278720999`. No checkpoint was deployed, and no ensemble, threshold tuning, or Test-based selection was used. C46 is not data-limit-safe; the strict best remains `KEEP_DEMA_C17_STRICT_BEST`, and the goal remains active.

## 2026-07-16 Goal DEMA_HT_AUC_090_PLUS: Phase C47-DRFE

### Pre-Edit Contract

- C46 retained mechanism semantics but still produced seed-dependent positive damage. The unresolved representation risk is that the aligned evidence projectors alone discard useful low-level encoder state. C47 therefore tests a dual-resolution representation without changing the patient chronology rule.
- C47 tests Dual-Resolution Fixed Evidence. For each visit, three frozen C17 raw encoder states and three frozen C17 pre-propagation aligned evidence states are kept as six separate streams. Every stream receives the same fixed latest/history/delta/dispersion statistics. One shared stream encoder maps the six summaries; fixed stream order, cross-stream consensus, and cross-stream discordance are read by one patient classifier.
- The six stream order is fixed to `raw_image`, `raw_text`, `raw_bio`, `aligned_image`, `aligned_text`, and `aligned_bio`. There is no learned visit score, temporal attention, router, visit selector, patient ID, date, visit-count feature, image/report count, padding field, source path, saved prediction, missingness classifier feature, or Test artifact. Missingness only masks a stream before fixed pooling.
- C47 uses BCE with logits only, one independent model/checkpoint per seed, formal seeds `[0, 42, 3407]`, Validation-AUC checkpoint selection, direct parallel execution after an exact gate, and reporting-only Test after the Validation decision. No ensemble, averaging, calibration, threshold tuning, secondary metric, smoke, pilot, sweep, EMA, or closed-route micro-variant is authorized.
...
### C48 Formal Result And Decision

- The initial C48 gate stopped at configuration construction because the shared C47 source implementation expected a `c47` block; no C48 seed started. The source configuration was adapted to the C48 stream contract, local static checks passed, and the corrected gate then passed exactly `11/11` before direct training.
- Direct parallel formal seeds `[0, 42, 3407]` completed. Validation-selected epochs were seed `0: 12`, seed `42: 4`, and seed `3407: 5`. Validation AUC was `0.8795835220 / 0.8899954731 / 0.8727931191`, mean/std `0.8807907047 +/- 0.0086644800`; the `0.9000` target was not reached.
- C48 improved mean AUC versus C17 by `+0.0111664403` but remained `-0.0015089784` versus C27. Positive preservation failed: C17 TP-to-C48-FN / FN-to-C48-TP was `3/6`, `15/0`, and `2/5`; seed 42 sensitivity changed by `-0.3191489362`, with aggregate counts `20/11`.
- Ranking safety failed. C27/C48 inversion counts were `217/266`, `284/243`, and `279/281`; repaired/introduced pairs were `108/157`, `115/74`, and `103/105`, aggregate `326/336`. Training health passed `9/9`, capacity passed, and selected-structure shortcut safety passed for all seeds with shortcut-only label AUC `0.2833861476`.
- Validation was frozen before reporting-only Test as `C48_POSITIVE_DAMAGE`; Test AUC mean/std was `0.8350340136 +/- 0.0094689870`. No checkpoint was deployed, and no ensemble, threshold tuning, or Test-based selection was used. C48 is not data-limit-safe; the strict best remains `KEEP_DEMA_C17_STRICT_BEST`, and the goal remains active.

## 2026-07-16 Goal DEMA_HT_AUC_090_PLUS: Phase C49-CREB

### Pre-Edit Contract

- C47 established that raw and aligned C17 states jointly carry the strongest post-C17 signal, while C48 showed that a global signed split alone does not stabilize the seed-42 positive side. C49 changes the evidence unit again by binding raw and aligned states within each modality before patient pooling.
- C49 tests Cross-Resolution Evidence Binding. For image, text, and biochemical evidence separately, each visit forms a learned interaction token from raw state, aligned state, absolute discrepancy, and elementwise product. The three bound modality streams then use fixed latest/history/delta/dispersion statistics; one shared bound-stream encoder, fixed consensus, and fixed discordance feed one patient classifier.
- The three modality order and chronology are fixed. There is no learned visit score, temporal attention, router, visit selector, patient ID, date, visit-count feature, image/report count, padding field, source path, saved prediction, missingness classifier feature, or Test artifact. Missingness only masks a bound stream before pooling.
- C49 uses BCE with logits only, one independent model/checkpoint per seed, formal seeds `[0, 42, 3407]`, Validation-AUC checkpoint selection, direct parallel execution after an exact gate, and reporting-only Test after the Validation decision. No ensemble, averaging, calibration, threshold tuning, secondary metric, smoke, pilot, sweep, EMA, or closed-route micro-variant is authorized.
- The trainable scope is limited to the shared raw/aligned binding encoder, shared bound-stream trajectory encoder, patient readout, and classifier; all C17 encoders and pre-propagation evidence projectors remain frozen. The declared trainable parameter limit is `5,000,000`.
- Promotion requires the goal AUC gates plus positive preservation versus C17, ranking safety versus C27, shortcut safety, finite training health, capacity, and patient-level split/Test isolation. If C49 fails, record the formal result and continue to the next genuinely different whole-model hypothesis unless the complete data-limit stop criteria are evidenced.
- Starting implementation for C49-CREB is authorized after this contract; local static checks, exact gate, direct formal seeds, Validation freeze, and reporting-only Test are required.

### C49 Formal Result And Decision

- The C49 gate passed exactly `11/11` on the canonical server. Direct parallel formal seeds `[0, 42, 3407]` completed on the NVIDIA GeForce RTX 5090 in the `ma` environment; Validation-selected epochs were seed `0: 19`, seed `42: 3`, and seed `3407: 2`.
- C49 Validation AUC was `0.8809416025 / 0.8918062472 / 0.8786781349`, mean/std `0.8838086615 +/- 0.0070179665`. The mean improved versus C17 by `+0.0141843972` and versus C27 by `+0.0015089784`, but no seed reached `0.9000`, so the goal AUC gate failed.
- Positive preservation failed: C17 TP-to-C49-FN / FN-to-C49-TP was `5/7`, `14/0`, and `1/7`; sensitivity changes were `+0.0425531915`, `-0.2978723404`, and `+0.1276595745`. Seed 42 had substantive positive damage, with aggregate counts `20/14`.
- Ranking safety failed. C27/C49 inversion counts were `217/263`, `284/239`, and `279/268`; repaired/introduced pairs were `124/170`, `127/82`, and `109/98`, aggregate `360/350`. Seed 0 increased inversions by `46` and exceeded the per-seed bound; its relative increase was `21.20%`.
- Training health passed `9/9`, capacity passed, and selected-structure shortcut safety passed for all seeds. The shortcut-only label AUC was `0.2833861476`; raw visit/image associations remained audit-only. Reporting-only Test AUC mean/std was `0.8265306122 +/- 0.0162234558`.
- Validation was frozen before Test as `C49_POSITIVE_DAMAGE`; no deployment checkpoint was selected. The strict best remains `KEEP_DEMA_C17_STRICT_BEST`, and no ensemble, threshold tuning, or Test-based selection was used.
- C49 is not evidence for the data-limit stop condition because its global AUC improved but positive and ranking safety failed. The goal remains active. The next hypothesis must change the patient readout geometry while preserving the useful C49 dual-resolution information and must not reopen C27/C37 visit-level tuning.

## 2026-07-16 Goal DEMA_HT_AUC_090_PLUS: Phase C50-CBIF

### Pre-Edit Contract

- C49 improved mean Validation AUC to `0.8838086615`, but binding raw and aligned streams before longitudinal pooling caused a seed-42 sensitivity collapse and a seed-0 ranking increase. The next bottleneck is patient-level organization and clinical conditioning, not another visit-level binding variant.
- C50 tests Continuous Biochemical Interaction Fusion. The frozen C17 raw and pre-propagation aligned states are first converted into fixed latest/history/delta/dispersion patient trajectories for all six streams. Raw/aligned resolution fusion is then performed after patient trajectory formation; image and text form a joint state from concatenation, absolute disagreement, and elementwise interaction.
- A separate fixed latest/history/delta/dispersion summary of the continuous biochemical values is encoded as a clinical condition. A bounded patient-level feature modulation conditions the image-text joint state on this biochemical trajectory, and one patient readout consumes the conditioned joint state together with the three modality states and fixed consensus/discordance.
- C50 is not C49's pre-trajectory binding, C39's pairwise relation stack, C40's biochemical query, or C43's within-visit FiLM route. It has no learned visit score, temporal attention, router, visit selector, patient ID, date, visit-count feature, image/report count, padding field, source path, saved prediction, missingness classifier feature, or Test artifact.
- C50 uses BCE with logits only, one independent model/checkpoint per seed, formal seeds `[0, 42, 3407]`, Validation-AUC checkpoint selection, direct parallel execution after an exact gate, and reporting-only Test after the Validation decision. No ensemble, averaging, calibration, threshold tuning, secondary metric, smoke, pilot, sweep, EMA, or ranking loss is authorized.
...
## 2026-07-16 Goal DEMA_HT_AUC_090_PLUS: Phase C55-PESK

### Pre-Edit Contract

- C54-LRRA preserved positive sensitivity and passed health/capacity/shortcut checks, but its learned low-rank source transport increased seed-0 patient ranking inversions. The binding bottleneck is stable organization of multiple patient evidence instances, not another source adapter.
- C55 tests Patient Evidence Set Kernel. The six frozen C17 raw/aligned stream states remain unchanged. Within each chronological visit, raw and aligned states of each modality are combined only by a fixed availability-weighted mean; no trainable source adapter or evidence-projector update is allowed.
- The patient representation contains fixed latest, inverse-log2 historical mean, latest-history delta, and historical dispersion for all six streams. It additionally contains fixed patient-level interaction moments for image-text, image-bio, and text-bio Hadamard products and their absolute cross-modal disagreement. These interactions are summarized over the patient set before the classifier; there is no learned visit score, temporal attention, router, visit selector, per-visit ranking head, or visit-level supervision.
- C55 is distinct from C39's trainable per-visit relation stack, C44's attention-based set encoder, C50's biochemical conditioning route, C51's distribution-only set readout, and C54's low-rank representation adaptation. The new mechanism is a fixed patient-level cross-modal evidence kernel with first-order and dispersion moments, followed by one low-capacity readout.
- Patient IDs, dates, visit counts, image/report counts, padding fields, source paths, saved predictions, missingness classifier features, and Test artifacts remain excluded from model inputs. Missingness is used only to form valid fixed moments and masks.
- C55 uses BCE with logits only, one independent model/checkpoint per seed, formal seeds `[0, 42, 3407]`, Validation-AUC checkpoint selection, direct multiseed execution after an exact gate, and reporting-only Test after the Validation decision. No ensemble, averaging, calibration, threshold tuning, secondary metric, smoke, pilot, sweep, EMA, or ranking loss is authorized.
...
## 2026-07-16 Goal DEMA_HT_AUC_090_PLUS: Phase C59-PMESE

### Pre-Edit Contract

- C58 failed because patient-level three-way tensor products amplified unstable cross-modal directions: mean Validation AUC fell to `0.8632865550`, positive sensitivity damage occurred in seed 42, and all seeds introduced more C27 inversions than they repaired. C55 remains the strongest AUC signal at `0.8975403652`, but its unconstrained patient kernel readout damaged positives. The unresolved bottleneck is the evidence unit and its patient-level aggregation order.
- C59 tests Patient-level Multimodal Evidence Set Encoding. For every valid historical visit, the frozen C17 raw/aligned image, text, and biochemical states are first combined into one multimodal evidence instance. Each modality contributes an availability-weighted raw/aligned mean and a fixed raw/aligned resolution gap; the instance also contains the three bounded elementwise cross-modal products. A shared low-capacity instance encoder maps this joint visit evidence to one token.
- The patient representation is formed from the encoded evidence instances with fixed chronology only: latest instance, inverse-log2 recency-weighted historical mean, latest-history delta, historical dispersion, masked set maximum, and recency-weighted patient mean. These are patient-level multi-instance moments, not per-visit predictions, learned visit scores, temporal attention, routers, or selectors. No visit label or visit-level loss is used.
- C59 is a new evidence-organization hypothesis: it fuses all three modalities before patient-level multi-instance aggregation, whereas C49 binds raw/aligned states within each modality before trajectory pooling, C51 pools six stream instances, C55 forms patient-level products after trajectory statistics, and C58 forms patient-level tensor products after modality summaries. The model uses one patient readout and one classifier in one forward.
- Patient IDs, dates, visit counts, image/report counts, padding fields, source paths, saved predictions, missingness classifier features, and Test artifacts remain excluded. Missingness only gates valid evidence instances and fixed statistics. All C17 encoders, propagation modules, and pre-propagation evidence projectors remain frozen.
...
### C59 Formal Result And Decision

- The C59 gate passed exactly `11/11` on the canonical server. Direct parallel formal seeds `[0, 42, 3407]` completed in the `ma` environment on the NVIDIA GeForce RTX 5090; Validation-selected epochs were seed `0: 9`, seed `42: 13`, and seed `3407: 27`.
- C59 Validation AUC was `0.8759619737 / 0.8967858760 / 0.9112720688`, mean/std `0.8946733062 +/- 0.0177495892`. The mean improved versus C17 by `+0.0250490418` and versus C27 by `+0.0123736231`; only seed 3407 reached `0.9000`, so the goal AUC gate failed.
- Positive preservation failed: C17 TP-to-C59-FN / FN-to-C59-TP was `10/4`, `5/3`, and `9/4`; sensitivity changes were `-0.1276595745`, `-0.0425531915`, and `-0.1063829787`, with aggregate counts `24/11`. Seeds 0 and 3407 crossed the substantive sensitivity-loss bound.
- Ranking safety failed on the per-seed criterion. C27/C59 inversion counts were `217/274`, `284/228`, and `279/196`; repaired/introduced pairs were `77/134`, `129/73`, and `172/89`, aggregate `378/296`. The aggregate mean improved, but seed 0 increased inversions by `57` and exceeded the allowed per-seed increase.
- Training health passed `9/9`, capacity passed, and selected-structure shortcut safety passed for all seeds. The shortcut-only label AUC was `0.2833861476`; raw visit/image associations remained audit-only. Reporting-only Test AUC mean/std was `0.8563869992 +/- 0.0104888051`.
- Validation was frozen before Test as `C59_POSITIVE_DAMAGE`; no deployment checkpoint was selected. The strict best remains `KEEP_DEMA_C17_STRICT_BEST`, and no ensemble, threshold tuning, or Test-based selection was used.
- C59 is not evidence for the data-limit stop condition because the mean is near but below `0.895`, positive safety failed, and one seed had ranking damage despite the aggregate ranking gain. The goal remains active; the next hypothesis tests single-model EMA stabilization of this patient-level evidence-set route, an explicitly permitted training-stability mechanism rather than a second model or ensemble.

## 2026-07-16 Goal DEMA_HT_AUC_090_PLUS: Phase C60-EMASE

### Pre-Edit Contract

- C59 is the closest recent patient-level route to the AUC target: mean Validation AUC `0.8946733062`, seed 3407 AUC `0.9112720688`, and aggregate C27 ranking improved. Its failure is concentrated in optimization stability and decision-side reproducibility: selected epochs were `9/13/27`, seed 0 increased inversions by `57`, and seeds 0 and 3407 crossed the positive-sensitivity safety bound, while structural health and shortcut safety passed.
- C60 tests EMA-Stabilized Patient Evidence Set Encoding. The patient-level multimodal evidence-set architecture is retained as the fixed model hypothesis, but every optimizer step updates one exponential moving-average state of that same model's trainable parameters with a fixed decay `0.99`. Validation checkpoint selection, reporting-only Test, and final checkpoint loading all use that seed's EMA state.
- EMA is a training-stability mechanism explicitly allowed by the active goal. It is not checkpoint averaging, prediction averaging, an ensemble, a second model, distillation, or a new inference branch: each seed produces one EMA checkpoint, and deployment would execute one model forward from one checkpoint.
- The source route, patient-level evidence-instance construction, fixed chronology, missingness masking, shortcut exclusion, labels, patient-level splits, and task definition remain unchanged from the C59 hypothesis. No visit scorer, temporal attention, router, selector, visit-level label, or ranking loss is introduced.
...
### C60 Formal Result And Decision

- The C60 gate passed exactly `11/11` on the canonical server. Direct parallel formal seeds `[0, 42, 3407]` completed in the `ma` environment on the NVIDIA GeForce RTX 5090; EMA Validation-selected epochs were seed `0: 4`, seed `42: 15`, and seed `3407: 30`.
- C60 Validation AUC was `0.8673607967 / 0.8899954731 / 0.9058397465`, mean/std `0.8877320054 +/- 0.0193390759`. The mean improved versus C17 by `+0.0181077411` and versus C27 by `+0.0054323223`; only seed 3407 reached `0.9000`, so the goal AUC gate failed.
- Positive preservation passed: C17 TP-to-C60-FN / FN-to-C60-TP was `2/7`, `5/3`, and `5/5`; sensitivity changes were `+0.1063829787`, `-0.0425531915`, and `0.0`, with aggregate counts `12/15`. No seed crossed the substantive positive-sensitivity loss bound.
- Ranking safety failed on the per-seed criterion. C27/C60 inversion counts were `217/293`, `284/243`, and `279/208`; repaired/introduced pairs were `77/153`, `112/71`, and `169/98`, aggregate `358/322`. The aggregate mean improved, but seed 0 increased inversions by `76` and exceeded the allowed per-seed increase.
- Training health passed `9/9`, capacity passed, and selected-structure shortcut safety passed for all seeds. The shortcut-only label AUC was `0.2833861476`; raw visit/image associations remained audit-only. Reporting-only Test AUC mean/std was `0.8601662887 +/- 0.0108502473`.
- Validation was frozen before Test as `C60_RANKING_DAMAGE`; no deployment checkpoint was selected. The strict best remains `KEEP_DEMA_C17_STRICT_BEST`, and no ensemble, threshold tuning, or Test-based selection was used.
- C60 shows that EMA improves positive safety but does not recover the target AUC or seed-0 ranking. It is not evidence for the data-limit stop condition. The goal remains active; the next hypothesis adds a fixed nonlinear continuous-bio evidence branch at the patient-level multimodal instance before set aggregation.

## 2026-07-16 Goal DEMA_HT_AUC_090_PLUS: Phase C61-CBPI

### Pre-Edit Contract

- C60 showed that EMA reduced positive-side damage but lowered mean Validation AUC to `0.8877320054` and retained seed-0 ranking damage. The C59 patient-level evidence-set route remains the strongest stable representation signal, while C57 demonstrated that the raw continuous biochemical measurements contain nonlinear information that was not present in C59's frozen biochemical state alone.
- C61 tests Continuous Biochemical Patient-Instance Fusion. For each valid visit, the C59 frozen raw/aligned multimodal evidence instance is encoded once. The seven raw continuous biochemical measurements are separately transformed by the fixed bounded basis `[x, tanh(x), x*tanh(x)]`, with missing values masked, and passed through one trainable continuous-bio encoder. The multimodal instance token, continuous-bio token, and their elementwise product are fused by one joint instance encoder before patient-level set aggregation.
- The patient representation remains the fixed C59 chronology/set moments: latest instance, inverse-log2 recency-weighted historical mean, latest-history delta, historical dispersion, masked set maximum, and recency-weighted patient mean. There is no visit prediction, learned visit score, temporal attention, router, selector, visit label, or visit-level loss.
- C61 is distinct from C57 because continuous bio is fused at the per-visit patient-instance level before set aggregation rather than being paired with a patient-level image-text summary. It is distinct from C58 because it has no patient-level pairwise/triple tensor expansion across all modalities and no EMA training route.
- Patient IDs, dates, visit counts, image/report counts, padding fields, source paths, saved predictions, missingness classifier features, and Test artifacts remain excluded. Missingness only masks the continuous-bio basis and valid evidence instances. All C17 encoders, propagation modules, and pre-propagation evidence projectors remain frozen.
- C61 uses BCE with logits only, independent formal seeds `[0, 42, 3407]`, Validation-AUC checkpoint selection, direct multiseed execution after an exact gate, and reporting-only Test after Validation freeze. No ensemble, averaging, calibration, threshold tuning, secondary metric, smoke, pilot, sweep, EMA, ranking loss, or C27/C37 visit-level route is authorized.
- The trainable scope is limited to the multimodal instance encoder, continuous-bio encoder, joint instance encoder, patient readout, and classifier under a `5,000,000` parameter limit. Promotion requires the goal AUC gates plus positive-preservation, ranking, shortcut, health, capacity, and patient-level split/Test gates. If C61 fails, record the formal result and continue to the next genuinely different whole-model hypothesis unless the complete data-limit stop criteria are evidenced.
- Starting implementation for C61-CBPI is authorized after this contract; local static checks, exact gate, direct formal seeds, Validation freeze, and reporting-only Test are required.

### Implementation Before Formal Run

- Added `dmea_ht/c61_cbpi.py`, `configs/dema_ht_c61_cbpi_multiseed.yaml`, `scripts/gate_phase_c61_cbpi.py`, `scripts/train_phase_c61.py`, and `scripts/collect_phase_c61_report.py`.
- The implementation keeps the C17 source stack under `no_grad`, builds the C59 multimodal per-visit evidence token, adds a separate fixed `[x, tanh(x), x*tanh(x)]` continuous-bio branch, fuses the two branches by one joint instance encoder, and then uses the fixed patient-level set moments and one classifier.
- The gate checks frozen source scope, finite modality-missing outputs, nonzero gradients for the continuous-bio/joint head with zero source gradients, shortcut exclusion, the exact continuous-bio basis and patient-instance route, BCE-only training, Validation/Test isolation, and direct single-model multiseed execution.
- Local Python compilation, YAML parsing, a no-data continuous-bio basis/joint-shape and set-statistics check, and `git diff --check` passed. The exact server gate and three-seed formal results remain pending; no C61 conclusion or Test result is valid before Validation freeze and reporting-only Test.

### C61 Formal Result And Goal Freeze

- The C61 gate passed exactly `11/11` on the canonical server. Direct parallel formal seeds `[0, 42, 3407]` completed in the `ma` environment on the NVIDIA GeForce RTX 5090; Validation-selected epochs were seed `0: 30`, seed `42: 10`, and seed `3407: 26`.
- C61 Validation AUC was `0.8981439565 / 0.9013128112 / 0.9262109552`, mean/std `0.9085559077 +/- 0.0153715952`. The mean improved versus C17 by `+0.0389316433` and versus C27 by `+0.0262562245`; seeds 42 and 3407 reached `0.9000`, and the std remained below `0.025`. The AUC goal gate passed.
- Positive preservation passed: C17 TP-to-C61-FN / FN-to-C61-TP was `7/8`, `2/3`, and `4/5`; sensitivity changes were `+0.0212765957` in all three seeds, with aggregate counts `13/16`.
- Ranking safety passed. C27/C61 inversion counts were `217/225`, `284/218`, and `279/163`; repaired/introduced pairs were `113/121`, `109/43`, and `154/38`, aggregate `376/202`. The largest per-seed increase was `8`, within the safety bound, and the mean inversion count decreased.
- Training health passed `9/9`, capacity passed, and selected-structure shortcut safety passed for all seeds. The shortcut-only label AUC was `0.2833861476`; raw visit/image associations remained audit-only. Reporting-only Test AUC mean/std was `0.8631897203 +/- 0.0082993920`.
...
- Task-semantic caveat: the final manifest stores `selected_visit_dates` and the C13 latest-date field but no explicit `anchor_date` or `target_date`. The exact last-year exclusion is therefore inherited from the manifest builder's declared `history_cutoff=final_year` contract and is not independently reconstructable from the final JSONL alone.

## 2026-07-16 Phase C64-STCV Pre-execution Contract

- The attached C64 plan defines staged tuning and patient-level development cross-validation, with a final held-out Test target of mean AUC at least `0.90`.
- Stage A is fixed Train/Validation only: candidates are `A_CBPI_HEAD_ONLY`, `B_PROJECTOR_CBPI`, and `C_FULL_LOW_LR`, with three formal seeds `[0,42,3407]`, maximum `60` epochs, and Early Stopping patience `15`.
- Candidate A trains only the C61 task path; Candidate B trains evidence projectors plus the C61 task path; Candidate C trains all predictive groups with positive learning-rate factors `0.05/0.10/0.25/1.00` for image-text encoders, biochemical source encoder, evidence projectors, and C61 task path.
- Stage A selection uses Validation AUC mean and predeclared safety tie-breaks only. No Test loader, Test metric, saved prediction input, representation input, C63 checkpoint, C62 checkpoint, ensemble, averaging, or threshold tuning is authorized.
- Stage B uses the 696-patient Train plus Validation development pool in five patient-level stratified folds with fold seed `20260716`, for `15` fold-seed runs. The gate requires mean OOF AUC at least `0.8900`, at least two seed OOF AUC values at least `0.8900`, OOF standard deviation at most `0.020`, minimum fold AUC at least `0.8400`, shortcut safety, and complete parameter-update health.
- Stage C is authorized only after the OOF gate. It trains all 696 development patients with a fixed per-seed epoch equal to the median selected fold epoch, with no Early Stopping. Test is evaluated once per final checkpoint only after this contract is frozen; deployment seed is predeclared as `0` and cannot be chosen by Test performance.
- Added the C64 common utilities, gate, Stage A/CV/final drivers and collectors, deterministic fold builder, and unattended autorun driver. Local Python compilation, YAML parsing, and `git diff --check` passed; no local data or training was run.
- The implementation was committed as `931581d`, pushed to GitHub, and fast-forwarded to the canonical server checkout by verified Git bundle because the server GitHub pull path has the known TLS failure.
- Server fold construction passed with fold seed `20260716`, five folds, development patient count `696`, fold sizes `140/140/140/138/138`, balanced labels in every fold, zero Test overlap, and `test_loaded=false`.
...
## 2026-07-17 Phase C65-VACS Pre-execution Contract

- C64 selected `A_CBPI_HEAD_ONLY` with Stage-A Validation AUC `0.9126301494 +/- 0.0197272599` and completed all `15` five-fold-by-three-seed CV runs. Its mean OOF AUC was `0.9251993`, but OOF AUC standard deviation was `0.0528677`, above the predeclared `0.0200` stability gate; all other reported OOF checks passed.
- C65-A is an analysis-only variance attribution stage. It may read the retained C64 CV outputs, C64 reports, the three C61 checkpoints, and the `696` development patients; it must not run optimizer steps, modify model parameters, load Test, or relax the C64 gate.
- C65-A will reproduce per-seed OOF predictions and patient coverage, build the `5 x 3` fold-seed matrix, audit selected epochs, decompose Seed, Fold, and Seed-by-Fold interaction variance, and measure cross-seed prediction stability.
- C65-A will run read-only forward passes of the three C61 checkpoints on the same development patients and compare frozen representations using linear CKA, patient-distance Spearman correlation, `k=10` neighbor Jaccard, and representation norms.
- C65-B is authorized only if OOF reproduction and fold/data integrity pass, Test remains unread, and C65-A supports material frozen-backbone or Seed-related variance: mean CKA `< 0.90`, mean distance Spearman `< 0.90`, Seed plus interaction variance at least `40%`, or mean cross-seed OOF probability Spearman `< 0.80`.
...
### C66 Route-Stage Recovery

- The first direct C66 execution completed all three fold-0 inner-source shards for Seeds `0`, `42`, and `3407` without Test access, then stopped before any Route F training step because `optimizer_and_inventory` incorrectly looked for a top-level `route_training.learning_rate_factors` key. Route-specific factors are declared only under `route_f` and `route_e`.
- The completed source artifacts, checkpoints, predictions, and raw audits are retained. No source training, outer refit, final training, or Test evaluation is invalidated or repeated by the recovery.
- The source raw update audit showed finite, nonzero aggregate updates in every declared source optimizer group, while a few inactive per-parameter branches remained unchanged. The original health code incorrectly required every trainable tensor in a group to update, causing a false group-health failure despite valid group-level gradients and aggregate parameter movement.

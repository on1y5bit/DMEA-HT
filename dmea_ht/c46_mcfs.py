...
from dmea_ht.c27_vtme import FrozenC17EvidenceBackbone, masked_mean
...
class C46MCFSModel(nn.Module):
    """Fixed mechanism-slot trajectories with shared patient evidence readout."""

    def __init__(self, config: Dict[str, Any], seed: int) -> None:
        super().__init__()
        model_cfg = dict(config["model"])
        c46_cfg = dict(config["c46"])
        hidden_dim = int(model_cfg["hidden_dim"])
        dropout = float(model_cfg["dropout"])
        self.hidden_dim = hidden_dim
        self.seed = int(seed)
        self.sources = FrozenC17EvidenceBackbone(config, seed, trainable=False)
        ...
        if list(c46_cfg["mechanism_slots"]) != ["M1", "M2", "M3", "M4", "M5"]:
            raise RuntimeError("C46 mechanism slot order is fixed to M1..M5")

    def train(self, mode: bool = True) -> "C46MCFSModel":
        nn.Module.train(self, mode)
        self.sources.eval()
        return self

    def _mechanism_source_states(self, batch: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        text_available = batch["visit_text_valid"].flatten(0, 1).bool()
        ...
        return {
            "states": source_states.view(batch_size, visits, 5, self.hidden_dim),
            "valid": source_valid.view(batch_size, visits, 5) & visit_mask.unsqueeze(-1),
    ...
    def forward(self, batch: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        with torch.no_grad():
            source = self._mechanism_source_states(batch)
        ...
        mechanism_tokens = self.mechanism_encoder(summary)
        ...
        weights = safe_available.to(mechanism_tokens.dtype)
        mechanism_tokens = mechanism_tokens * weights.unsqueeze(-1)
        denominator = weights.sum(dim=1, keepdim=True).clamp_min(1.0)
        consensus = (mechanism_tokens * weights.unsqueeze(-1)).sum(dim=1) / denominator
        discordance = (
            (mechanism_tokens - consensus.unsqueeze(1)).abs() * weights.unsqueeze(-1)
        ).sum(dim=1) / denominator
        patient_input = torch.cat(
            [mechanism_tokens.flatten(start_dim=1), consensus, discordance], dim=-1
        )
        patient_state = self.patient_readout(patient_input)
        logit = self.classifier(patient_state).squeeze(-1)
        evidence_tokens = torch.stack(
            [consensus, discordance, consensus * discordance, mechanism_tokens.mean(dim=1)], dim=1
        )
        evidence_valid = available.any(dim=1).unsqueeze(1).expand(-1, 4)
        attention = evidence_valid.to(mechanism_tokens.dtype)
        attention = attention / attention.sum(dim=1, keepdim=True).clamp_min(1.0)
        bio_states = source["states"][:, :, 1:3]
        bio_valid = source["valid"][:, :, 1:3]
        bio_weights = bio_valid.to(bio_states.dtype)
        bio_denominator = bio_weights.sum(dim=(1, 2), keepdim=True).clamp_min(1.0)
        bio_state = (bio_states * bio_weights.unsqueeze(-1)).sum(dim=(1, 2)) / bio_denominator.squeeze(-1)
        return {
            "logit": logit,
